"""Start-state kinematics.

``test_too_few_waypoints_raises`` passes already (it exercises the input guard). The two
``xfail`` tests are the **implementation target**: fill in the TODO ① in
``start_state.initial_state_from_track`` and they turn green (xpass).
"""
...
from flight_scenarios.start_state import initial_state_from_track
...
@pytest.mark.xfail(reason="implement TODO ① in start_state.initial_state_from_track", strict=False)
