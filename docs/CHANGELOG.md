...
### 2026-09-25 — Training：审查的遗留项补完

整模块审查后留在模块内、当时没改的几项，全部补上（分支 `dev-autopilot-live`，用户合并）。

- **连点不再排队**：请求带页面的 `clientId` 和页面给它的编号 `seq`；同一页面编号更大的请求一到，编号更小的——排队的不飞，
  正在飞的在下一个周期前停下（`fly_until` 每个周期先问 `superseded()`），晚到的立即拒绝——以 409（`errors.Superseded`）结束。
  以页面的编号为准：审查指出，只按到达后端的先后排，机器忙时先发的请求后到，屏幕上的那一个反而会被拒。
- **航向带被段尾截短就拒绝**（`payload.band_cut_by_stop`）：段尾按句子步、带按飞过的步，两者分开时带会被截短还照判。只查
  段尾结束的飞行（到了段尾且结局是时限，`ended_at_stop`；审查发现：越过入口没截获后执行器还会接着飞到段尾，原来那样的正常
  答复会被错拒成 500）。按现在的执行器它不会发生（段尾与听词都在开始一步的周期上，两步的提前量总放得下）：五个机场 200 架
  航班的全部 3,526 个航向词逐个试飞，3,230 段飞成、296 段机型没有动力学，这个拒绝 0 次。会截短的两种（距离词钟一步越过
  下一个航向词、更长的提前量）由测试用 `fly_until` 真实飞出。
- **只在提示里的说明也写成页面文字**：读数窗口的色样、先验窗口的读法、三维图例、面板的 Draw 开关与叠加层各有一个 ⓘ
  （`training/NotesToggle.tsx`），键盘能到、读屏能读。
- **页面编号不用 `crypto.randomUUID`**：它只在 https 或 localhost 下存在，从别的机器用 http 打开时整个应用在加载时报错白屏
  （浏览器核对时发现，单元测试的环境里它存在所以没测出来）；改用 `getRandomValues` 生成 128 位随机数，并加了一条去掉
  `randomUUID` 再载入模块的测试。
- **后端日志**：被取代的请求，页面早已断开连接，写回 409 时原来留下两段 BrokenPipe 报错（兜底的 500 又写一次）；现在
  `_send_json` 把"客户端已断开"记成一行（`client gone status=…`），`do_POST` 之后不再补记一行错误。
- **叠加层下载**：只保留最新一次下载的答复（每次下载一个令牌）；原来同一个文件先后问两次、前一次失败时，会把后一次的
  "下载中"盖成失败。
- **结果卡**：没飞成时有 **Fly again**，重新请求卡片上的那个词（句子条此时选中的也许已是别的词）。
- **叠加层**：下拉里选中的那一个属于它的机场与集合（换了回到最后列出的，换回来恢复）；下载失败的，开关关掉再打开就重新下载。
- **镜像**：先验读数表的三条规则名 `TRAINING_PRIOR_RULES` 成为 `prior.readout.RULES` 的镜像，读取时按名字核对；它和
  `TRAINING_STRATA` 由后端 `MirrorTest` 逐字比对（`docs/code-health-followups.md` 的第 11 条就此解决，已删）。
- **补上的测试**：回放里"没说到"与"被取代"的词、速度段在到达速度带前被截断、句子在越过入口处截断、"Replay in 3D" 不重复
  添加实体、第一个周期就动力学失败的答复（读取、结果卡、读数窗口、三维）、Fly again、叠加层的选择与重下。
- 测试：前端 95 个文件 774 条，`tsc` 与 `typecheck:scripts` 无错；后端 149 条；ts 的 `test_instruction_training_export.py`、
  `test_training_overlays.py`、`test_frontend_mirrors.py` 通过。浏览器（5174 / 8766）：连点三个航向色块，前两个以 409 结束（一个在
  排队时、一个在飞第 0 个周期前），只飞了最后一个。

### 2026-09-25 — Training：整模块审查（前后端），修错、理结构、界面精简

四个审查（数据层、hooks 与三维、组件与界面、Python 后端）逐项核实后，模块内的改了，模块外的记进
`docs/code-health-followups.md`（"Training module review"一节，11 条）。分支 `dev-autopilot-live`，用户合并。

- **会造成错误显示或错误请求的**：换航班 / 换集合 / 离开 Training 再回来，上一次选中的词会被**重新飞**（选择现在属于屏幕上
  这架航班，`trainingSelectionKey`）；高度与速度只出界一行时红线**画不出来**（一个点的折线）；实时执行器的航向带曾按句子的段尾
  设上界——执行器晚听到下一个航向词时会被错拒（`executor_v5_20260924` 回放里 3,153 条有后续航向词的带中 279 条如此），改为按判决读到的飞出航迹；
  执行器离开航向词自己去切入航道时，若这个词没有可判的行，后端判成"不判"而回放判"出界"，现在一致判出界，且只算这个词
  生效期间的周期；动力学失败时的航向带现在也画（判决读的行航迹里都有）。
- **读取更严**：一个读取器（`trainingReader.ts`）读三种文件，原来两份各有各的 `Refusal`；新增核对：回放的 `wordsInside`、
  样本航向带的结束行（原来只查不超过）、门拒掉时不能有判定、先验第一个预测步说词概率为 1 且 truthP = changeP × wordsP、读数
  表的比例在 [0, 1]、没有换词步时换词指标为空、列顺序逐名比较、叠加层与清单条目和机场绑定、门表只有本机场与全部。真实数据：
  五个机场的现行集合与五个执行器叠加层都通过。
- **后端**：拆成 `aeroviz_backend/autopilot_segment/` 包；状态码 400 / 404 / 422（机型没有动力学）/ 500 分清（原来任何缺文件都是 404、内部拒绝是 400）；
  机场必须是机场代码；集合必须是这个词表的格式与读法、从 val 抽的；规格目录变了就重新找规格；`flyS` 只算执行器的周期；
  新测试钉住与 `replay.fly_sentences` 的装配一致、前端四个镜像名一致。
- **结构**：读数窗口拆成纯模型 + 四张图 + 画图小件 + 共用窗口外壳；三维拆成 `scene/trainingEntities.ts` 与两个 hook，一架航班
  的实体只建一次，Draw 开关只改显示；飞完的实时航线换成静态属性（不再每帧重建，虚线部分画得出来）；措辞统一在 `trainingText.ts`。
- **界面**：句子条头部只剩呼号、跑道、一个判定小块、回放小块、实时执行器一行、游标和 ▶ Fly / Read-back / Prior / ⓘ，完整读法在
  提示和 ⓘ 里；两个窗口同时只开一个，Esc 一打开就能关；面板开关名字变短、航班列表上移、"none published" 的命令可折叠；读数
  窗口只在有东西时写一行说明、底部一排色样；三维图例默认收起、每行几个字。
- 测试：前端 95 个文件 760 条，后端 142 条，ts 的两个镜像测试文件通过；浏览器（5174 / 8766）核对过。

### 2026-09-25 — Training：实时执行器，选中一个词就在后端现飞它的一段

分支 `dev-autopilot-live`（未合并）。用户："当前端在词表中选定某一段的时候，可以调用 autopilot 实时计算并飞出轨迹……必须实时
计算航迹，不要用已经计算好的航迹"；飞哪一段由用户定为"只飞这一段"；只改前端和它的后端，不改执行器。

- 后端 `POST /autopilot/segment`（`aeroviz_backend/autopilot_segment.py`）：观测飞机说这个词时的状态为初态，第 0 步是那一步
  生效的六列词，之后是段内的词（词钟 `track`），飞到这个词的包络结束之处（同列下一个词说出之处；航向词再加一个提前量，否则
  它少判几行、只持续一步的航向词——四成——根本判不了；到句子末尾就飞到落地）；执行器原样使用，用它的单步接口 `Executor`
  一个周期一个周期地飞、到段尾就停（用户："接入并优化流程，避免全程计算并截取"；与先飞满时限再截断逐位相同，216 段核对）；
  只判选中的词（执行器的判决）。规格在 `outputs/POOLED/executor/` 里找恰好一份现在的代码接受的。只有点击
  句子条上的色块才请求（代码审查：跟着游标的话，图表上一悬停就连发请求）。
- 前端：面板开关与结果卡、句子条一行摘要、三维里飞机把这一段飞出来、读数窗口四张图的蓝线；答复绑到句子条这一段（告诉执行器
  的词必须是屏幕上这一段的词），格式 `aeroviz-autopilot-segment-v2`；结果卡顶部并排给出模拟飞行时间（对照观测、周期数）与计算
  用时（后端分项：执行器、判定、重建航班或沿用、排队；浏览器往返），句子条与三维标签也写出（用户要求）。
- 界面（用户："我怎么没看到启动段内飞行的ui"、"注意设计的简洁和和谐……飞出的结果是否在词段的包络内要写出来……飞出包络的可以是
  更醒目的红色"）：句子条头部加 **▶ Fly this segment / ↻ Fly again** 按钮；面板最上面单独一栏 "Autopilot (live)"（开关：点色块
  就飞）；句子条状态只剩一行——词、在不在包络内、"N s flown in M ms"；结果卡第一行是判定，其余收进 Details；飞出包络的整条线
  （三维、读数图、标签）换成醒目的红 `#ff2d2d`。
- 读取器的规格换成 `145d6911e75b`（`v4_20260924`，按运行日划分），因为合并 `dev-prior-v3` 之后的执行器代码只飞这份产物（现在的
  执行器规格 `v7_20260925`）；用户同意后导出新集合 `instruction_v3_day_split`（五个机场各 40 架新验证集航班，`public/data`，导出时代码
  `9fdc0e85`）。旧的 `instruction_v3` 集合与 `executor_v5_20260924` 叠加层从此按规格 sha 拒读（盘上没动）。
- 文档：`aeroviz-4d/docs/36-2026-09-20-training-module.zh.md` §4.7、`35-viewer-reference.md` AV26。

### 2026-09-25 — 先验的后训练：闭环监督微调（CAT-K）失败、不采用；改为按落地强化；闭环提速（逐位不变）

分支 `dev-prior-v3` `e760ab1d`、`dev-prior-rl` `e8ee1373`、`dev-prior-fast` `002998fb`（都未合并，按这个顺序合）。

- **单机自由生成**的门先只记录（用户）；验证集读一次：先验说的词落地 90.2 %，标注的词 99.9 %（读数文档 §5）。
- **闭环监督微调（CAT-K）失败**（读数文档 §6）：每架飞 4 个分支、每 20 s 留下离观测最近的一支，把标注按"标注说词的那些行"对齐到链上作
  目标，只用链上的数据训练 8 轮。验证集总落地 90.2 → 93.1 %，但全部来自直线进近（94.0 → 99.7 %），被引导的 83.3 → 81.0 %，航向词只有
  标注的一半，teacher forcing 负对数似然 0.284 → 0.452（8 轮后 1.114）。原因：链离观测越飞越远（220 s 偏 1 km），按时间对齐的标注在链上
  不成立，模型学成"早许可、少说话"。目标规则在开跑前改过一次：按"链上生效的词与标注相同"逐行要求，试跑时要求说词的步占到 26–57 %
  （标注本身 0.4–8.5 %），改成按标注的说词事件对齐。设计 §9.2 改为"不采用"，代码暂留，是否归档等用户定。
- **按落地强化**（用户的想法，设计 §9.3）：先验自由说 8 句、执行器飞到结局，落在机场此刻落地方向的跑道上得 1，同一架航班的几句互相比
  （GRPO），加拉回起点模型的项（0.04）和 teacher forcing 数据项（1），关掉 dropout；从第 1 步的模型起跑。选择集第 1 轮落地 90.0 → 96.5 %
  （被引导的 84.3 → 93.5 %），负对数似然不变（0.284 → 0.285），护栏都守住（读数文档 §7，跑完再补）。
- **闭环提速，逐位不变**（用户："优化的前提是不改变运行结果"）：说话器每步按机场成批算输入（`data.rows_inputs`）、一次拷到 GPU；新旧
  代码同一种子的输出 29/29 逐位相同，一块 16 架 × 8 句 26.6 → 10.0 s。多进程并行会改变每块的随机数，没做。
- **CAT-K 的代码归档**（用户）：`archive/closed_loop_sft_2026_09/`（`dev-prior-fast` `e5f18692`）——runner、`prior/relabel.py` 与其测试原样搬过去，
  `take`、`FineTuner` 等段落逐字剪过去；执行器代码回到 `4205e186`，规格用 v7；在用代码的输出逐位不变。


### 2026-09-24 — 先验设计第三版：输入只给那一刻看得到的东西、按场景做；设计文档只写最终设计

分支 `dev-prior-v3`（未合并）。用户问了泛化、多机、"离跑道 24 km 怎么会已许可"之后定下：

- 原则：输入只给说这一步之前管制员能知道的东西，标注器读出的词只当目标，第一个预测步六列都预测（上一版把交接时的五列词当
  条件，其中"下降到哪个高度""正在转向航道"是事后读出的；已建立在五边上的航班给了"许可加入"再问跑道，位置就说出了答案）。
- 按运行日划分，**两层模型这条线另封一批 test 运行日**（用户选 c；原来按航班的 test 留给 ts 的其他模型）；句子产物要按新划分
  重建。
- 多机按场景做：时间 × 飞机的分解注意力，飞机之间的注意力带相对几何的边特征（即全连接图上的图注意力，不另接图网络框架）；
  预训练用场景上的联合教师强制，交互在闭环后训练里学。量到 25 km 圈里同时只有 1–3 架其他到达（最多 8 架），排序大多在圈外完成。
- 分步与门：单机 → 窗口里看不看得到交互（M1）→ 前机一条边 → 完整场景 → 多机闭环与后训练。
- `docs/2026-09-24_prior_design.zh.md` 重写为只讲最终设计（用户："设计文档应该只给出最终设计；版本管理是 git 的事"）；第一版的
  读数、第二版的配置与跑之前写定的比较规则移到新的 `docs/2026-09-24_prior_readouts.zh.md`。

### 2026-09-24 — 执行器只用词表（第二阶段）：速度快慢取词表的、落地越过公布的 TCH、落地时留在词的管子里

分支 `dev-vocab-v3`（`51c3ebcb`、审查后 `1b0cd4f4`，未合并）。执行器除词表外只读被指跑道公布的入口跨越高度（TCH）。

- 速度变化的快慢：词表的一个速度级差 ÷ 速度词最短保持时间 = 0.25 m/s²，减速、加速、"未指定"同一个（用户定），取代数据
  量出的 0.28 / 0.19 / 0.30 m/s²。
- 落地越过被指跑道公布的 TCH（FAA CIFP，harvest 的跑道数据，读取在 `autopilot/runway_data.py`），取代数据量出的瞄准高度
  20.8 m 与窗口 8.4–36.5 m；`instruction_signals` 在写任何东西之前拒绝没有 TCH 的候选跑道；`evaluation.cli` 只提供两个数据
  路径，移出执行器源码指纹，回放的抽样记录下 TCH。量这些值的代码已归档（`archive/executor_vocabulary_only_2026_09/`）。
- 落地飞法：在生效那条词的管子里下降，直到从管子出发、在平飞与最陡一档的下沿（3.73°）之间飞也够不到入口允许的高度
  （TCH ± 12.5 m）才离开；截获后下降不陡于"朝越过高度的高度保持律"与"到越过点的直线"中较陡的一个（代码审查指出上一版离开
  管子太早，坡度中途变化的进近落地词出包络）。
- 格式：`ts-executor-spec-v5`、`ts-executor-replay-v4`。
- 正式产物 `4dTrajectory/outputs/POOLED/executor/v5_20260924/`（规格 `0d6a68a92c6f`）：train 每机场 400 架，自己的动力学 1,914 架，
  落地 99.9 %、evaluation 配对 98.7 %、词 98.0 %（高度词 88.6 → 95.7 %）、按原话飞出 72.2 → 82.5 %，**每格都过门**（KSJC 直线
  进近 93.2 → 97.8 %）。航向词 98.4 → 98.0 %：减速变慢，飞机偏快，落地比观测早（中位数 −9 s），说词更密。敏感度：τ_ψ、γ̇ 系数、
  p 6–10°/s 都不敏感。val 回放等用户同意。

### 2026-09-24 — 执行器只用词表（第一阶段）：去掉 r_turn、φ_cap、延迟与办法 B

用户的原则：执行器的设计不用词表以外的信息，要靠大量从数据挖的参数才能做成，就说明它不能泛化。分支 `dev-vocab-v3`
（`485d1e60`、`8ca8d98f`，未合并）。

- 所有转弯（跟航向词、自行切入、复飞、截获、沿线）都在词表的转弯率 0.5–4.7°/s 和坡度上限 32° 内；截获按两者的几何平均
  1.53°/s 规划起点（按下限规划时转弯卡在包络边界上）。
- 词说出即执行，每 2 s 一行只在行首听一次；办法 B（量延迟）归档。
- p = 词表坡度上限 ÷ 提前量 = 32° ÷ 4 s = 8°/s，τ_ψ = 提前量 4 s；按合成转弯检查定 p 的旧办法（结果不稳）归档。
- 归档：`4dTrajectory/ts_transformer/archive/executor_vocabulary_only_2026_09/`（模块与测试原样）。
- 格式：`ts-executor-spec-v4`、`ts-executor-replay-v3`（发布脚本的镜像同步）。
- 正式产物 `4dTrajectory/outputs/POOLED/executor/v4_20260924/`（规格 `4c4038511ca2`）：train 每机场 400 架，落地 99.9 %、
  evaluation 配对 98.8 %、词 97.7 %，只有 KSJC 直线进近的词没过门（"下降落地"那条高度词，词表 §10.2）。比用数据参数的
  `v3_20260924`（p 5°/s）不差。还从数据量的：三个加减速度、落地瞄准高度与窗口（第二阶段）。

### 2026-09-24 — 词表第三版（instruction-v3）的航向词与执行器：逐行标注，执行器"听到后正好 4 s 到达"

分支 `dev-vocab-v3`（未合入 `dev-two-tier`）。设计：词表设计 §10.1，执行器设计 §4.1、§9、§10；阶段记录 §2.6。

- **航向词的读法改为 H3**（用户选定）：每个 2 s 行标上"4 s 后的平滑航迹"最近的 5° 格，相邻同格合并成一条词；
  不再有保持段、拆分转弯、补切入航向。许可在截获转弯开始那一行说，且不早于在用航向词收敛。train 上用真执行器比较
  （`outputs/POOLED/analyses/heading_reading_20260924/`，1,992 架）：落地 99.0 → 99.8 %、evaluation 97.1 → 98.3 %，
  "转上五边"组 87.9 / 73.6 → 99.2 / 98.4 %。旧读法的代码与比较 runner 移进 `archive/heading_reading_2026_09/`。
- **执行器**：跟随航向词时转弯率只受词表最大转弯率（4.7°/s）与坡度上限限制；航向词没有延迟（办法 B 只量高度 /
  角度 / 速度）。**跟随航向词的律**原提议是一阶律 τ_ψ = 4 s，代码审查时的合成检查发现它在每次转弯的最后一两条词上
  必然出包络（90° 按词转弯，19 条词里 3–7 条，p 取多大都一样），改为转弯率 = 误差 / 剩余时间（听到词的时刻 + 4 s −
  现在，至少 2Δt），再限在坡度收得回的转弯率 `sqrt(2 g p |e| / V)` 以内（第二轮审查：只按剩余时间会冲过转弯的最后
  一条词，3°/s 的转弯冲过 5.5°）。航向词、许可、跑道指针每 2 s 一行只在行首听一次（第二轮审查：按航迹 / 按路程
  说词时，词比判定读它的行早一个周期被听到，合成盘旋 73 条里 12 条出包络）。τ_ψ 只管执行器自己的转弯，取 4 s；p 由
  两条定（自己最大的 124.5° 转弯越过目标 ≤ 4.5°；r_turn 和词表最大转弯率的按词转弯每条词都在包络内，速度每 5 m/s），
  测试参数上 4.5°/s。前端导出改用判定自己的记词函数（原来的副本在按航迹说词时对不上）；航迹时钟一步跨两行时同时到
  的航向词在回放里单独计数。train 预跑（每机场 400 架）后又改：跟随航向词时坡度可到词表的 32°（按 φ_cap 25° 时航向词
  只有 90.9 % 在包络内，几乎全是转弯中段落后；32° 时 98.2 %），执行器自己的转弯仍按 φ_cap；p 因此为 5.0°/s。
- **判定**：航向词从执行器听到它的那一行加 4 s 起判，到下一条词同样加 4 s，截止在许可或截获；动力学失败的航班只在
  失败之前的状态上读词（原来 NaN 行被算成出包络）。
- **格式**：读法 `instruction-v3`、词表规格 `ts-instruction-spec-v4`、执行器规格 `ts-executor-spec-v3`、回放
  `ts-executor-replay-v2`；执行器源码指纹按模块名计入包外模块，与检出位置无关；回放自己评判抽到航班的观测记录
  （harvest 的观测报告早于速度门现在的做法）。
- **还没做**：正式的词表规格、句子、执行器规格、敏感度和 train 回放都还没跑；新律在真实航班上的词合格率要看 train 回放。
...
橙色是两条曲线之间的面积（转弯区），深橙是转弯可能结束的位置（两条曲线终点之间），蓝色是保持漏斗（从深橙那条带
出发、沿新航向 ±4.5°）；大转弯后深橙带有十几公里长，漏斗起始就 12–18 km 宽（KRDU 的 N248ML：16.9 km 宽、
7.8 km 长，"29/29 在里面"），几乎约束不了什么。用户选了只改前端的三项：

- 原来的 `lateral` 开关拆成 `turnRegions`（默认关）、`holdFunnels`（默认关）、`corridor`（默认开）；`turnPaths`
  （默认开）画两条曲线和转弯可能结束的平行四边形（新增截获转弯的）。航向图的保持带是航向容差，总是画。
- 三维右下角的图例（`TrainingLegend`），只列打开的开关。
- 判了但漏斗起始宽度超过 2 km（`TRAINING_WEAK_HOLD_WIDTH_M`，读数阈值）的保持段标为"弱判定"：边线点线、
  航向图标题和句子条头部写明；标注器的判定不变。改漏斗的起点（从实际转弯终点出发）是词表语义，没有做，留给用户。
- 三维和平面图里路径名字与实际转弯起止的标签错开放置。

### 2026-09-24 — Training：大转弯看得懂——航向随时间的楔形（样本 v5）、路径命名、实际转弯、其余淡化

用户："heading 遇到大弯的表现形式我怎么看不懂"。原因：转弯区由最慢的边界主导（0.5°/s、晚 10.5 s；118 m/s 时
半径约 13.5 km，最快的边界受 32° 坡度限制约 3.0°/s、半径约 2.3 km），150° 的转弯区有十几到二十公里，"转弯可能
结束的位置"是十几公里长的带子，平面图又只按航迹裁剪；航向图只有一个方框。用户同意三项一起做并重新导出。

- **航向图的楔形**（`display.turn_heading`）：同样的最快、最慢两个转弯画成航向随时间，从 `envelope.turn_path`
  返回的路径逐点读出（每一步按前后两行航向的平均值飞：下一行航向 = 2 × 这一步的方向 − 上一行；这一步用时 =
  路程 / 两行地速的平均），`envelope.py`（在标注器 sha 里）没动，不重新标注。导出的最慢路径改成晚开始的那条
  （转弯区的外边）；去掉 `turnBandDeg` 和截获转弯的 `bandDeg`；样本格式 v4 → v5（两边同改，契约测试钉住）。
- **选中一个词时**：其余的词淡化（读数窗口透明度 0.3，三维颜色透明度 × 0.3）；平面图按它的整个包络取景；
  它的两条转弯路径标名字；航向词在航迹上标出实际转弯的起止（`check.departureRow` / `arrivalRow`）。三维选航班
  时的取景也框住所有转弯区。
- 读取器逐点核对两条曲线与两条平面路径、起点等于发令时刻；转弯的 `arrivalRow` 必须是句子里的一行。
- 提交 `a66c3276`（v5）、`37e7e916`（取景、标签位置）；在 `a66c3276`（干净）从 `v2_20260924` 重新导出五个机场
...
- **高亮按词，不按步**：新的共享状态 `AppContext.trainingColumn`（选中的词类）加游标，经 `trainingWordAt` 得到
  这一列在游标处生效的词；句子条、读数窗口、三维只亮它：它自己的包络（本色加深、黄边）和它生效的那几行（黄色，
  画在航迹上和画它信号的那张图上）。走廊从"截获后的航向包络"改归进近的"许可"词。点带选列、再点取消，步号只移
  游标，读数图上点一下选这张图的列。前端文档 36 §4.5、`35-viewer-reference.md` AV21。
- **三维审查后的改动**（前端文档 36 §4.4、AV22）：选航班时相机不取景，航迹常在画面外 → 选航班时取景一次；贴地
  包络和空中航迹斜看有视差 → 加航迹的地面投影；贴地多边形没有轮廓、叠在一起糊成一片，且三维不显示判定 → 每个
  贴地包络加贴地边线，按标注器判定着色（红 / 虚线，同平面图）；±25 m 的管子墙隔远了看不见 → 加上下沿两条线；
  选中时原来把填充整块换成黄色，转弯区和漏斗分不出 → 保留本色、加深、黄边，另加发令点标签。
- 前端 Vitest 88 个文件 655 条、`npm run build`、`typecheck:scripts` 通过；本机 Chrome 里 KRDU N994FG 逐类核对过。
- 同日追加（用户）：**速度列换成紫色 `#be76ff`**——原来的琥珀色和选中的黄色 OKLab ΔE 只有 3.4；用 dataviz 的配色
  校验脚本在句子条底色上比过，离黄色 39、判定红 23、原始灰 18，色盲模拟下离其余每个颜色都 ≥ 10（青绿在绿色盲下
  离许可粉只有 4.6，没选）。高度与下降角两列本来就只差 4.3，靠行名区分，没动。**最快 / 最慢转弯的路径**在三维里
  也画出来（贴地，颜色跟转弯区的判定，转不完画虚线，含截获转弯），和平面图一起由新开关 `turnPaths` 控制。

### 2026-09-24 — 执行器能把词表飞出来；先验（阶段 5）第一次训练跑通

分支 `dev-executor`。执行器设计 `ts_transformer/docs/2026-09-23_executor_design.zh.md`，先验设计 `…/2026-09-24_prior_design.zh.md`。

- **词在观测飞机听到它的位置说**（`autopilot/sentence.py` 的三种时钟，回放用 `track`）：一条词只给目标、不给快慢，
  按观测的时刻说，执行器按自己的节奏飞时后面的词就落错地方（train 正式敏感度 82.5 % 落地的主要原因）。另外：执行器
  碰不到航线时按 30° 自己切入（出了这条航向词的包络，照实判）；"下降至落地"截获后瞄准词的管子与观测越过高度窗口
  （train p5–p95，8.4–36.5 m）的交集；"未指定"在入口前减到进近速度。两轮代码审查的发现都已改。
- **train 2,000 架**（每机场 400，自己的动力学 1,581 架）：落地 98.2 %，与观测配对的 evaluation 96.5 %，词在包络里 94.4 %
  （高度词 85–89 %，其余 ≥ 94 %）。直线进近全部落地；雷达引导 KSJC、KSMF 两格落地 91 % / 89 %：词不说转多急，别人的大
  转弯飞出的半径不同，五边前就差出几公里——闭环里先验看执行器自己的状态说词，会改航向。
- **先验第一次训练**（`ts_transformer/prior/`，runner `prior_train`，`11888357`，干净的工作树）：因果 Transformer，六个词头，
  230 万参数，27 分钟；产物 `4dTrajectory/outputs/POOLED/prior/v1_20260924/`。val 每步负对数似然 0.1778，"重复上一条" 0.3444，
  "只看上一个词" 0.3260，每一列都低于两个基线；取值前 5 覆盖 83–100 %，"什么时候换词"最难。没有做交叉验证。
- **val 回放门**（用户同意后跑，规格 `2674ab8c71a9`，`outputs/POOLED/executor/v2_20260924/`）：自己的动力学 7,773 架，落地
  98.6 %、evaluation 配对 96.7 %、词在包络里 94.2 %。15 格里落地 14 格过（KSMF 雷达引导 90.3 %），evaluation 12 格过（三个
  雷达引导格没过），词多数格在 93–95 %——高度词在落地段离开管子（直线 83.7 %、雷达引导 88.9 %）。train 敏感度：没有一项
  可取的变动比规格的值好。旧的 `executor/v1_20260924` 按用户同意删了。分支 `dev-executor` 已合入 `dev-two-tier`
  （`dc717045`），工作树与分支已删除。

### 2026-09-24 — 执行器（阶段 3）：按词飞的自动驾驶、参数的量法、回放 runner

分支 `dev-executor`，包 `4dTrajectory/ts_transformer/autopilot/`，设计 `ts_transformer/docs/2026-09-23_executor_design.zh.md`。

- **执行器**：每 1 s 读一次飞机状态和生效的词，横向、纵向、速度三条律给出想要的变化率，按点质量方程精确反解出
  坡度、载荷因子、推力（限制按设计的顺序，每条都记录），再用控制路径的同一套动力学积分一个周期。那个后端不跑
  hook，所以执行器自己一周期一周期地走（设计 §2.3）。
- **判定三层**：每周期的限制；每个词用标注器自己的检查判飞出来的航迹；每架的结局用 harvest 的落地条件。回放的
  记录写成控制路径的预测记录，交给 evaluation 判三道门，按 `flight_key` 与观测航迹自己的结果配对。
- **参数**：数据（train 全部已标注航班，每架重读并与存下的句子逐格核对）、办法 A（执行器自己的 90° 转弯）、
  办法 B（执行器按真值句子、延迟为 0 飞，飞出的航迹经数据平面同一串拟合函数再让标注器重读）。执行器规格
  `ts-executor-spec-v1` 写一次，带参数 sha 与执行器源码 sha，回放时核对。runner：`executor_spec`、
  `executor_sensitivity`、`executor_replay`（ts runners 参考 R12）。
- **E3–E6 审查后改的**（12 条，每条有测试）：截获越过中线后交给沿线跟踪（原来会以固定角度飞离航线）；截获与
  沿线跟踪的转弯率受"坡度来得及收回"限制，走廊内只修正走廊航向容差的一半；航向误差从生效的词量起，拆分的
  大转弯在执行器落后时不再反向；判定里被截获接过去的转弯、360° 盘旋、截获转弯本身、失速都按设计判。train
  100 架（种子 3，每机场 20）：落地 83 → 86，按原话飞出 63 → 84。
- **两个发现，属于词表的读法**：办法 B 量出标注器把高度/角度词的发令行读得比动作开始晚约 4–6 s、速度词晚约
  8–9 s（航向词早 2 s）；延迟不能取负（执行器不能在词说出之前动作），所以按 0 飞。E8 试跑（train 15 架，审查修正之前）里，雷达
  引导的 9 架有 5 架越过入口时高 64–289 m，方向与此一致；敏感度 runner 里"提前 4 s"一项专门量这个代价。
- 待办：E7–E8 代码审查，之后在干净的工作区正式量执行器规格、跑 train 敏感度；val 回放（阶段 4）等用户确认。
...
一次复核，所有发现已改。新产物 `4dTrajectory/outputs/POOLED/instruction_language/v2_20260923/`，规格 `103a6eae6b90`，
量于 `773c49ab`（工作区干净）；v1 产物 `v1_20260923` 与前端 `training/instruction_v1/` 集合未删，删不删由用户决定。

- **转弯按转弯率**：各行转弯率 ≤ 4.7°/s、坡度 ≤ 32°（坡度由转弯率和当时的地速算），转角 ≥ 10° 的转弯平均转弯率
  ≥ 0.5°/s。依据：稳定转弯的转弯率不随速度变，坡度随速度增大（执行器设计 §4.1）。转弯可以比发令晚开始，最多
  10.5 s——速度拟合 15 s 与航迹平滑 6 s 两个居中窗口之和的一半，是读法的性质，不拟合。
- **保持段的漏斗**：从"转弯可能结束的位置"出发——最快、最慢两个转弯按实际地速飞到航迹进入 θ ± 4.5°（在步内
  插值），两终点连线再沿发令航迹平移最多 10.5 s，围成平行四边形；从它沿 θ 张开 ± 4.5°，长度是沿位置点累计的
  已飞路程，从转弯段最后一行之前 10.5 s 起算（同一对窗口让标注器最多晚 10.5 s 看到转弯结束）。包含判定是精确
  的凸多边形判定；标注器和前端显示共用 `heading_spans` / `turn_ends_at` / `span_funnel`，画的就是判的。
- **落地**：越过入口平面（按 harvest 的方式取相邻两行，用 `final_approach.crossing.bracket_fraction` 插值），越过点
  离中线 ≤ 1000 m 且 ≤ 到平行跑道横向间距的一半、离入口高度的绝对值 ≤ 100 m（`LandingScreen`）。平行跑道从 harvest
  建出的全部跑道端取（`AirportGeometry.runway_ends`，由 `instruction_signals` 用 `load_airport` 读入）；
  `airport.landing_cross_limit_m` 是 harvest `_runway_bracket_cross_limit` 的镜像，测试逐跑道核对。各跑道的界：
  KSJC 106.7 m、KSTL 198 / 427 m、KRDU 534 m、KSMF 913 m，其余 1000 m。
- **读数**（train）：成功 50,214 / 拒绝 435（val 10,540 / 85）；"落地之后又回到入口前"从 v1 的 70 架降到 16 架。
  转弯单调 99.7 %、转弯率与坡度在范围内 97.3 %；判定的保持段 84,421 / 99,417（其余 13,580 个跟在 < 10° 的小转弯后、
  1,416 个最慢转弯飞不完），逐行 99.7 %、整段 96.2 % 在漏斗里，漏斗末半宽 p50 1,430 m、p95 6,059 m。
- **过程中量出的两处长度偏差**：第一版漏斗长度用平滑地速积分的路程，比位置点的路程短约 0.2 %，长保持段最后几行
  落在漏斗远端外 0.1–30 m；又从读出的保持段起点起算，没有算上转弯结束最多晚看到 10.5 s。两处改完，
  val 1,500 架抽样上转弯后的保持段整段包含 89.4 % → 96.2 %，进入切片时已在飞的词 39.7 % → 96.1 %。
- 标注器源码 sha 加入 `final_approach.crossing`（标注器与 harvest、evaluation 共用的插值）；删去两个永远不起作用的
  下限（转弯率的 0.1°/s、坡度的 89°）。
...
参考面随盒子开关显示，选段时增强填充并显示黄色边框；左侧补充颜色说明。
联动测试增加目标面高度基准、轮廓范围及显隐／高亮检查。

### 2026-09-21 — Training 词列表选段高亮三维对应盒子

将底部句子条的航班相对游标提升到共享 Training 状态。选择事件或词带时，对应三维盒子的侧面、顶盖
和起点标记高亮为黄色，上一盒恢复原样；切换航班归零，图层重新打开后恢复当前选择。
样式更新与几何创建分开，不移动相机或 Cesium 时钟。新增联动测试覆盖点击、键盘、事件边界、
航迹终点、航班切换及图层生命周期。

### 2026-09-21（晚，续）— 一个词的可飞空间是锥台,不是等高的扇柱

用户第二次指出同一类错误:"为什么总是高度一致的扇形体?我们设计里不是垂直面的梯形和水平扇形结合出的
三维锥形吗?而且垂直空间的阴影还有扭曲弧线。"两处都对。

**一、立体要跟着收。** 上一版把每个词画成"扇形底面 + 一个高度区间拉平"。错在:外轮廓上离顶点 `d` 米
的那个点,**到这一段高度的段末就少了 `d` 米航迹**(直线是过去最短的走法,绕的只会更少),所以那里的
楔形是 `wedge(r₀ − d)`,比顶点窄。正确的立体是**锥台**:平面上是扇形,任何一条径向剖面是**梯形**。
按一个高度拉平,在长保持上会把远端的天花板抬高到**盒子自身高度的 26 %**(实测一架引导进近的第一个
词:703 m 高的盒子,楔形在它自己的 4 141 m 进深上要收 181 m)。

实现:文件里改成**外轮廓每一点带一对高度**;三维里画成一圈闭回顶点的 `wall`(两个径向面就是那两个
梯形)加一张 `perPositionHeight` 的斜顶盖 —— `polygon` 的 `extrudedHeight` 只有一个数,表达不了斜盖子。

**二、"扭曲弧线"是楔形的墙被硬穿过了跳变。** 高度界在**每一个高度词**处跳一下:这一段收到它自己的
目标 ±5 %,下一段重新张开。一架引导进近上实测 **9 次跳、最大 59 m**。一面墙从头铺到尾就得穿过这些
跳,在转弯的地面航迹上渲染成扭曲的面。改成**一个高度段一面墙**,每一面在自己段末收到目标再交给下一
面,那一跳看上去就是一跳。

**顺带答"这个空间都是可飞的吗"**:不全是,而且我刻意没去收紧。锥台里没有任何东西限制航向在盒子里
**转多快**,因为**词不限制** —— 转弯速率是执行器的规矩,这份文件里没有执行器(回放门没做)。所以它是
"**词允许的位置**",比真正飞得到的那块地大。把转弯速率加回去,就等于把执行器的假设偷偷加回来。

五机场重导,包含率仍是 **39 670 / 39 670 行**。前端 **676** 条、Python **40** 条全过,tsc 干净,
浏览器里核过、控制台无错。
...
**标注器同一天晚些时候进了仓库**(`manoeuvre/box_vocabulary.py`,提交 `b38c5745`),于是导出器从"照 spec
重建"改成 **import**。当场露出三处差别,**没有一处会让当时发出去的那批数据变红**:

| 漏掉的 | 后果 |
|---|---|
| `altitude_half_width` 取的是 **|T|**,重建时写成了 `T` | 梯子上 61 个目标里有 13 个是负的(它要跨过入口高程)。`0.05·(T+50)` 在 T = −50 处归零、再往下变负——一个**上下界翻过来**的盒子 |
| 段末取的是**这一段自己的最后一行** | 我取的是下一个高度事件的时刻,差一行航迹,80 m/s 下 4 m 楔形 |
| 路径积分**不加地速下限** | 我加了。这支机队上永远不 binding(所以当时差 5e-6 m),但那不是保留差别的理由 |

另外加了一条更硬的缝检查:**每一架都用 `read_boxes` 重读一遍**,读出来的句子必须和产物里那一句逐词
逐时刻相同。句子是抄产物的、航迹是这边重建的,除此之外没有任何东西说它们是同一架飞机;这比它取代的
"时长对得上"强——一条长度对、但不是那一条的航迹会被它抓住。实测 200 架全部重读一致。

**发布**:五个机场各 40 架(val),`…/training/box/`,包含率 **39 670 / 39 670 行**。
前端 **675** 条、Python **72** 条全过,两个 tsc 干净,浏览器里核过。
...
前端跟着改,**三维几何航迹要突出包围盒/envelope 而不是简单的曲线**。

**换掉了什么。** 旧读法里一个词是一个格心加一条容差,所以"词说了什么"画出来是一条线加一条走廊;
新读法里**一个词就是一个区间**,一句话是一串包围盒,判据是"航迹被它装下"。于是:

- **没有"按规则飞出来的那条橙线"了**。飞一句包围盒需要**跟高度**的执行器——高度词说的是"待在这个
  高度区间里",不是"用这个角度下降"——那是词表设计里的回放门,还没做。取楔形中线硬飞一条出来,
  形状取决于我们自己的选择而不是取决于词。
- **三维画两样东西**:高度楔形沿飞机自己的地面航迹立成一面 Cesium `wall`(这是这份词表里唯一一个
  绝对的位置界),加上**一个词一块地**——那个词生效期间航向词和速度词允许的位置,在楔形的高度范围
  上拉起来。**那块地是扇形,不是长方体**:顶点在飞机、半径 `保持时长 × 速度盒上界`、张角等于航向盒。
  第一版画的是它的外接长方体,用户当场问"难道这个 box 都是可飞空间吗、我们设计里不是更接近锥体吗"
  ——他是对的:长方体在顶点旁边多出一块**航向盒里任何方向都到不了**的地。顺带一条,**速度盒的下界
  根本不约束这块地**,保持结束之前的任何一刻飞机都更近,所以扇形一直连回顶点,只有上界定半径。
  实测中位 441 m 深、远端 23 m 宽,这个窄本身就是结论。
- **判据进了界面**:句子条表头和读数窗口每张图的标题都读出包含率,出盒子的行画成红。

**产物的标注器不在本仓库**,这是这次最要紧的一条。盘上这份产物是在树外跑出来的,没有源码可 import,
所以导出器这一侧的盒子是**照产物自己的 spec 重建的**。顶替"和原件对一遍"的是两件事:

1. **重建是量出来的,不是猜的**。三路信号各试了两三种写法,取把产物自己接受的航迹 100 % 装进盒子的
   那一种:航向是 **wrapped 上平滑 6 s**(unwrapped 上平滑会有 67/13922 行落在盒外)、速度和高度是
   平滑 10 s、楔形的剩余航迹是**平滑后**地速的积分(用原始地速最大越界 0.3 mm)。
   最终五机场 200 架 **39 670 / 39 670 行全在盒子里**。
2. **包含率算两遍**:导出器算一遍写进文件,前端拿着它送来的列再算一遍,对不上**拒掉整份文件**。

顺带记下一条标注器的算术:**wrapped 上的滑动平均跨过 ±180 会把 +179° 和 −179° 平均成 0°**。
前端照抄不改——一个和产物不一致的视图画的是另一份读数。

**发布**:五个机场各 40 架(val,直线 20 / 引导 20),`aeroviz-4d/public/data/airports/<ICAO>/training/box_v3/`。
`check-publication` 对这五份 0 errors;盘上另外十一份旧集合(`segment-v13` 的十份先验 + 一份
`segment-v14`)被按 schema 和读法拒掉,选择器里标着 `superseded`,共 11 errors——**这是对的**,
删不删是用户的决定。前端 **672** 条全过,两个 tsc 干净,浏览器里核过。

**还没有在这份词表上训练的先验**:`prior-generated` 那一类的契约(逐事件的词 + 置信度 + 它自己那份
包围盒)已经定下来并有测试,导出器的 `--prior` 那条路**写了但从未跑过**。

设计:`aeroviz-4d/docs/36-2026-09-20-training-module.zh.md` §3–§5、V53–V58;
读者须知:`aeroviz-4d/docs/35-viewer-reference.md` AV19。


### 2026-09-21 — `segment-v14`:补上唯一一个位置词,分段改成"2 秒起合并",跑道离开模型输入

用户问了两个问题,把上午那版的两个洞问了出来。

**"航向 0 和保持中线,物理上不是没区别吗?"** 区别只有一个:前者约束速度的方向,后者约束位置。而这
份词表里**六个词全是速度量,没有一个约束位置**——所以横向误差没有任何机制能自我纠正,开环飞只有
36.7 % 到得了跑道。上午我把纠正塞进了**解码器**(把方向词 0 当成跟踪律),那是错的:读的时候从没说
"加入航道"(实测 v13 里带方向词 0 的行有 8.2 % 在 500 m 走廊之外,刚发出那一刻中位还差 535 m、
p90 差 5019 m),而且**读和飞对同一个词的理解不一致**。现在航向类多一个 `heading_established_word`,
按 `course_frame` 本来就有的 `established` 列读出来,方向词 0 恢复原意。**落地率从此是词表挣的**。

**"不如以 2 秒为单位、在阈值内不断合并,而不是别扭地用 20 秒算平台"**(用户的设计)。平台要求信号
先"保持"——对会稳下来的信号对,对持续变化的信号错,而进近速度持续变化:速度平台只覆盖一次进近的
51.3 %(航向 84.1 %),61 % 的时间在减速,漏掉的那一半没有任何词负责。`tile_segments` 从数据自己的
2 秒行合并、再吸收短于一条指令的碎片,**一条规则给出两种行为**:转弯 0→90°(30 秒)读成 2 段,减速
140→70 m/s 读成 8 段且完全铺满。

代价是句子变长(KRDU 每架事件 p50 7 → 11,速度指令 3 → 5),**收益是回放更贴近真实航迹**:同一个
KRDU 验证集上 gap p95 中位 1085 → **821 m**、平均 gap 中位 651 → **453 m**,而落地率只从 99.6 %
降到 99.1 %——并且这 99.1 % 不再有 58 个百分点是解码器给的。

**跑道离开模型输入**:标注器把跑道词写成整条航迹的常量,同一列又进输入又进目标,"预测"就是抄
(NLL 0.001、top-1 1.000,六个交叉熵里一个恒为零)。现在它只做预测头,必须从航迹推断。此前所有先验
读数里的跑道那一行都是在给抄写打分。

文档:词表部分新增英文规格 `4dTrajectory/ts_transformer/docs/instruction_vocabulary.en.md`;
参考文档新增 H11,H10 标注为 v13 的基线。**v13 的产物与两个先验一律保留,没有删除。**

### 2026-09-21 — 一句话必须能飞到跑道:`segment-v13`,降落率 36.7 % → 99.6 %

用户在前端看重建出来的几何航迹,发现它和原始航迹差很多,有些转弯方向完全相反。查清楚是三件事,
前两件是**词表的表达能力**,第三件是预览模型;词本身是准的(按平台到平台量,每次转弯还原到 p50 1°、
p90 3°、最大 4°)。

- **航向词说不出往哪边转。** 绝对目标方向不含左右,`wrap_deg(180)` 恒为 −180,半圈转弯永远往同一边
  转,中了整条航迹镜像出去(最多 18 km)。五机场 28,221 次航向变化里 13.4 % 超过 150°、9.9 % 正好
  半圈,17.1 % 的航班至少中一次。改法:超过 150° 的转弯拆成两句,中间那句在转弯开始时发出。
- **航向词说不出"保持中线"。** 这是雷达引导航班降落不了的原因:在 10 km 内首次对正航道时,真实航迹
  离中线 13 m,失败的重建 2,464 m——对正了航道却沿平行线飞下去。词 0 命名的是航道本身,飞它就是
  切上并保持中线。单这一条 36.7 % → 94.7 %。
- **预览的机动模型比真机快一半**:实测转弯率只有 20° 坡度的 0.67 倍(=14°),加速度 p90 0.54 m/s²
  (原来的 1.0 是 p99)。

另外评审查出两个阻碍缺陷:句子从不说自己什么时候落地(终止词写在最后一次变化上,中位数 136 s、约
11 km 之前,时长词又只表示"距上一个事件多久",所以最后一段有多长在任何词里都没有);三个解码容差被
算进了 sha。

**误差的大头不是分档粗**:速度词换成未量化的平台值只降 18 %,换成逐秒实测地速降 55 %——剩下的是
"一段一个常速"这个模型和没有风。

新读法 `segment-v13`、sha `fffa8bdf24a0`,v12 的两套产物作废。验收做成了 runner
`run_ts.py instruction_replay`,飞的是产物文件里写的句子(新增 `Reading.from_dict`),不是重读航迹。
KRDU 验证集 1,404 条:**99.6 % 到达跑道入口**(直入 100 %,雷达引导 99.2 %),gap p95 中位 1,085 m
(原 2,035–2,586)。ts 1,338 条测试、前端 154 条测试通过。

### 2026-09-21 — 跑道词必须带机场前缀,五机场词序列重建完成

新词表 `feee49bb2f7b` / `segment-v12` 的词序列已在两套产物上读完:KRDU 单机场
(`two_tier_v3_bprime_20260920/vocabulary_segment_v12`,train 6,853 / val 1,404)与五机场
(`two_tier_v3_bprime_20260921/vocabulary_five_airports`,train 21,890 flights / 143,434 events,
val 4,492 / 29,412)。

**读出来发现的问题:五机场只有 16 个跑道类。** `flight_runway` 返回的是光跑道号,而跑道号跨机场重
名——KSJC 与 KSTL 都有 12L/12R/30L/30R,KSTL 与 KMSY 都有 11/29:六个号码盖住十二条进近航道朝向完
全不同的跑道,同一个嵌入要同时代表两条,这个词就说不清自己指的是哪一条。类名改成 `机场:跑道号`
(`KSJC:12L`)。**类别集合是队列的,不是机场的**:五机场的到达清单里有 23 条跑道
(KRDU 4 / KSJC 4 / KSTL 8 / KSMF 3 / KMSY 4),而这个 pooled 队列里没有一架 KSTL 06,所以读出来是
**22 类**。单机场时前缀只是冗余,不改变读数。类频差别很大——KSJC:30L 有 5,322 架,KMSY:20 只有 1
架、KSJC:12L 只有 2 架,先验训练时要记住这条尾巴。

**没有重跑词序列,而是就地重映射。** 跑道词是每条航迹的一个常量,别的列跟它无关,所以改的是
`runway_idents`、每条航迹的 `runway` 标签、词矩阵的跑道列、那条 `runway` 指令的 `word`,以及按新索
引重算的跑道词频与 `words_used`;`summary.txt` 重渲。**验证**:两个产物各抽 40 条航迹,用当前代码重
读一遍,整条 `Reading.to_dict()`(不只是跑道列)与文件逐字节一致。

顺带订正:`instructions.__all__` 里还留着已删除的 `ALTITUDE_BIN_M` / `SPEED_BIN_MPS` 等五个名字;
`Reading.words` 的列注释仍写着旧的六类顺序。设计文档里 KSTL 的跑道数写成 8,实测是 7。

### 2026-09-21 — 词表改版之后的 Training：一句话画出来的是一条走廊

词表从 `plateau-v11` 换成 `segment-v12`：航向 10° → 5°（72 类），**高度词换成垂直词**（航迹倾角，
六档，**下降为正**），速度换成 16 个拟合档心（m/s），并给**垂直和速度各加了一个冗余**
（平飞 ±0.1°、其余 ±7 %、速度 ±3 %）。前两次改的是"有哪几类词"，这次改的是"**一个词是什么**"——
词从一个点目标变成一条带子，所以它动的不只是契约，还有画法。

**画法的规矩：冗余画在它累积到的那根轴上。** 垂直的冗余积到高度（δ × 飞行距离），画成随距离张开的
扇形和三维里的一面墙；速度的冗余积到到达时间（≈ D·δ/v²），画成平面图上两条低饱和的角点线加句子条
标题行的到达时间窗。用户原提议是"整条曲线按速度上下界各画一条"——照做了，但**读数不放在那两条线
上**：速度在平面里只差转弯半径 6 %（70 m/s、20° 坡度上 82 m），只看线会得出"速度的松量无害"这个错
结论。航向、跑道、时长、终止**没有冗余，就不画带子**。

**两条便宜的事实**：`fly()` 里 γ 只进 `height +=` 那一步，水平位移、转弯速率、停止判据都不看它，
所以垂直的上下界与标称航迹**逐位对齐**，产物里只多两路高度而不是两条航迹；速度反过来，它改水平
位移、转弯半径和穿越时刻，必须真的重飞两遍。

**垂直图上两个误差同时可读**：按真斜率还原的细线与按档心还原的线之间是**档位误差**，扇形的宽度是
**解码松量**。以前这两个数只能各自被引用，现在画在一张图上能直接比大小。冗余还顺便给出一个读数：
**在带子里的时间占比**（按类算，前端算，不进产物）——它量的是读法，不是模型。

**T9 / T11 / T12 / T13 / T14 落地**（前端 707 条全过，两个 tsc 干净）：契约改写（档心改成查表、
两个容差镜像函数、`verticalBand` / `speedBand` / `observed.pathM` 的解析与校验）、读数窗口的垂直图
（按段重锚的高度线 + 扇形 + 真斜率细线、越界着色、按类的占比）、句子条（垂直行 + 箭头标签 + 带冗余
的词标签 + 到达时间窗）、三维的 Cesium wall、发布检查带上读法。**T10 是 Python 侧**（导出器与运动学
的 band），`instructions.py` / `instruction_kinematics.py` / `instruction_sample_export.py` 正被词表
那条线的会话改着，共用一个工作区，所以没动。

**旧产物被取代**：新读者按 `kinds` 逐字比对，旧文件每一行都解析得过、只有那一行看得出第二列换了
意思，所以整份拒绝、界面落到设计 §4.5 的 ③（盘上那份后来被词表那条线删掉了，现在是 ①）。
**没有加任何兼容分支。** 设计与决定表：`aeroviz-4d/docs/36-2026-09-20-training-module.zh.md`
§4.3 / §5.6 / V29–V39；长期事实：`aeroviz-4d/docs/35-viewer-reference.md` AV19。

### 2026-09-21 — Training 模块 T5 / T8 / T6 前半：把词飞出来，看它到不到得了跑道

**T5 几何航迹**（`manoeuvre/instruction_kinematics.py`）。一句话只说四个绝对目标，不说转多快降多快，
所以「把词画出来」要补一套最简单的运动学：按航路的 20° 坡度转、按控制器的 12 s 时间常数收高度
（下降 ≤ 6°、爬升 ≤ 2°）、按 1.0 m/s² 加减速。**常数全部 import**。放在 `manoeuvre/` 而不是
`experiments/`：后者除 `__main__` / `support` 之外每个模块都会被注册成一个 runner。

**第一个读数（KRDU 40 架，直线 20 / 引导 20）——这是这个模块存在的理由：**

| | 按词飞到了跑道 | 停下来时离入口 p50 | p90 |
|---|---|---|---|
| 直线进近 | **15 / 20** | 154 m | 7.9 km |
| 雷达引导 | **5 / 20** | **9.0 km** | 14.8 km |

**一半的句子，按规则飞出来到不了跑道**；引导进近尤其到不了。整段的平均差分解开：
沿航道 546 m、横向 379 m、高度 234 m（p50），横向在 18/40 架上大于沿航道 —— 所以
**尾部是横向主导，中位是沿航道主导**，不能笼统说「差的主要是横向」。

**这个读数是词 + 这套运动学的联合结果，不是词本身的判决**（§5.5：规则制导只作基线与诊断）。
评审跑了三个变体：喂**精确目标**（不过 10° 档）反而更差，说明**档位不是原因**；
喂**观测的真实航向**（理想航向）仍剩下约 47 % 的平均差，那部分是速度档 + 加减速率 + 高度档，
是这套诊断自己的地板。真正的机制更准确的说法是：标注器把第一个平台的值**回填到 t = 0**，
于是一架切入之后才稳定的飞机，整段只得到**一个**航向词（航道本身），句子里根本没有它飞的那次切入。

**评审改了 7 处，其中一处是真 bug，而且它改变了上面每一个数**：

- **`crossed-threshold` 只看了入口平面**。仓库 2026-09-09 已经为这个错误付过一次代价
  （雷达引导的下风边平行于航道、横着 8.7 km，从那里穿过 `d = 0`）。这里的第一版让
  **40 架全报「到达」**，其中 14 架落在中心线外 2 km 开外，**10 架在最后一条航向词飞出来之前
  就被截断**。现在是**朝里穿越** + **在最后进近上**（500 m / 30°，用 `course_frame_rows`
  判 `established` 的同一条规则）。改完才有了上面那张表；改之前的表是错的。
- **地速被当成了航径速度**：`course_frame` 的 `ground_speed_mps` 就是水平速率（实测
  \|Δposition\| / (V·Δt) 中位 0.99966），所以水平步长是 `V·dt`、升降是 `V·tan γ·dt`，
  不是 `V·cos γ` / `V·sin γ`。另外一步之内竖直用旧速度、水平用新速度，已改成一步一个速度。
- **转弯速率不该由 `route_turn_radius_m` 反推**：那个函数给**半径**设了 40 m/s 地板，
  当速率读会把物理倒过来（V→0 时速率趋零，实际是变快）。改成 `g·tan φ / V`。
- `wrap_deg` 是第二处定义，改为 import `data/runway_context.py` 的那一个。
- `fly` 自己施加「观测时长 + 120 s」的上限，因为 `geometry` 块把这条规则写进了产物。
- V19 那道缝补了一个检查：句子是从产物抄的、航迹是重建的，**除了这一行没有任何东西说它们是同一架**。
- 删掉一个永远不会命中的 `max(speed, 1.0)`。

**评审的变异测试发现两条测试是空的**：把 `cross -=` 改成 `+=`，原来 12 条一条都不会红；
加速度那条在拿模块自己的再导出做断言，把模块里的常数改成 2.5 也全绿。两条都补了，
外加爬升上限（真数据里 9/40 架起步是爬的）和「终止词不参与飞行」（这条被写过一次又反过来）。

**T8 发布检查**：`npm run check-publication` 学会了 `training/index.json` —— 清单、每个集合的
`sample.json` 都过前端自己的读者，并比对两份文件的 sha / 读法 / 架数。**动机和比较视图是反的**：
那边一个坏条目清空整个机场，检查是为了拦住；这边坏集合按 §4.5 ③ 只标灰自己，所以「导出到一半」
很容易发布出去没人发现。真产物上 0 errors；故意加一个磁盘上没有的集合，脚本报
`ERROR [half_written] half_written/sample.json is listed but missing on disk`。

**T6 后半（T6b）**：两条航迹进三维。图层**画静态折线，不建时间采样实体** —— 后者会驱动共享的
Cesium 时钟，而那个时钟是 Observe 播放用的（V2）。导出器给两条航迹都补了经纬高，**并在出口处
把 MSL 换成 HAE**：记录是 MSL，Cesium 的 `cartographicDegrees` 读的是椭球高，漏了这一步每条线
都会沉到自己地形底下 33.5 m。写的时候踩了一个坑：图表的竖直轴是从**坐标系锚点**量的
（`channels.py`：u = altitude − frame.alt0），少加锚点高程会把接地点算到椭球面以下 4 m。
**对着已有的观测 CZML 核过**：那边末行 HAE p50 129.5 m，这份导出末行 p50 135.7 m。

**T6 前半**：读者解析 `geometric`，读数核对窗口画上第二条线（平面图 + 三张图），标题栏给出
**这一刻两条线差多少**（词停了就说「the words have stopped」）。第二条线的图例按既有规矩写明是
**按规则飞出来的基线、不是模型的答案**。三维图层和经纬高（连同 MSL→HAE 的换算）是 T6 后半。

**核过的**：ts 全套 **1310 条**、前端 **670 条**全过，两个 tsc 干净，产物重导四次。

### 2026-09-21 — Training 模块 T4b：读数核对窗口（人工核那 300 页的可交互版）

`TrainingReadbackWindow`：平面图（跑道坐标系）+ 三张时间图（航向、高度、速度），每张图把
**此刻生效的词画成台阶**压在实测信号上。纸上那 300 页人工核图，人是在上面标
一致 / 漏读 / 误读；这个窗口回答同一个问题，但数都挂着，而且看的是**同一批飞机**。

**最要紧的一条是航向图**：实测画的是**展开角**（unwrap 过的），所以词的台阶必须放到
**航迹所在的那一圈**上（`+360×round(...)`，判定时刻取这条指令 settled 的那一行）。不这么做，
一架转过 360° 的飞机会被读成「误读词」。评审把 `chartLevels` 和 Python 的 `hand_check_figure.steps`
拿真产物逐条对了一遍：**328 条台阶，零条不一致**。

窗口**不自己存游标**，用句子条那个 —— 同一架飞机的同一刻就该是一个数。`formatSeconds` 也因此
收进数据模块成为一处定义：两边各写各的时，同一个游标一个显示 201.9 s 一个显示 202 s。

读者补上了 `observed` 八列（跑道坐标系的那几路），并按**航迹自己的时钟**校验：从 0 开始、
到 `durationS` 结束（按导出自己的精度比，不做位级相等——`tS` 写出来四舍五入到 0.1 s 而
`durationS` 没有，早晚会因为一个舍入差把整份文件拒掉，还把原因说成「航迹和句子不是同一架」）。

**评审改了四处**：① 悬停提示把 `Instruction.target` 原样印了出来——那是 SI，旁边的标签却是
英尺和节，于是出现「speed 190 kt … target 98.7」。这恰恰是人用来判「这个词读对没有」的那一个
提示，已改成按图自己的单位读。② never-settled 那条分支**测了个寂寞**：夹具里唯一的
`settledS: null` 是高度指令，而高度分支在读 `settledS` 之前就返回了——回退写成「按 t=0 判定」
也能全绿。现在钉在航向指令上，配一条中途换圈的实测。③ 词类颜色两个视图各定义一份，已并成一处。
④ 一个永远不会命中的 `?? WORD_COLOUR` 回退删掉了——Python 在那里是直接 KeyError 的。

**核过的**：前端 86 文件 652 条全过，`tsc --noEmit` 干净；浏览器里在 KRDU 真产物上核过。
...
- **`manoeuvre/instructions.py`** — the vocabulary (heading 10° × 36 relative to the final approach course; height above
  ...
  the ONE course-frame definition (the anchor covariates, the failure modes and the labeller read it).
- **`plan_conditioning = instruction`** (`outputs/control/instruction_token.py`): the executor is handed the words in force
  at the Δ/τ positions of its segment as bin centres, fused beside the condition like the CTA; one source for a training
  row, `predict` and the closed loop (lockstep protocol `truth-instruction`: the truth's words at the truth row nearest
  the FLOWN position, read on the whole record before any cohort cut, the truth time / distance recorded per round;
  payload `ts-manoeuvre-lockstep-v5`). Config field `instruction_vocabulary` (control view, `vocab=` in the name); the
  head opens the artefact once and the checkpoint stores THAT sha; load refuses a moved or missing artefact by name.
  An `off` run is byte-identical to before (both checkpoint canaries load).
- **`manoeuvre/instruction_sequences.py` + `instruction_prior.py`** — the sentence with threshold-relative state tokens,
  `ends_at_landing` (a rolled prefix's last position has a next word), the causal Transformer with four factorised
  heads + landed, readings with their denominators stated (undefined rates read as None), joint top-K coverage,
  hold / bigram baselines; `run_ts.py instruction_prior`.
- **Gate B1 per D57**; `run_ts.py two_tier_bprime_queue` (baselines flown by the queue → train two seeds → truth-instruction
  readings → gate b1 → the prior past the gate); `docs/experiments/two_tier_v3_bprime_arms.json` (I20 × 2 seeds) and the
  campaign's intent. B2′ (closed-loop fine-tuning) and B3′'s decoding are designed (plan §5.2.2), not built.
- Config: the archived plan token's five fields are retired as UNREAD serialized fields (`PLAN_TOKEN_RETIRED_FIELDS`) —
  stage A checkpoints store them inert beside `plan_conditioning='off'` and had stopped loading after the archive.
