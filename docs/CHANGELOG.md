...
- 在工作树自己的机场目录（实时数据的镜像：`training/` 是真目录，其余都是链接）里导出两份、核对过；**还没有写进真正的 `public/data`**
  （写共享产物要用户同意）。base 模型 `prior/v3_step1_20260924/full_s1337` 与后训练第 1 轮 `prior/v3_rl_20260925/grpo_s1337/round_01`，
...
- 同一次还在镜像里导出了现行集合上还没有的两种旧叠加层（用现有导出器，代码不改）：base 模型的教师强制预测
  ...
  相同；200 架里飞 188 架，全部落地）。发布脚本一次写四份，也要等用户同意。
