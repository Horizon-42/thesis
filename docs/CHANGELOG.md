...
### 2026-09-07 — anytime predictions became something you can LOOK at: `--write-records`, one category per bin
...
**Hazard hit at the end of the session**: KRDU's `arrivals/lateral_pass_eligibility.json` was
regenerated by another process at 08:15, after the publish. Its SHA-256 no longer matches what
these checkpoints recorded, so `preflight_error` now blocks ANY (re)publication of them —
correctly, but including yesterday's L−1 categories. `--refresh-labels-only` is unaffected (it
reads stored manifests only). The published artifacts predate the change and are valid.
...
cannot load. Restart the frontend after publishing new categories.
