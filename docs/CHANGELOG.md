...
- 后端 `POST /autopilot/segment`（`aeroviz_backend/autopilot_segment.py`）：观测飞机说这个词时的状态为初态，第 0 步是那一步
  ...
  它少判几行、只持续一步的航向词——四成——根本判不了；到句子末尾就飞到落地）；执行器原样用 `replay.fly_sentences` 飞，按它
  记下的词钟截断；只判选中的词（执行器的判决）。规格在 `outputs/POOLED/executor/` 里找恰好一份现在的代码接受的。只有点击
  句子条上的色块才请求（代码审查：跟着游标的话，图表上一悬停就连发请求）。
- 前端：面板开关与结果卡、句子条一行摘要、三维里飞机把这一段飞出来、读数窗口四张图的蓝线；答复绑到句子条这一段（告诉执行器
  的词必须是屏幕上这一段的词），格式 `aeroviz-autopilot-segment-v1`。
- 读取器的规格换成 `145d6911e75b`（`v4_20260924`，按运行日划分），因为合并 `dev-prior-v3` 之后的执行器代码只飞这份产物（规格
  `v6_20260924`）；用户同意后导出新集合 `instruction_v3_day_split`（五个机场各 40 架新验证集航班，`public/data`，导出时代码
