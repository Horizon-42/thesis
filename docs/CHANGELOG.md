...
- **布局**（用户：左边栏会和底部的词表看板重合）：句子条量自己的高度（`--training-bar-height`），句子条在时左右两侧的面板都停在它上面；
  航班列表占左栏剩下的高度（最少约六行，再挤就整个左栏滚动）。AV30。
- **模型自己说的句子**（用户：已经有训练和后训练出的词序列，前端展示没做完；先验产出的要和真值在视觉上区分）：新的叠加层种类
  `prior-generation`（`aeroviz-training-generation-v1`），导出器 `prior_generation_training_export`（ts R13）——和正式自由生成读数同一条
  路径，每架航班 4 个样本，只飞自己机型有动力学的航班，带正式 val 读数。前端：句子条头部的标签页 Truth / 各模型、样本按钮；模型的带
  斜线加虚线边框，前 8 步灰斜纹，每行底边白色短竖线标真值说词的地方；横轴是观测航班的长度，读的样本更长时延到它结束。左栏
  "Sentences read" 与标签页同一个选择，航班列表每行末尾是所选模型的样本（● 落地 ○ 没落地），底部折叠一张落地表（这个集合 / 正式
  val / 标注的词）。三维：真值总是画；读的样本实线、其余样本细线半透明，选中的词画在模型的航迹上。颜色按角色：base 模型品红
  `#d946ef`、后训练黄绿 `#a3e635`（配色校验：两者与三维里画的每种颜色 OKLab 色差 ≥ 9.5）。AV31。
- 先在工作树自己的机场目录（实时数据的镜像：`training/` 是真目录，其余都是链接）里导出核对，再经用户同意发布到 `public/data`：
  每个机场 4 份叠加层，`check-publication` 五个机场 0 错误，数字与镜像里的逐个相同。base 模型 `prior/v3_step1_20260924/full_s1337` 与后训练第 1 轮 `prior/v3_rl_20260925/grpo_s1337/round_01`，
  执行器规格 `v7_20260925`（`v8` 内容 sha 相同，但它的源码 sha 包含已归档的 CAT-K，现在的代码拒读）。集合 `instruction_v3_day_split`
  五个机场 200 架里自己机型动力学 180 架 × 4：base 模型落地 652/720（90.6 %），后训练 694/720（96.4 %）；正式 val 读数 90.2 % / 97.2 %。
  重跑逐字相同。`check-publication --airports-root` 五个机场 0 错误，两份叠加层都按前端的读取函数对照集合读过。
- 同一次还在镜像里导出了现行集合上还没有的两种旧叠加层（用现有导出器，代码不改）：base 模型的教师强制预测
  （`prior_v3_step1_20260924_full_s1337`）、执行器 v6 的验证集回放（`executor_v6_20260924`，规格 v7 打开，每架重飞都与正式回放那一行
  相同；200 架里飞 188 架，全部落地），同一次发布。
- 两轮 opus 审查：第一轮 8 条应改、11 条小处全部改掉（换模型从第 1 个样本开始；模型不飞的航班画真值连同真值的整个头部；结束之后
  还说的词涂暗；航迹每点一步的核对；正式读数分这个机场与全部机场；每个模型自己的抽样说明；只留当前集合的下载、Retry；导出器端到端
  测试；打开先验的检查只留一份；换跑道 / 复飞 / 结束时许可与词对账；Read-back 与 Prior 只在读真值时给出）；第二轮确认，另补一个模型
  不飞集合里任何航班时读数表会崩的问题和 `TRAINING_APPROACH_GO_AROUND` 的镜像钉住。
- 测试：前端 98 个文件 822 条、`tsc`、`typecheck:scripts` 通过；ts 全套 1,434 条通过。浏览器（5174 测试服务器）核对：左栏停在句子条上面、
  标签页与样本切换、模型不飞的航班、结束之后涂暗（KMSY DAL8784 后训练第 2 个样本 325 s 越过入口没截获、模型说到 650 s）、三维模型航迹、图例。
...
- **Observe 的采样数输入框不再每敲一个键就重新规划轨迹加载**（审查又发现的）：原来输入 200 会依次按 2、20、200 各规划一次加载；
  现在输入时只是草稿，离开输入框或按 Enter 才提交，草稿未生效时下面写着 "Enter to apply · Esc to keep N"，Esc 放弃草稿。清空
  为 0（全部）；浏览器读不成数字的输入（"-"、"1e"、Firefox 里的 "20o"）和负数被拒、保持原值——原来它们都变成 0，加载全部轨迹
  （审查发现）。浏览器：逐字输入 30 时后端没有收到请求，离开输入框后只请求一次 `limit=30`。
- 测试：前端 97 个文件 788 条通过，`tsc` 与 `typecheck:scripts` 无错；浏览器核对：HUD 在 KSTL 从 Preload 0/41 数到 Active、
...
- **图上悬停不再让整个应用重新渲染**：Training 的游标在图上每次鼠标移动都变，它原来在 `useApp()` 展开的 context 里，而
  `FlightApp` 自己调用三维图层——每移动一下，外壳、两侧面板、航班列表、HUD 和约 50 个 `useApp` 的组件都重新渲染。现在游标是
  单独的一个 context（`useTrainingCursor`，`useApp` 不读），三维图层在什么也不渲染的叶子组件 `TrainingScene` 里；测试钉住
  移动游标时 `FlightApp` 与 `useApp` 的读者都不重渲染。
- **离开 Training 再回来，会话都在**：面板打开后一直挂着，在别的任务里只隐藏；集合、航班、选中的词、游标、实时执行器的答复
  都保留，什么也不重新下载（原来要重下索引、样本——KRDU 的 6 MB——和叠加层，航班回到第一架）。句子条也和三维一样只在
  Training 里画。会话只属于打开它的那个机场：审查发现隐藏的面板会在别的任务里为每次换到的机场后台下载 Training 文件（样本
  还下载两次），现在在别的任务里换机场就丢掉会话，下次进 Training 重新打开。
- **Pilot 的测试夹具换成后端真实的机型目录**（今天的机型都是单值：目标速度 = 区间下限，上限 +20 kt）；另加一个"区间型"
  条目专门让解析与提交的测试分得清目标和下限；补上机型目录加载失败时面板的测试。
- 审查另外发现、记进 `docs/code-health-followups.md` 的：本地地形预加载每个瓦片、测距环滑块每一步都会让整个应用重渲染；
  Pilot 的机型目录加载失败的原因一进 Trajectory 就被清掉。
- 测试：前端 96 个文件 781 条通过，`tsc` 与 `typecheck:scripts` 无错；浏览器（5174 / 8766）核对过：离开再回来不重新下载、
  Observe 里没有 Training 的东西、在 Observe 里换机场不下载 Training 文件、回到 Training 为新机场各下载一次。

### 2026-09-25 — 后训练第二阶段：只留下滑道下沿（第 0 步的开发测量）

- 选择集上量（读数文档 §8.1）：RNAV 在 FAF 以外的高度限制无论怎么划区域，都会屏蔽 10–15 % 的标注高度词、结束 10–14 % 的重飞；观测航迹
  低于过渡段下限的 12.4 %（雷达引导按 MVA 飞）。FAF 以内的下滑道下沿与数据相符（观测 0.85 %，平飞词 0.2 %）。
- 用户决定只保留下滑道下沿；"下降至落地"不做直线预判；重飞的通过线 3 %。后训练设计 §3、§7、§8 改写；词表设计 §5.1 同步。
...
- 新文档 `4dTrajectory/ts_transformer/docs/2026-09-25_post_training_design.zh.md`：后训练的全部内容从先验设计搬过来（先验设计
  §7 的后训练顺序、§9.2 CAT-K 不采用、§9.3 按落地强化、§10 的对应取值、§11 的 CAT-K 一段；原处留指路）。
- 第二阶段（用户 2026-09-25 提出）：RNAV(GPS) 进近编码的"不低于"高度（FAF 以外，按位置的阶梯）和 FAF 以内不低于下滑道 60 m
  （照优化器 `approach_constraints` 的下沿）作屏蔽；屏蔽照顾不到的由飞出的航迹再查，违反即失败、奖励 0；从训练集起点绕机场
  转、平移高度、缩放速度造新起点，只用终点奖励 + 拉回第 1 步模型训练。取值都是"建议"，先在标注数据上验屏蔽（第 0 步）。
- 量了程序数据：25 条候选跑道都有 RNAV(GPS) 文件；25 km 以内只有下限；FAF 下限与下滑道在 FAF 处的高度相差 −31…+11 m。
- 词表设计 §5.1：下滑道下沿与 RNAV 高度下限由"只评估"改为屏蔽（指向后训练设计 §3）。
