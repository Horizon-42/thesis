"""ARCHIVED 2026-09-20 — the intent-code gates T / X / P / E / S, copied VERBATIM out of
`manoeuvre/gates.py` when the intent-code second layer was retired (plan v3 §10 audit; the
2026-09-20 rewrite of stage B to an instruction vocabulary). `manoeuvre/gates.py` stayed live
for `gate_grid` / `gate_relative` (stage A and the relative gate), so these functions were
copied here rather than moved with the file; the three helpers below (`_two_seeds`,
`_require_protocol`, `_cell`) are copies of the live ones, so this file reads on its own.

Nothing imports this file: `archive/` is off the import path (`tests/test_architecture.py`).
Their numbers: `docs/2026-09-18_manoeuvre_token_results.zh.md` §9–§11.
