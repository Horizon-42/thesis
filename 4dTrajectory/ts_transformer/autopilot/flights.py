...
The rebuilt flight must reproduce the stored signals row for row (`instructions.signals.
signals_from_series`), or it is refused: an executor flown from another flight than the one the
sentence was read off answers nothing.
...
#: manifest reproduces them exactly; the tolerance absorbs only float formatting).
SIGNAL_TOLERANCE = 1e-6


def rebuild_series(directory: Path, signals: Sequence[FlightSignals]) -> list[FlightSeries]:
    """The `FlightSeries` behind each of ``signals``, in the same order."""
...
def require_same_flight(series: FlightSeries, signals: FlightSignals, geometry: AirportGeometry) -> None:
    """The rebuilt flight reproduces the stored signals, every row of every field."""
