...
**门 L1 的 lockstep 句全部不过**：雷达引导 ADE 2133–8061 m（门 < 1500），直线 341–534 m（门 < 250），雷达引导完全可飞 0–0.47（门 ≥ 0.95），雷达引导建立 0.02–0.25（门 ≥ 0.88 × 规则制导 0.879 = 0.77）；只有直线建立（0.89–0.94 对 0.88）过。雷达引导航班几乎都在时限被切（435/497 `horizon`），即跟踪器沿着真值航路点也没飞到五边。receding 比 no-plan 好 130–800 m（tf/sf/pa），说明 token 在多次询问下有一点作用，但离"跟着计划飞"很远——与 §10.1 的 token 盲一致。2c（hook 开）与 2d（门文件）待补。
