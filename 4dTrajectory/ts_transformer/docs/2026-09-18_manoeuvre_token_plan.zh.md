...
| **全部实验已停（用户，2026-09-18 15:29）** | campaign 链在 90 s 段第 13 臂（`S90_K256_s1337`，未训完，目录留有 running 状态，重跑前按 E8 移开）被终止；20 / 120 s 段未跑。**本计划待重新设计**：审核结论见 §6.3 第 18–19 条——token campaign 建在未定的执行器基线上（回看被静默固定在 60 s；段长该先在无 token 执行器上消融，而不是当 token campaign 的轴）。新计划草案：`2026-09-18_l1_executor_plan_v3.zh.md`（逐项审核中），签字前不启动任何实验 | — |
