...
> **状态（2026-09-20）**：本方案的**代码已归档**到 `4dTrajectory/ts_transformer/archive/manoeuvre_codes_2026_09/`（该目录的 README 写了搬了什么、为什么、读数在哪）。原因见 `2026-09-18_two_tier_plan_v3.zh.md` §10 审计与 2026-09-20 重写的 §5.2 / §6.2：第二层改为**指令词表**。本文与读数文档不删，作为意图码版本的设计与证据记录。
...
| **全部实验已停（用户，2026-09-18 15:29）** | campaign 链在 90 s 段第 13 臂（`S90_K256_s1337`，未训完，目录留有 running 状态，重跑前按 E8 移开）被终止；20 / 120 s 段未跑。**本计划待重新设计**：审核结论见 §6.3 第 18–19 条——token campaign 建在未定的执行器基线上（回看被静默固定在 60 s；段长该先在无 token 执行器上消融，而不是当 token campaign 的轴）。新计划草案：`2026-09-18_two_tier_plan_v3.zh.md`（逐项审核中），签字前不启动任何实验 | — |
...
17. **门 E 的进展线被 L1 自己过了：计划的下一步怎么走（用户定）。** 60 s 时域训练、每 60 s 再问的无 token 执行器：雷达 ADE 2362 / 2481（线 ≤ 2919 / 2949）、建立 0.699 / 0.703
    （线 ≥ 0.690 / 0.699），两种子过进展线；v2 的无计划跟踪器（30 s 再问、118 s 时域、120 s 回看）是 ~3700 / ~0.60——差别在段长、整段执行与回看三处（第 18 条），不能只归到段长；但无论哪一处，都不是 token。§0 的论点"让 L1 短、单峰"
...
18. **回看窗口是我定的、没有消融的变量（用户 2026-09-18 下午指出）。** §2.6 的 `seq_len = 30`（60 s 回看，所有段长相同）是我晚间改写时定的：
    v2 的执行器是 120 s 回看，我改成 60 s 是为了让部署的首问早（§2.2：首问在进入切片后 max(60 s, 段长)），但没有测量依据，也没列进本节让你审。
    后果：段长曲线（30 / 60 / 90 s 建立 0.70 / 0.70 / 0.75）里混着"承诺更长"和"信息没跟上"，分不开；第 17 条里"对 v2 的差别是段长"说过头了——
    v2 是 118 s 时域 + 120 s 回看 + 每 30 s 只执行前一段，三处同时变。**"回看 = 段长"是最自然的默认，固定锚点 59 处四个段长都做得到**
    （`seq_len` 15 / 30 / 45 / 60）。补法：无 token 执行器上做 L × Δ 网格（12 配置 × 两种子，约 5–6 h）或只做 L = Δ 对角线（新增 3 × 2 臂，约 1.5 h），
    协议 `none` 各读一次；等你选。
19. **审核结论（2026-09-18 下午，用户要求）：本计划的下游设计不合理，需重写。** 前提（L1 需要意图码才能短、单峰）被读数推翻：无 token 的分段执行器已到 v2 两层的目标；
    真值码只换跟踪不换穿越；先验码一无所得；学习码只装航向；语义词表的码执行器不看；瓶颈是执行器在雷达引导段的穿越（0.3–0.4）。逐项：P1 的 ADE 联合目标装不进意图；
    门 T 无区分力；门 S 前提不成立；P3.3 无对象；P4 / P5 建在惰性码上。保留的工具：lockstep 四协议、门 X / E 的绝对线、门 G 先 oracle、门 R。
    重写方向（待用户敲定）：阶段 A 无 token 执行器的 L × Δ 网格 + 雷达引导穿越诊断；阶段 B 第二层的相对门（协议 A 对 `none`）；阶段 C 分层放到信息真正缺失处（跑道、多机），
    连续 z。**在新计划签字前不启动任何实验。**
