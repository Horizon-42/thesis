...
| P1 分词器 + 执行器（联合） | **P1.1–P1.3 完成 2026-09-18**（`manoeuvre/segments.py` 81ca7b1、`manoeuvre/tokenizer.py` 31b9c24、执行器接线 + review 修正 + 码本导出 runner 见下一提交）：ts 套件 1218 全过；opus review 的 2 BLOCKER / 3 MAJOR / 6 MINOR 全部修掉（航向 wrap、码本未记录尺度、越界码、常量重复、冻结模式、身份进 sha）。**P1.4 campaign `manoeuvre_tok_20260918` 已启动 2026-09-18 06:13**（c391265；5 段长 × 14 臂 = 70 臂，队列顺序 60→30→90→20→120 s，每臂约 11 min，链 `<campaign>/queue_p14.sh` PID 2671532；读数 `run_ts.py manoeuvre_readout` 每段训完由 opus 队列代理跑并回报；结果进 results 文件 P1.4 节）。**60 s 段全读 09:40（14/14 臂）：门 T 六个词表全 FAIL**——K16…K256 与指令词表两种子配对增益 p50 +2…+10 m（线 30 m；K 轴整个差 3.6 m；增益在雷达引导层、重尾），有效词表饱和在 8–27 个码、最大份额 30–79 %（指令词表自己 79 %：25 % 线在这个 cohort 上无法达到）；T(ii) 过（K32 先验 0.564 / 0.526 对 bigram 1.511 / 1.404 nats/码）；**30 s 段全读 12:15：六个词表也全 FAIL**（增益 +1…+3 m 对无 token 的 32 m；K16 用满 16 码但最大份额 38–43 %）；90 s 段 12:12 开始（每臂更长，约 3–4 h），再 20 / 120 s；事后改门的问题在 §6.3 第 14 条。**闭环里指令词表臂好于 K32**（`p23_S60_cv`，09:04：协议 A 建立 0.594 / 0.608 对 0.422 / 0.481）**，但它的执行器几乎不看码**（09:12 核实）；**无 token 对照（协议 `none`，09:15）建立 0.699 / 0.703，高于所有带码协议，并自己过了门 E 的进展线**——60 s 段的意图码在闭环里目前没有价值；T(i) 没有区分力；下一步见 §6.3 第 16–17 条 | §4.1 P1.1–P1.4，门 T（§3.3）；§6.3 我替你选的 |
...
13. **门 T 在 60 s 段 FAIL 后我做的事（2026-09-18 08:08）**：门按预注册判、不改线；campaign 照跑其余段长（30 → 90 → 20 → 120）；
    ...
    P3.3 是回答——先验侧（回灌训练）08:20 排队、08:49 完成：**门 E 仍 FAIL**（建立 0.462 / 0.476，雷达 ADE 略差；results 文件 P2.3 + P3.2 节）；执行器侧不写（门 E 没动）。 **协议 `none`（aa9090b）：无 token 执行器建立 0.699 / 0.703，过门 E 进展线**（第 17 条）。 指令词表的回灌先验（`p33_S60_cv`，09:22）也没帮上：建立 0.601 / 0.534 对平的 0.594 / 0.608，门 E FAIL。 **08:51 又在指令词表臂上跑同一诊断链**（`queue_p23.sh 60 cv` → `p23_S60_cv/`，无连续对照）：一个固定的语义词表在闭环里的建立与 ADE，是学习码本的非学习参照，第 14(a) / 15 条要用。
...
15. **离散对连续的第一读不在这里定。** 门 P 的对照在 (60 s, K32) 上是连续版赢（雷达 ADE 好 272 / 141 m，建立平手；results 文件 P2.3 + P3.2 节）。
    ...
    用户定。**
    **09:04 的补充**：指令词表臂跑了同一条链（results 文件 `p23_S60_cv` 节）——它在闭环里全面好于 K32 的离散版**和连续版**（建立 0.594 / 0.608
    对 0.422 / 0.481 与 0.397 / 0.478），所以"离散输给连续"是塌成航向扇的学习码本的事，不是离散化的事；这条不再是换主线的理由。
16. **意图空间的主线怎么选（用户定；我的建议在最后；09:12 按核实后的读法改写）。** 事实：(i) 三个词表在门 T(i) 上不可区分且都 FAIL；
    (ii) 闭环里指令词表臂两种子都明显好于学习码本 K32（门 E 的建立差线 9–10 点对 22–27 点；雷达 ADE 2342–2363 已过 G3 的目标线 2745）；
    (iii) **但原因是指令词表的执行器几乎不看码**（码图谱：63 个类的段末散布 sd 204 m 对 K32 的 1468 m；协议 A 里先验错码一半、e_plan 却只有 ≤ 230 m），
    两种词表逐轮 e_track 相同，差别全在末几轮 K32 被错码带偏、穿不过入口。也就是说：目前最好的闭环数字来自一个不用码的执行器，而"有效力的码"（K32 的航向扇）
    在先验出错时是负担。缺的对照是无 token 执行器在同一 lockstep 下的读数（协议 `none`，我在加；results 文件 `p23_S60_cv` 节补）。
    选项：**(a) 先补无 token 对照再定**——若无 token ≥ 指令词表，则 60 s 段的意图码在闭环里目前没有价值，问题回到"码里装什么才有用而不有害"
    （垂直 / 速度内容，第 14(d) 条；先验与执行器对错码的鲁棒性）；(b) 以指令词表为 P3–P5 的工作空间（可解释、与管制语言对齐、T(ii) 过）——
    但它的码在执行器里是惰性的，多机与掩码接口会建立在一个不起作用的码上；(c) 学习分词器加垂直 / 速度分量再比（约 14 臂 × 12 min）。
    **无 token 对照已补（09:15，协议 `none`，aa9090b）：不用码的执行器建立 0.699 / 0.703，高于所有带码协议，雷达 ADE 2362 / 2481 与最好的带码持平，
    并且自己过了门 E 的进展线**（results 文件 `p23_S60_cv` 节末）。所以 (a) 的答案已经有了：60 s 段的意图码在闭环里目前没有价值——真值码只换来 −500 m 的雷达跟踪（读未来），
    先验码什么都没换来。**我的建议**：主线不选任何词表；把"意图码有没有用"改成相对读数（第 17 条），先看其余段长的无 token 执行器（第 17 条第二点）。
17. **门 E 的进展线被 L1 自己过了：计划的下一步怎么走（用户定）。** 60 s 时域训练、每 60 s 再问的无 token 执行器：雷达 ADE 2362 / 2481（线 ≤ 2919 / 2949）、建立 0.699 / 0.703
    （线 ≥ 0.690 / 0.699），两种子过进展线；v2 的无计划跟踪器（30 s 再问、118 s 时域）是 ~3700 / ~0.60——**差别是段长，不是 token**。§0 的论点"让 L1 短、单峰"
    不需要意图码就成立了。建议：(i) 把无 token 的同协议读数定为每个段长的基线，意图码的门改成"协议 A 相对 `none` 的建立与雷达 ADE，两种子都不差且至少一项好过种子线"；
    (ii) campaign 其余段长（30 / 90 / 20 / 120）的无 token 臂训完就跑协议 `none`（每对 3 min，我会跑）——段长对闭环建立的曲线本身就是一个结果，也是门 S 的诚实版本；
    (iii) 意图码要有价值，得装进真值码里有而历史里没有、且先验能预测的东西（垂直 / 速度的时机，第 14(d) 条）；在此之前 P4 / P5 不要建在码上。
    目标线的建立 ≥ 0.94 仍远（雷达层 0.33）：那是执行器在雷达引导段的穿越问题（门 X 的建立线也没过），与 token 无关。
