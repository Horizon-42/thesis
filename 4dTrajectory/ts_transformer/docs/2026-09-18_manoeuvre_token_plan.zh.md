...
16. **意图空间的主线怎么选（用户定；我的建议在最后）。** 事实：(i) 三个词表在门 T(i) 上不可区分且都 FAIL；(ii) 闭环里指令词表（63 类固定语义词表）
    两种子都明显好于学习码本（门 E 的建立差线 9–10 点，K32 差 22–27 点；雷达 ADE 2342–2363 已过 G3 的目标线 2745）；(iii) 差别来自垂直 / 速度类，
    学习码本没学到（码图谱）。选项：**(a) 以指令词表为 P3–P5 的工作意图空间**（可解释、与管制语言对齐、T(ii) 过、闭环最好；学习分词器降为研究臂，
    先做第 14(d) 条的垂直分量臂族再回来比）；(b) 学习分词器加垂直 / 速度分量后再选（多一轮 campaign，约 14 臂 × 12 min）；(c) 两者都留，门 S 用闭环建立选段长时
    两个词表各选各的。**我的建议：(a)，并把门 T(i) 换成闭环读数**（协议 A 的建立与雷达 ADE，两种子）——T(i) 在这个 campaign 里没有区分力，
    而闭环读数一条链 15 min 就有。改门是事后的，所以留给你；campaign 其余段长照跑，它们的 T(i) 读数仍写进 P1.4 节作记录。
