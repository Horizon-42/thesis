...
| P1 分词器 + 执行器（联合） | **P1.1–P1.3 完成 2026-09-18**（`manoeuvre/segments.py` 81ca7b1、`manoeuvre/tokenizer.py` 31b9c24、执行器接线 + review 修正 + 码本导出 runner 见下一提交）：ts 套件 1218 全过；opus review 的 2 BLOCKER / 3 MAJOR / 6 MINOR 全部修掉（航向 wrap、码本未记录尺度、越界码、常量重复、冻结模式、身份进 sha）。**P1.4 campaign `manoeuvre_tok_20260918` 已启动 2026-09-18 06:13**（c391265；5 段长 × 14 臂 = 70 臂，队列顺序 60→30→90→20→120 s，每臂约 11 min，链 `<campaign>/queue_p14.sh` PID 2671532；读数 `run_ts.py manoeuvre_readout` 每段训完由 opus 队列代理跑并回报；结果进 results 文件 P1.4 节）。**60 s 段全读 09:40（14/14 臂）：门 T 六个词表全 FAIL**——K16…K256 与指令词表两种子配对增益 p50 +2…+10 m（线 30 m；K 轴整个差 3.6 m；增益在雷达引导层、重尾），有效词表饱和在 8–27 个码、最大份额 30–79 %（指令词表自己 79 %：25 % 线在这个 cohort 上无法达到）；T(ii) 过（K32 先验 0.564 / 0.526 对 bigram 1.511 / 1.404 nats/码）；其余段长照跑（30 s 段 09:32 开始）；事后改门的问题在 §6.3 第 14 条。**闭环里指令词表臂好于 K32**（`p23_S60_cv`，09:04：协议 A 建立 0.594 / 0.608 对 0.422 / 0.481）**，但它的执行器几乎不看码**（09:12 核实）；**无 token 对照（协议 `none`，09:15）建立 0.699 / 0.703，高于所有带码协议，并自己过了门 E 的进展线**——60 s 段的意图码在闭环里目前没有价值；T(i) 没有区分力；下一步见 §6.3 第 16–17 条 | §4.1 P1.1–P1.4，门 T（§3.3）；§6.3 我替你选的 |
