...
16. **意图空间的主线怎么选（用户定；我的建议在最后；09:12 按核实后的读法改写）。** 事实：(i) 三个词表在门 T(i) 上不可区分且都 FAIL；
    ...
    **我的建议：(a)，并把门 T(i) 换成闭环读数**（协议 A 与 `none` 的建立与雷达 ADE，两种子，一条链 15 min）——T(i) 在这个 campaign 里没有区分力。
    改门是事后的，所以留给你；campaign 其余段长照跑，它们的 T(i) 读数仍写进 P1.4 节作记录。
