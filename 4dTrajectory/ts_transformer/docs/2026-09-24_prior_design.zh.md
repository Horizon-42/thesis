...
| 项 | 状态 |
|---|---|
| 设计 | 用户 2026-09-24 已确认全部取值（§10）；机型这一版不加，模型留一个"每架飞机的静态属性"接口（§4.5）。只剩运动与方向差两块输入给不给，等第二版训练的比较结果 |
...
- **机场层面的落地情况**：在 t 之前的 `T_cfg` 分钟里，同一机场有多少架飞机落在这条跑道上；离上一次有飞机落在这条跑道上
  ...
  和落地时刻，与 `data/runway_context.build_airport_context` 的取法相同），只取落地时刻早于 t 的，并排除测试集运行日的航班。
