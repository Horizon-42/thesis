...
| 代码 | 第 0、1 步和单机自由生成已实现并合并进 `dev-two-tier`（第 0 步 `297c3e10`，第 1 步 `47c7b790`，自由生成 `0bbe6abf`）。第 1 步是单机：每架航班一个场景，模型已按 §6 带飞机轴（本步只有一架）。按落地强化（§9.3）在分支 `dev-prior-rl` 上实现（`e8ee1373`），闭环提速（逐位不变）在 `dev-prior-fast`（`002998fb`），都等用户合并。闭环监督微调（CAT-K，§9.2）不采用，代码已归档（`dev-prior-fast` `e5f18692`，`archive/closed_loop_sft_2026_09/`）；该分支上的执行器代码与 `4205e186` 逐字节相同，执行器规格用 v7 |
...
- **代码**：已归档（用户 2026-09-25）到 `archive/closed_loop_sft_2026_09/`（分支 `dev-prior-fast` `e5f18692`，README 说明搬了什么）；
  逐行编码（`Prior.extend`）、`chain_record` 留在活代码里，自由生成和 §9.3 用到它们。
