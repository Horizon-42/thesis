...
| 训练与读数 | 第 0 步（划分、重测规格、重建句子产物、普查）：读数文档 §3。第 1 步：选中 full（候选跑道带落地情况、各列按顺序输出），验证集每个预测步 0.2643，**§9 第 1 步的门过了**：每列都赢两个基线；还没建立在五边上的航班上，第一步跑道 81.5 %，B1 69.6 %、B3 72.1 %（读数文档 §4）。单机自由生成（内部选择集）：先验说的词落地 89.3 %，标注的词 100 %；门先只记录（用户 2026-09-25）（读数文档 §5）。以前按航班划分的两次训练（读数文档 §1、§2）不作本设计的对照 |
...
- **每一轮**：
  1. **飞出一条链**。从训练集运行日里抽航班（有自己动力学的，与回放门同一组），从第 `N_look` 行的观测状态起飞。每架航班同时
     ...
     - **为什么按段挑、不像 CAT-K 那样按步挑**：CAT-K 的一个词就是 0.5 s 的一段运动，下一步的位置由它直接决定；我们的词是交给
       ...
       足够让航向、速度、高度词的效果显出来。
  2. **重新定目标**（链上从第 `N_look` 行起的每一行 t）：
     - 第一个预测步：第 `N_look` 行标注生效的六个词（与预训练相同，链在这一行就是观测的状态）；
     - 之后每一列：记 L 为标注在第 t 行生效的词，R 为链上到 t 之前生效的词（先验自己说的）。R 等于 L，目标是"不变"；R 等于标注
       在之后 `W` 行内才生效的词（链上说早了），目标也是"不变"，不让它改回去；否则目标是说 L（说晚了、说错了都立即要求说对）；
     - 链比标注的句子长时，超出的行 L 取句子最后一行生效的词（飞机最后就是按这些词落地的）；
     - 链上跑道已锁（执行器的规则：已许可、或已截获）时，跑道列的目标是"不变"——说了执行器也不听；
...
| 说早了的容忍 `W` | 5 行（10 s） | 建议 | 在标注之前 10 s 内说出的词不要求改回去；说晚了立即要求说 |
