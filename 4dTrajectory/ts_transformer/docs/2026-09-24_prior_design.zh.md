...
| 代码 | 第 0、1 步和单机自由生成已实现并合并进 `dev-two-tier`（第 0 步 `297c3e10`，第 1 步 `47c7b790`，自由生成 `0bbe6abf`）。第 1 步是单机：每架航班一个场景，模型已按 §6 带飞机轴（本步只有一架）。单机闭环监督微调（§9.2）在分支 `dev-prior-v3` 上实现（`e760ab1d`），按落地强化（§9.3）在分支 `dev-prior-rl` 上实现 |
| 训练与读数 | 第 0 步（划分、重测规格、重建句子产物、普查）：读数文档 §3。第 1 步：选中 full（候选跑道带落地情况、各列按顺序输出），验证集每个预测步 0.2643，**§9 第 1 步的门过了**：每列都赢两个基线；还没建立在五边上的航班上，第一步跑道 81.5 %，B1 69.6 %、B3 72.1 %（读数文档 §4）。单机自由生成：先验说的词落地 89.3 %（内部选择集）、90.2 %（验证集，只读一次），标注的词 100 % / 99.9 %；门先只记录（用户 2026-09-25）（读数文档 §5）。以前按航班划分的两次训练（读数文档 §1、§2）不作本设计的对照 |
| 下一步 | 单机闭环监督微调（§9.2，代码审查改完即开跑），然后按落地强化（§9.3，取值待用户确认），然后 §9 第 2 步 |
...
- **后训练**（闭环，§9 第 5 步）：执行器同时飞场景里所有飞机，按文献里效果从大到小的顺序做（§11）：
  1. **闭环监督微调**：先验自己说、执行器飞，把飞出的状态喂回先验，目标仍是记录下的指令（CAT-K 的做法），不另加间隔项；
     先在单机上做（§9.2，第 1 步的模型本来就是单机的），多机的在第 5 步；
  2. **间隔作为生成时的筛选和屏蔽**，不作损失：门槛取规章的最小间隔（FAA JO 7110.65 的雷达间隔和尾流间隔，原文在
  ...
  3. **最后才考虑强化学习**：奖励用间隔与落地，带一个把模型拉回参考模型的约束项和数据项，权重适中；不用只有间隔的奖励。
     单机上先只用落地作奖励（§9.3，接在第 1 项的单机闭环监督微调之后）；
...
- **门**：先只记录（用户 2026-09-25），不设通过线；先验说词时的落地率留给闭环监督微调（§9.2）去提高。

### 9.2 单机闭环监督微调（CAT-K 的做法，自由生成之后）

- **为什么**：自由生成里（读数文档 §5），先验说的词只落地 89.3 %，标注的词 100 %；落不了地的航班一直在说航向词、进近许可
  给得晚。先验只在观测的航迹上训练过（teacher forcing），闭环里它读到的却是执行器按它自己说的词飞出来的位置，一旦偏开，就
  到了训练里没见过的状态。闭环监督微调让先验在它自己会飞到的状态上学，目标仍是标注的指令（§7 第 1 项；文献：CAT-K 与
  DAgger，`docs/literature/trajectory_as_language/` §1.2、§1.6、§2.1）。
- **每一轮**：
  1. **飞出一条链**。从训练集运行日里抽航班（有自己动力学的，与回放门同一组），从第 `N_look` 行的观测状态起飞。每架航班同时
     飞 `K` 个分支，每个分支都由先验自己采样说词，与自由生成完全相同（按列顺序、温度 1、相容规则屏蔽、跑道锁），执行器照着飞。
     每过 `S` 步（一段），比较各分支在段末那一行的位置与观测航迹在同一行的位置（三维距离，米；超出观测最后一行的，用最后一行），
     留下最近的一个分支，其余分支全部换成它（执行器的状态、先验读过的各行、说过的词一起复制），再飞下一段；执行器已经判定结束、
     但不是截获后越过入口的分支（入口前触地、动力学失败、超时），只要还有别的分支在飞，就不留；一直飞到执行器判定
     结束或超时（与自由生成相同）。留下来的这一串分支就是这架航班的**链**。
     - **为什么按段挑、不像 CAT-K 那样按步挑**：CAT-K 的一个词就是 0.5 s 的一段运动，下一步的位置由它直接决定；我们的词是交给
       执行器的目标，一步（2 s）里不同的词造成的位置差只有米的量级（坡度、加速度才刚开始变——这是我的估算，没有量过）。按步挑
       "离观测最近"，几乎每一步都会挑一个把飞机往观测那边推一点的词，链上说的词会比先验自己说的多得多，先验读到的"已经说过的
       词"就不再像它自己会说的。按段挑，每个分支都是先验自己的样本，说词的频率是它自己的；分支只在有分支说了词的地方分开，20 s
       足够让航向、速度、高度词的效果显出来。
  2. **重新定目标**（`prior/relabel.py`；链上从第 `N_look` 行起的每一行 t）：目标仍是标注的词，但按**标注说词的那些行**对齐到链上，
     而不是逐行要求链上生效的词与标注相同：
     - 第一个预测步：第 `N_look` 行标注生效的六个词（与预训练相同，链在这一行就是观测的状态）；
     - 标注在第一个预测步之后没再说过的列，目标是"不变"（进近许可除外，见下）。链上第一步猜的词与标注不同，也不要求它改：这些词是先验开口之前已经
       生效的，标注器有一部分是看航迹后来怎么飞才读出来的（§11），链上的飞机没照它飞，它就无从推断；逐行要求改，只会教它一遍遍
       重说。（在试跑的 40 条链上，第一步的词与标注相同的：速度 23 %、高度 53 %、航向 68 %；逐行按"相同"要求，要求说词的步占
       速度列 57 %、下降角 49 %、高度 39 %，而标注本身每列只在 0.4–8.5 % 的步上换词。）
     - 标注在第 u 行（u 在第一个预测步之后）说了一个词：链从第 u − `W` 行起回应它才算数——链上生效的就是这个词；或者（航向、高度、
       下降角、速度，这几列的取值链可以自己挑）链在这一列从 u − `W` 行起说过任何词（链的第一步不算，那一步本来就说全部六列）；
       跑道和进近必须是同一个词。回应了，目标是"不变"（说早了不要求收回）；没回应，从 u 到 u + `W` 行目标是说标注的词；再晚，
       标注已说不出链此刻该说什么，这一列不计损失——**只有"许可进近"例外：标注里已经许可（包括交接时就已许可）而链还没许可，
       一直要求到链说出为止**（自由生成里最常见的失败就是迟迟不许可，读数文档 §5；内部选择集的自由生成里，标注在第一个预测步已
       许可的 2,256 句中，有 351 句先验第一步没说许可）；
     - 链比标注的句子长时，超出的行按句子最后一行（飞机最后就是按这些词落地的）；
     - 执行器不会照做的词，目标是"不变"：已许可或已截获后的跑道、已许可后的"未许可"（只有复飞才撤销许可）、已截获后的航向（它
       沿航道飞）——标注器在这些状态下也从不写这些词（内部选择集 6,842 句：截获后的航向词 0 个，从许可退回未许可 0 次）；
     - 这样得到的一步，如果在链上飞机当时的高度上违反相容规则（与解码屏蔽用同一段代码），这一步违反的那一组列（跑道与进近，或
       高度与下降角）不计损失，并计数。
     - 试跑（2 轮 × 40 条链）：要求说词的步占各列 1.6–5.8 %（进近 5.3 %，其中 4.3 % 是一直要求的许可），不计损失的列：下降角
       18 %、速度 12 %、高度 8 %；链上每步的负对数似然 1.79（预训练在内部选择集上 0.28）。
  3. **训练**。从第 1 步选中的模型（`v3_step1_20260924/full_s1337`）开始；只用链上的数据，不再混 teacher forcing 的数据（CAT-K 的两段
     做法：先行为克隆，再纯闭环微调）；每一轮在到这一轮为止所有轮的链上过一遍（DAgger 的累积做法）。模型的输入就是先验在链上
     读过的那些行，损失与预训练相同（§7）。
- **选哪一轮**：每轮之后在内部选择集上做自由生成（每机场 200 架 × 2 句，每轮同一批航班、同一个种子），落地率最高的一轮胜出；与最高
  的差不超过 1.5 个百分点（2,000 句上二项分布约两个标准差）算平手，取更早的一轮。第 0 轮是微调前的模型，同样读一遍。同时列出
  teacher forcing 下选择集每步的负对数似然、落地跑道与观测一致的比例、落地时刻与观测之差、每架每列说的词数：落地率上去了而这几项
  明显变差（例如学成只会直进），读数里写明，交用户看。
- **验证集**：定下的那一轮只读一次：自由生成（每机场 400 架 × 4 句，与微调前的那次验证集读数同一批航班、同一个种子）和 teacher
  forcing 的负对数似然。
- **门**：与 §9.1 相同，先只记录。
- **测试集运行日**：一架都不读。链只在训练集运行日上飞；落地情况去掉封存的测试日（与预训练相同）。
- **产物**：`outputs/POOLED/prior/<名字>/`，只写一次：`config.json`（起点模型、执行器规格、句子产物、各项取值、git 状态），每一轮一个
  目录（这一轮的链 `chains.npz`：航班、各行位置、说过的词、目标、不计损失的列；`checkpoint.pt`；选择集读数 `readout.json`），
  `history.json`，`choice.json`。在没有未提交改动的工作树上跑。

### 9.3 按落地强化（单机，接在 §9.2 之后；用户 2026-09-25 同意这个顺序）

- **为什么**：§9.2 教先验在自己飞出的状态上"像管制员那样说"，目标来自标注，不直接看飞机落没落地。这一步反过来：让先验
  自由地说，只看结局——落地了，就提高这句话里各个词的概率；没落地，就降低。这正对准"说到飞机落地"这个目标，也不需要回答
  "链偏开之后标注此刻该说什么"。信号是现成的：验证集自由生成里，每架航班说 4 句，591 架（30 %）是有的落地、有的没落地
  （读数文档 §5.2）。
- **起点**：§9.2 选定的那一轮（若选定的是第 0 轮，就是第 1 步的模型）。
- **每一轮**：
  ...
  2. **奖励**：落地（执行器判定的 `landed`）并且落在"机场此刻的落地方向"上的跑道，得 1；其余结局（超时、偏出跑道、未截获、
     ...
     任何跑道都算。不要求落在观测的那条跑道上——那是 §9.2 和数据项的事。
...
- **选哪一轮**：每轮之后在内部选择集上自由生成（与 §9.2 同一批航班、同一个种子），先排除"护栏"变差的轮：落地在观测跑道上的
...
- **验证集**：选定的那一轮只读一次，与 §9.2 同样的读法（每机场 400 架 × 4 句，同一批航班、同一个种子），并与 §9.2 的验证集读数
  并排。
- **门**：先只记录（与 §9.1、§9.2 相同）。
- **测试集运行日**：一架都不读。
...
| 闭环微调：每段步数 `S` | 10 步（20 s） | 建议 | 航向词转 30° 约 10 s；速度、高度词的效果 20 s 才看得出。段越长，链在一段之内偏得越远 |
| 每架的分支数 `K` | 4 | 建议 | 计算量：自由生成每句约 0.1 s，一轮 4,000 架约半小时。CAT-K 的 K 是每步的（32 最好），这里是每段的 |
| 分支之间比什么 | 段末那一行的三维位置与观测之差（米） | 建议 | CAT-K 比下一步的状态；观测结束之后的行用观测的最后一行 |
| 回应的时间窗 `W` | 5 行（10 s），标注说词的前后各 `W` 行 | 建议 | 早于标注 10 s 以内的回应算数、不要求收回；没回应的在标注之后 10 s 内要求说，再晚不计损失（许可进近除外） |
| 每轮航班、轮数 | 每机场 800 架（每轮 4,000 架），8 轮；每轮换种子抽，轮与轮之间可能重复，读数写明重复了多少 | 建议 | 共约 32,000 条链 |
| 微调的优化器 | AdamW，学习率 1e-4（预训练的 1/3），前 100 步预热，权重衰减 0.01，梯度裁剪 1.0，每批约 16,000 步 | 建议 | 每轮在所有已有的链上过一遍，8 轮合计约 1,300 个优化步，约为预训练（约 11,000 步）的 12 % |
| 选哪一轮 | 选择集自由生成落地率最高；差 1.5 个百分点以内取更早的一轮 | 建议 | §9.2 |
| 按落地强化：每架采几句 `K` | 8 | 建议 | 验证集上 4 句时 30 % 的航班有对比；8 句对比更多。每轮 2,000 架 × 8 句，计算量与 §9.2 的一轮相当（约半小时） |
| 每轮航班、轮数 | 每机场 400 架（每轮 2,000 架），8 轮；每轮换种子抽 | 建议 | |
| 奖励 | 落地且在机场此刻的落地方向上 = 1，其余 = 0 | 建议 | §9.3；"此刻的落地方向"：前 30 分钟有落地的跑道，航向差 < 90° 的都算，没落地时任何跑道都算 |
| 优势 | 奖励 − 同一架航班 `K` 句的平均，不除以标准差 | 建议 | 除以标准差会放大只有一句落地（或只有一句没落地）的航班；先不除，读数里报两种航班各占多少 |
| 拉回项 β、数据项 λ | β = 0.04，λ = 1 | 建议 | β 取 GRPO 原论文（DeepSeekMath）的值——凭记忆，还没在本仓库的文献里核对；λ = 1 让数据项与预训练同一量级 |
| 优化器 | AdamW，学习率 1e-5，前 20 步预热，权重衰减 0.01，梯度裁剪 1.0；每批约 16,000 步（奖励项和数据项各占一半） | 建议 | 学习率是 §9.2 的 1/10：奖励项的梯度方差大 |
| 护栏 | 落地在观测跑道上的比例降 ≤ 2 个百分点，每架航向词数增 ≤ 20 % | 建议 | 防止学成"绕来绕去才落地"或乱挑跑道 |
