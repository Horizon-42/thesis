...
| 项 | 状态 |
|---|---|
| 设计 | 用户 2026-09-24 已确认全部取值（§10），只有两项还要等：运动与方向这两块输入给不给，等第二版训练的比较结果；机型怎么加（§4.5）是新写的，待确认 |
| 代码 | 还没有按本设计实现。现有的包 `prior/` 是上一版（候选跑道 token、跑道打分头、几种输入变体），在分支 `dev-prior-v2`（`e7f0cf66`、`8f5af0a3`，未合并），本设计在它的基础上改 |
| 已有的训练 | 两次，结果见读数文档：`outputs/POOLED/prior/v1_20260924/`，`outputs/POOLED/prior/v2_20260924/`（比较几种输入，队列还在跑）。这两次都按航班划分数据，第 0 步给的条件也和本设计不同，所以它们的数字不能当本设计的对照基线 |
| 下一步 | §9 的第 0 步 |
...
1. **输入只包含"说这一步指令之前，管制员能知道的东西"**：本机过去的位置、机场此前的落地、其他飞机此刻和之前的位置以及
   已经对它们说过的指令、本机的机型。每一项都写明来源和时刻，时刻都不晚于这一步。
...
- **场景**：同一机场、同一时刻的一组飞机，按绝对时间对齐，2 s 一步。某架飞机某一行的绝对时间 = 它进入 25 km 范围的时刻
  （到达清单里的 `entry_time_utc`）+ 这一行在它句子里的时间。据此建一个**场景索引**：每一步有哪些飞机在场。
...
- **运行日**：`data/runway_context.operational_day` 给出的日期，即 UTC 时间减 9 h 后的日期（在夜里航班最少的时候切开一天）。
  harvest 现在覆盖 90 个运行日（2026-04-30 到 09-21）。
- **按运行日划分**：所有机场使用同一批日期，用日期字符串的哈希、种子 1337 来分：测试集 15 % 的运行日，验证集 15 %，训练集
  70 %；训练集的运行日里再取 1/7 作为"内部选择集"，专门用来比较不同的模型变体。验证集用于提前停止训练和最后读一次结果。
- **测试集的运行日完全封存**：这些天的全部航班（不论它们原来按航班被分到哪一份），在两层模型这条研究线上都不打开——不读
  信号、不标注、不进入落地统计，也不进入场景。其余日子的航班全部可用，包括原来按航班被分进测试集的那些。
- **原来按航班抽的测试集保留给 ts 的其他模型**。需要跨模型比较时（例如"先验 + 执行器"对"控制路径模型"），只用双方都没
  见过的航班：落在新测试集运行日里、并且原来也在测试集里的那些航班。
...
- **运动**只从过去的位置计算：用上一行到这一行的位移除以 2 s，得到地速（除以 100 m/s）、运动方向（sin、cos）、垂直速率
  （除以 10 m/s）。不用信号里的航迹、地速、垂直速率，因为那些是用前后各 7.5 s 的数据拟合出来的，含有未来的信息。这一块
  输入最后给不给，按第二版训练对几种输入变体的比较结果定（§10）。
...
- **相对位置**：每一步都对本机场所有候选跑道计算（用 `instructions.airport.relative_to_runway`，与标注器是同一个函数）：沿
  ...
  另有一项"方向差"（过去的运动方向 − 跑道航向，sin、cos），给不给同样按第二版的比较结果定。
- **机场层面的落地情况**：在 t 之前的 `T_cfg` 分钟里，同一机场有多少架飞机落在这条跑道上；离上一次有飞机落在这条跑道上
  过去了多久（如果没有，记为"无"并置一个标志位）。落地记录来自 harvest 的到达清单（每架的落地跑道和落地时刻），只取
  落地时刻早于 t 的，并排除测试集运行日的航班。
...
- §9 第 1 步时为空（用掩码去掉）。从第 3 步起，对第 t 步场上的每一架其他飞机，输入它的位置与运动、已经对它说过的指令（编码
  方式与本机相同；背景飞机没有指令，记为"无"）、机型（§4.5），以及它与本机之间关系的一组特征（下称**边特征**）：
  - 相对位置：E、N、高度之差；
  - 沿本机生效跑道的航道方向，两者离入口的距离之差：谁在前、相隔多远；
  - 由过去位置算出的接近速度；
  - 两者生效的是不是同一条跑道，或者一对平行跑道；
  - 按规章，这两架飞机前后排队时至少要隔多远（§4.5）。

### 4.5 机型与其他背景

- **机型**：管制员从飞行计划里知道每架飞机的机型，属于那一刻就知道的信息。机型不用编号嵌入（与跑道同理：编号嵌入会让模型
  记住个别机型，遇到少见或没见过的机型就不会用），而是换成几个有物理或规章含义的量：
  - **尾流间隔类别**（CWT 类别，`inference/runway_schedule.wake_category`，出自 FAA JO 7360.1K 附录 A）：决定前后两架至少要隔
    多远；
  - **公布的进近速度**及其上下限（`aircraft/reference_speeds`，来自 FAA 的飞机特性数据库，执行器"未指定速度"时也用它）：
    直接影响速度指令；
  - **最大着陆重量**（同一张表）：代表飞机的大小。

  没有识别出机型、或者表里查不到的，这几项记为"无"并置标志位。这些量经过一个小网络，加到本机每一步的输入里；其他飞机的
  机型用同一个网络编码。在场景里，前后两架的尾流类别再换算成"按规章至少要隔多远"（`runway_schedule.Separation`，每个数都
  引到 FAA 原文条款），作为一项边特征（§4.4）。
- **什么时候加**：在 §9 第 1 步里作为一个变体，与不加机型的版本在内部选择集上比较，按比较结果决定保留与否。第 0 步先量机型
  的覆盖情况：多少航班识别不出机型，多少机型在两张表里查不到。
- **机场**：一个机场嵌入。
...
- 每架飞机、每一步的输入 = 本机位置与运动的投影 + 机型的投影 + 各列生效指令的嵌入 + 生效跑道的向量 + 所有候选跑道向量之和
  的投影 + 机场嵌入 + 时间位置嵌入。
- 每一层依次是：**沿时间的因果注意力**（每架飞机只看自己之前的步）→ **同一步里各架飞机之间的注意力**（边特征经过一个小网络，
  变成每个注意力头的偏置）→ 前馈层。飞机之间带边特征的注意力，就是在一张全连接图上的图注意力；一个场景只有几架飞机（§11），
...
- **后训练**（闭环，§9 第 5 步）：执行器同时飞场景里所有飞机。按"不失去间隔"（FAA JO 7110.65 的雷达间隔和尾流间隔，原文在
  `docs/literature/arrival_separation/`）、"能落地"、"不偏离数据太远"来筛选样本或给奖励；生成时再用间隔做硬性屏蔽。具体用
  闭环监督微调还是强化学习，等第 1 步的结果出来后再定，相关文献在 `docs/literature/multi_agent_interaction/`。
...
- **交互**（从 §9 第 2 步起）：每一步标出"同一条跑道前方 10 km 内有没有前一架飞机（下称**前机**）"和"场上是否有 ≥ 3 架飞机"；
  速度指令、航向指令的负对数似然和换词时刻，按"有无前机""忙闲"分开读。
...
0. **准备**：在训练集的运行日上量这几项：第 `N_look` 行时已建立在五边上的航班占多少；t 之前 `T_cfg` 分钟内有落地记录的步占多少
   ...
   样本量）；每一步场上有几架要说指令的飞机、几架背景飞机，有前机的步占多少；串成段后段的长度；机型的覆盖情况（§4.5）。然后
   用新训练集重新测量词表规格，按新划分重新生成句子产物。
1. **单机**：按 §4–§7 训练，场景里只放本机；机型作为一个变体比较（§4.5）。通过标准：每一列都赢两个基线；在"第一个预测步时
...
| 运动、方向差这两块输入 | 按第二版的比较结果（V1 / V1d / V2 / V2d 选中哪个） | 等第二版读数 | 读数文档 §2 |
| 机型 | 尾流类别、公布进近速度、最大着陆重量，作为第 1 步的一个变体比较 | 待确认（新增） | §4.5 |
