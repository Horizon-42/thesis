...
- **第二版计划**（用户 2026-09-24 同意写入，先验设计 §8）：输入去掉拟合的速度类量（含 7.5 s 未来信息，repo `docs/open-items.md`）、第 0 步已知的列当条件（逐列划分待用户确认）、每条候选跑道的相对量、候选跑道 token；在 train 内部选择集上比变体；在下一代句子产物上做。
