# 两层模型：阶段记录（更新于 2026-09-25 10:50 UTC）
...
4. **单机自由生成（进行中，2026-09-25）**：用户已合并 `dev-prior-v3`（`d1950bbc`）。设计写在先验设计 §9.1。代码在 `dev-prior-v3`：
   ...
   日志 `select.log`，10:46 UTC 起）→ `select_400x4/`（内部选择集每机场 400 架 × 4 句 + 标注的词作对照）。试跑（50 架 × 2 句）：先验说词落地
   88 %（标注的词 100 %）；模型放在被禁组合上的概率很小（下降角列每步 0.04 %）。**门等用户定**（§9.1）。跑完：读数写进读数文档 §5 →
   等用户定门 → 验证集读一次。
