...
| 下一代产物 | **进行中**：信号已重建为 `outputs/POOLED/instruction_language/v3_20260924/`（`ts-instruction-signals-v2`，train 50,649 / val 10,625，只有信号和候选跑道）；词表第三版修订（词表设计 §10）在分支 `dev-vocab-v3`（工作树 `.claude/worktrees/vocab-v3`）：航向词 §10.1 已量完，待用户定 | |
| 6 后训练 | 未开始 | |
| 7 约束 / 8 多机 | 未开始 | |
...
1. **用户看前端上的执行器结果，定航向词的做法**（§2.5 的 A–D）。
2. **下一代产物**（不管选哪个都要做：信号 v2）：先改执行器源码指纹；按选定的航向词改词表 / 标注器 / 执行器 / 判定，
   重导句子产物、训练样本 v6 集合，重量执行器规格（train），回放门（val），重训先验，用 §5 的 runner 重发。
