# 两层模型：阶段记录（更新于 2026-09-25 15:25 UTC）
...
6. **按落地强化**（设计 §9.3，用户 2026-09-25 同意"先跑 CAT-K，再做这一步"）：从 CAT-K 选定的一轮起，自由生成每架 8 句，落地且落在
   ...
   CAT-K 读数（前 4 轮）：选择集落地 90.0 → 91.3 → 72.2 → 76.5 → 85.0 %，被引导的航班第 2 轮只有 37.5 %；teacher forcing 负对数似然
   0.284 → 0.926。原因：链离观测越飞越远（100 s 偏 460 m、220 s 偏 1 km），按时间对齐的标注（尤其"一直要求许可"）在链上不成立。
7. 之后按门走 §9 第 2 步（M1：交互看不看得到）。
