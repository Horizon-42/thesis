# 两层模型：阶段记录（更新于 2026-09-25 19:50 UTC）
...
| 6 后训练 / 7 约束 / 8 多机 | 闭环监督微调（CAT-K）不采用；**按落地强化在跑**（最后一轮，选择集 90.0 → 95.1–96.5 %）；**第二阶段设计已写**（RNAV 高度下限与 FAF 以内下滑道下沿作屏蔽 + 起始状态扩充，只靠终点奖励和拉回基础模型），取值等用户确认；多机按场景做已写进先验设计（§3） | `docs/2026-09-25_post_training_design.zh.md`（后训练设计）；先验设计 §6、§9 |
...
8. **后训练第二阶段**（用户 2026-09-25 提出，设计写在后训练设计 §3–§5）：RNAV 进近编码的高度下限（FAF 以外）和下滑道下沿（FAF 以内，
   ...
   ±150 m、速度 ±5 % 造新起点；损失只有终点奖励 + 拉回第 1 步模型，没有数据项。**等用户确认 §8 的取值**，然后先做第 0 步：在训练集
   标注数据上验屏蔽（三种划法，取通过的最严的一种），不训练。代码走工作树 + opus 审查。
