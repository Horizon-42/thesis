# 两层模型：阶段记录（更新于 2026-09-25 01:55 UTC）
...
2. ~~第三版第 1 步训练~~：2026-09-25 01:41 UTC 跑完，选中 full，门过了（读数文档 §4）。请用户合并 `dev-prior-v3`。
3. 导出与前端的审查意见（opus 只读审查已完成）改在 `dev-prior-v3` 上——队列跑完之后再改（`prior_select` 要求代码与训练时一致）：
