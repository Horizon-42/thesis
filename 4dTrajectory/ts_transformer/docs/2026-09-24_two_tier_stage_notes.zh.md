...
| 6 后训练 / 7 约束 / 8 多机 | 闭环监督微调（CAT-K）不采用；**按落地强化完成、采用**（选中第 1 轮，验证集 90.2 → 97.2 %，被引导 83.3 → 93.9 %，读数文档 §7）；**第二阶段设计已定**（RNAV 高度下限与 FAF 以内下滑道下沿作屏蔽 + 起始状态扩充，只靠终点奖励和拉回 base 模型；取值用户 2026-09-25 全部确认），正在写第 0 步（在标注数据上验屏蔽）；多机按场景做已写进先验设计（§3） | `docs/2026-09-25_post_training_design.zh.md`（后训练设计）；先验设计 §6、§9 |
...
- 还在的工作树（2026-09-25 清理后，用户同意）：
  - `.claude/worktrees/post-train`（`dev-post-train`，基于 `dev-two-tier` `a71b89d5`）——后训练第二阶段的代码（屏蔽、第 0 步）；
