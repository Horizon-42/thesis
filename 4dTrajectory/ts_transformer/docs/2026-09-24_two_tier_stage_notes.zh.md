...
8. **后训练第二阶段**（用户 2026-09-25 提出，设计写在后训练设计 §3–§5）：RNAV 进近编码的高度下限（FAF 以外）和下滑道下沿（FAF 以内，
   ...
   **用户 2026-09-25 决定只留下滑道下沿**（重飞通过线 3 %）。代码 `dev-post-train` `3cb1fe72`（两轮 opus 审查；ts 全套 1,434 通过；
   ...
   冒烟跑通；未合并）。**正式训练 2026-09-26 开始**（用户同意；时限按源航班 1.5 倍，已定）：`outputs/POOLED/prior/v3_augmented_20260926/`，从工作树 `post-train` `00c9cc0e` 跑，`run.pid` 看进程；训练完自动读一次验证集（选中的一轮与第一阶段模型并排，真实 / 扩充起点，开着屏蔽）。
   **代码健康的遗留问题里会影响训练的**（用户 2026-09-25）：在工作树 `training-followups`（分支 `dev-training-followups`，`dcbf6388`，经 opus
   审查，ts 全套 1,421 通过）改完，**第二阶段之后与重建一起合并**（用户：第二阶段之后再重建）；审查另记三条（`docs/code-health-followups.md`
   最后三条，其中两条要在重建前定：原生失速余量范围是否按新质量重判、OpenAP 原生机型是否也改用公布的 MALW）；多翻襟型别的减襟翼 Vref（#21，占训练集 25 % 的航班）由 opus 子代理找公开的原始出处。
