# 两层模型：阶段记录（更新于 2026-09-25 03:30 UTC）
...
4. **单机自由生成（进行中，2026-09-25）**：用户已合并 `dev-prior-v3`（`d1950bbc`）。设计写在先验设计 §9.1（`9fdc0e85`、`9b6ffdb0`）。代码在
   `dev-prior-v3`：执行器改成可逐步推进（`9ebf9e15`：`Executor`、`Spoken`、`flight_inputs(anchor=)`；100 架回放改前改后逐字节相同，
   但执行器源码指纹变了，要重写一份执行器规格 v7，内容与 v5/v6 相同）；未提交：`prior/generate.py`（`Speaker`：逐列采样、逐行输入、
   词表相容规则作屏蔽）、`instructions/grammar.py`（调用标注器自己的相容检查，不在标注器指纹里）、`experiments/prior_free_generation.py`
   （R17）、测试；ts 全套 1,408 通过；opus 审查中。选择集 50 架 × 2 句的试跑：先验说词落地 88 %（标注的词 100 %），超时 9 %、偏出跑道 3 %，
   先验说的航向词多（每架 27 对 16）。**门等用户定**（§9.1）：先验说词的落地率是否要求每格 ≥ 95 %，还是只记录、留给后训练。
   下一步：审查改完 → 提交 → 写执行器规格 v7 → 选择集每机场 400 架 × 4 句正式跑 → 读数写进读数文档 §5 → 验证集读一次。
5. 之后按门走 §9 第 2 步（M1：交互看不看得到）。
