...
| 下一代产物 | **必须重做**：`9fb1b137`（另一会话）把信号升到 `ts-instruction-signals-v2`，现代码拒绝 `v2_20260924` 的信号；等词表第三版修订（词表设计 §10，航向词 §10.1：H1 / H2 / H3，待在 train 上用真执行器量后由用户定） | |
| 6 后训练 | 未开始 | |
| 7 约束 / 8 多机 | 未开始 | |
...
**已并入词表设计 §10.1**（第三版修订）：那里有三种读法 H1（现在）/ H2（= 下面的 E）/ H3（用户的逐行标注再合并）、train 上的粗估和要量的东西；下面是最初的诊断。
