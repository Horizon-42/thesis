...
**当前状态（2026-09-06）**：Phase 0、P0、P1.a/b/c 完成并提交。**P1.c 两个门都过**（结果 `2026-09-06_closure_p1c_results.zh.md`）：closure 解码器只看本机就好于 simple-v3（全体 ADE 996 vs 1333，雷达引导 2197 vs 2858），给真值 (d_join, T) 时雷达引导 1235 m / chamfer 492（Phase 0 的控制头臂 2011 / 847），家族上界 455 m。**closure 成为输出侧主线**。下一步：P1.d 可飞性（closure 整条全可飞 22 %，control 基线 0.1 %，观测 98 %；逐样本 99.8 % = 观测；违反是 CSC 交界的 bank 跳变与节点处的 thrust 跳变）→ P2 数据平面（验收口径改为 C_pred → C_truth_intent 的 960 m）。没有在跑的 campaign。
...
| P1.a 几何族 oracle：`closure_geometry.py`（F0 规则模板从诊断脚本搬入；F2 沿锚点航向直飞 + Dubins；F3 经由点 Dubins；拟合目标 = 弧长对齐水平误差；标签规范化：d_join = 首次稳定进入航向道处，F3 经由点 = 拟合路径上最早能复现的位姿，做不到的标 `canonical=False`）、`docs/p1_closure_oracle.py geometry` | 完成 | `d3be105`；`closure_p1_20260905/oracle_geometry.txt`、`closure_labels.json`（逐航班各族参数与残差 = P1.c 标签）；测试 `tests/test_closure_geometry.py`（19 项）；两轮 opus review | 雷达引导 497 架（真值 = supervision 行，真值时序）：F0 chamfer 685 / Fréchet 3381 / ADE 1688；F1（拟合 d_join）384 / 1860 / 1301；F2 699 / 2578 / 794；**F3 180 / 1179 / 510，过门**（chamfer < 500 ∧ Fréchet < 1500 ∧ ADE < 1000）；F3 超 1 km 残差 1.4 %，标签不可规范化 4.0 %，经由点标签两起点差中位 3 m / p90 13 m；F2 不包含 F1 的 trombone（37 % 更差）；直线进近 F0 92 → 拟合 38 m |
| P1.b 速度 / 高度剖面 oracle：`closure_profile.py`（沿进度分数的分段线性慢度节点，时间对节点线性、有界最小二乘；高度节点）、`p1_closure_oracle.py speed` | 完成 | 同上；`oracle_profile.txt`、`profile_labels.json`（K=8 标签）；测试 `tests/test_closure_profile.py`（5 项） | 雷达引导，真值路径上：朴素 1308（复现 Phase 0）→ 朴素形状 × 真值时长 622 → 慢度节点 K=2 166 / K=4 97 / K=8 61 m；高度：闭合剖面 120 → 节点 K=4 31 m（末节点钉在阈值 0）；两者合起来 K=4 111 m、K=8 68 m；**K=2 已过 400 m 门**；标签同时存 K=4 与 K=8 及各自残差 |
| P1.c 解码器 `prediction_output="closure"`：c1 全体航班标签文件 → c2 输出路径接入（config / 模型 / 数据上下文 / 损失 / forecast / 导出 / 命名）→ c3 campaign（C_oracle / C_truth_intent / C_pred 对 A、O_join_duration） | **完成（2026-09-06），两个门都过** | c1：`p1_closure_oracle.py labels` → `4dTrajectory/outputs/KRDU/closure_labels/closure_labels_20260905.json`（9720 架 = 名册 14435 减 openap-direct 过滤 4704 与短航迹 11；22 MB）；c2：`closure_output.py`（`ClosurePrediction` / `ClosureOutputModel` / `closure_loss_components` / `fit_labels` / `reconstruct` / `replay_batch`）+ `config.py`（`PREDICTION_CLOSURE`、`closure_*` 六字段与校验）、`train.py`（契约 `closure-v1-slowness4-height4`、损失注册、回放分支）、`models.py`、`dataset.py`（上下文行）、`batching.py`、`forecast.py`（`closure_forecast` / `forecast_closure_from_labels`）、`run_naming.py`（`closed-form` / `closure-v1`）、`anchor_eligibility.py`、`__main__.py`（`--closure-labels`、`predict --closure-from-labels`）；`tests/test_closure_output.py`（6 项，含训练→检查点→预测→导出→评估整链）；c3：`docs/experiments/closure_p1c_arms.json`（C_pred、C_truth_intent）+ `closure_p1c_oracle_arms.json`（C_oracle，在 C_pred 有检查点后跑）| c1：标签有效 96.8 %，雷达引导（3609 架）F3 误差中位 209 m；c3（`closure_p1c_20260905`，代码 `8d03fe1`…`560d5b8`，结果 `2026-09-06_closure_p1c_results.zh.md`）：雷达引导 ADE / chamfer——A 2858 / 942，O_join_duration 2011 / 847，**C_pred 2197 / 722，C_truth_intent 1235 / 492，C_oracle 455 / 180**；直线进近 A 469 → C_pred 310；解码器规则：经由点在锚点一个转弯半径内改画 CSC（第一次 C_pred 直线进近 6260 → 310）；可飞性：整条全可飞 22 %（control 基线 0.1 %、观测 98 %），逐样本 99.8 %；违反 = bank / thrust 跳变（P1.d） |
...
- **P1.a 几何族 oracle（numpy，无训练；≈1–2 天）。** 模板构造搬成包内单一来源 `closure_geometry.py`（诊断脚本改为导入）。在 KRDU 验证集（Phase 0 的 1404 架，同 config / anchor，`_series_for`）上按分层量每个几何族在**真值时序**（真值弧长分数处的时间，与 Phase 0 的"template + truth timing by arc length"同口径）下的 ADE，以及 chamfer / Fréchet（P0 的 `geometric_metrics`，真值 = supervision 行）：F0 = 现模板（真值 d_join，规则选 trombone / Dubins，半径按锚点速度 25° 坡度）= 1688 m；F1 = F0 但 d_join 按 chamfer 最优拟合（量 k=0.5 真值门的 d_join 对模板是否最优）；F2 = trombone 加第三个决策量（三边位置 d_base ≥ d_join + r，或下风延伸）；F3 = 经由点族（锚 → 1 个自由经由点 → Dubins 到 (d_join, 0) → 五边），经由点按 chamfer 拟合。每族逐航班记录拟合参数与残差（成为 P1.c 的标签与后备判据）。**门**：某族在雷达引导层 chamfer 中位 < 500 m 且真值时序 ADE < 1.0 km（给速度剖面留 0.5 km）；都不达 → 家族再加一个决策量，不进场景。
  > **结果（2026-09-05，`closure_p1_20260905/oracle_geometry.txt`，两轮 opus review 后）**：雷达引导 497 架，F0 = 1688 m（复现 Phase 0）；F1 只拟合 d_join → 1301（拟合出的汇入比 k=0.5 真值门远 1.5 km：门开在转弯完成之后）；F2（沿锚点航向直飞再 Dubins）794，但它**不包含** trombone（下风沿跑道轴而非锚点航向，37 % 航班比 F1 差）；**F3 经由点 Dubins：chamfer 中位 180、Fréchet 1179、真值时序 ADE 510 m，过门**；超 1 km 残差仅 1.4 %（P1.d 后备候选）。标签：拟合目标改为弧长对齐水平误差（chamfer 不保序会放过绕路）；d_join 规范化为路径首次稳定进入航向道处（否则沿航向道直飞的一段让 d_join 不可辨识）；F3 经由点规范化为拟合路径上最早能复现它的位姿（纯 CSC 航班 = 锚点本身，trombone = 三边转弯点），两个最优起点的经由点差中位 3 m / p90 13 m，规范化代价中位 1 m；拟合成环的 4 % 航班做不到，保留拟合经由点并标 `canonical=False`（不可辨识，P1.c 训练时按后备处理）。经由点落在五边前路径的 0.62 处（p25–p75 0.58–0.64）。
- **P1.b 速度剖面 oracle（≈1 天）。** 在真值路径上（几何误差归零）量各参数化的 ADE：朴素剖面（锚点速度线性到 70 m/s）= 1308 m；朴素剖面按真值 T 缩放；沿路程分段线性地速 K 个节点（K = 4 / 8 / 16）按真值时间最小二乘拟合；垂直剖面：到汇入点线性下降 + 下滑道，对 K 节点高度剖面。**门**：某参数化 < 400 m，K 尽量小。
  > **结果（2026-09-05，`oracle_profile.txt`）**：参数化用**慢度**（1/地速）沿进度分数的分段线性节点——时间对节点线性，拟合是有界线性最小二乘（`closure_profile.py`）。雷达引导：朴素 1308（复现 Phase 0）→ 朴素形状 × 真值时长 622 → K=2 166 / K=4 97 / K=8 61 m；高度：闭合剖面（线性到汇入点的下滑道高度再下滑道）120 → 高度节点 K=4 31 m（末节点钉在阈值 0——不钉时 28 % 航班落在阈值下方最多 33 m）；两者合起来 K=4 111 m、K=8 68 m。**K=2 已过门**；杠杆在时长（朴素→缩放减 700 m）。
- **P1.c 解码器 `prediction_output="closure"`（≈1 周；下一步）。** 网络回归低维决策向量——按 P1.a/b 定为：d_join（航向道入口）、F3 经由位姿**用跑道坐标系** (d_via, xt_via, Δψ = ψ_via − ψ_course，以 cos/sin 回归、atan2 还原——图坐标下四条跑道是四个标签分布、航向有分支切割；标签里两种都有：`via_e/via_n/via_heading` 与 `via_d/via_xt/via_heading_rel`)、慢度节点 K=4（**时长由其积分给出，不单独出时长头**）、高度节点 K=4（末节点钉在 0，不回归），约 14 个数；标签 = `closure_labels.json`（F3；`canonical=False`（4 %）与超残差的航班**不进回归损失**，按后备处理）与 `profile_labels.json`（K=4 与 K=8 各一套，带各自的 ADE 与时长误差；按 flight_key，与名册对齐，不进 `tracks/`）；损失 = 各参数按尺度的 L1 + 现有时长项；推理时闭式重建路径 + 剖面 → 稠密状态（速度由路径切向与地速给出）→ 现有记录契约。**先不做可微几何**（先证明标签回归够用），**不做 rollout**（几何族按 25° 坡度弧线，动力学可行性作为读数报告，不作约束）。臂：C_oracle（真值标签直接重建 = 家族上界）、C_truth_intent（真值 (d_join, T) 经 intent 通道输入，网络出其余参数）、C_pred（全部自己出）；对照 simple-v3 与 O_join_duration；读数用 P0 的两族指标。**门**（雷达引导，KRDU）：C_truth_intent ADE < 1.5 km、chamfer < 500 m；C_pred 不差于 simple-v3 且直线进近不退；否则在 P1.a 的家族上加决策量，或回到控制头作后备。
  > **实施计划（2026-09-05，按代码勘察定）**，每步：代码 → opus review → 修 → 跑 → 记录：
  > - **c1 标签文件**：`p1_closure_oracle.py labels --airport KRDU` 对名册**全体**进场（训练 + 验证都要有标签；直线进近也拟合，不再走规则分支）做 F3 拟合 + K=4/8 剖面拟合，写 `4dTrajectory/outputs/<ICAO>/closure_labels/closure_labels_<date>.json`（按 `flight_scenarios.identity.flight_key`；每航班：d_join、经由 (e, n, ψ) 与 (d, xt, Δψ)、慢度 / 高度节点、残差、`canonical`、难度协变量）。标签只在损失里用，永不进输入。
  > - **c2 接入**（勘察到的接缝）：`config.py` 加 `PREDICTION_CLOSURE`、字段 `closure_labels_path`（必填）、`closure_slowness_knots=4`、`closure_height_knots=4`、三个损失权重；校验：只允许 `enu` 图、`absolute` 位置基准、`horizon_mode=normalized`（pred_len 只是数据集的目标网格）、`checkpoint_selection_metric=fixed-anchor-objective`（验证回放只认 state / control 两种预测类型，闭式路径不进训练回路）、拒绝随机训练锚（标签对固定锚）；`train.target_contract` 与 `anchor_eligibility._POLICIES` 加 closure 分支；`run_naming` 加 closure 的 dynamics（"closed-form"）与 loss 词汇。新模块 `closure_output.py`（顶层，架构测试允许）：`ClosurePrediction(decision[B,14], final_time_s[B])`、决策向量的编解码（d_join softplus ≥ 500 m、via_d / via_xt 线性、Δψ 以 (cos, sin) 回归、慢度 sigmoid 映到 [1/160, 1/40] s/m、高度 4 个自由节点 + 钉 0）、`ClosureOutputModel`（骨干 `encode_features` → MLP → 14；不继承 `control/` 的特征模型）、`closure_loss_components`（注册进 `PREDICTION_LOSS_HANDLERS`；四个固定分量：state := 几何项、final_time := 慢度项按标签路径长折成秒、kinematic := 高度项、terminal := 0；按 `closure_valid`（canonical 且残差 ≤ 阈值）加权，不进回归的航班权重 0）、`reconstruct(decision, anchor_state, psi) → Forecast`（numpy：`via_dubins` → 失败退到 `dubins_join(…, 0)` → 再退直线；高度 `height_from_knots`、时间 `times_from_slowness`、速度 = 切向 × 地速 + dh/dt，导出走 `controls is None` 分支不改契约）。数据侧按 `final_approach_arrays` 的上下文字典模式挂 `closure_decision` / `closure_valid`；`forecast_approaches` 加 closure 分支，`predict --closure-from-labels` 直接由标签重建（= C_oracle 臂，不训练）。
  > - **c3 campaign** `closure_p1c_20260905`（KRDU）：C_oracle（标签重建）、C_pred（closure，只看本机）、C_truth_intent（closure + `intent_conditioning=truth-join-duration`）；对照 `control_procedure_20260905/A_control_v3` 与 `scene_phase0_20260905/O_join_duration`；读数 `compare_constraint_arms.py`（两族指标）。
  > **结果（2026-09-06，`2026-09-06_closure_p1c_results.zh.md`）**：两个门都过。C_truth_intent 雷达引导 ADE 1235（门 1500）、chamfer 492（门 500）；C_pred 全体 996 vs 基线 1333，雷达引导 2197 vs 2858，直线进近 310 vs 469。同样的真值意图，闭式解码器比控制头好 40 %、chamfer 减半——输出侧是瓶颈的判断成立；剩下的雷达引导 2.2 km 里，本机 → 真值意图 960 m 是场景信息（P2/P3 的价值上限），真值意图 → 家族上界 780 m 是回归误差。训练每轮 2–2.5 s（无 rollout）。解码器规则：经由点在锚点一个转弯半径内不算决策（直线进近的标签把经由点放在锚点上，预测的小偏差经 Dubins 成整圈）。
- **P1.d 后备分支 + 可飞性。** 已量到（`flyability_report.json`）：closure 逐样本可飞 99.8 %（观测 99.8 %，control 基线 93.3 %），整条全可飞 22 %（观测 98 %，control 基线 0.1 %——它的违反是 5.9 万个 stall 样本）；closure 每条只有几个违反样本：bank（CSC 弧–直交界的曲率跳变）、thrust_over_max（慢度节点的加速度跳变）、少量 load factor / stall。做法：回旋线过渡或坡度率限制 + 剖面平滑，或事后 rollout 一致化，目标整条全可飞率接近观测；后备分支：标签无效的 3.2 % 航班与重建退化（`closureConstruction`）在读数里单列。几何族不适用的航班（Phase 0 模板构造计数：雷达引导 497 架里 trombone 305、Dubins 181、直线 2、已过汇入点 9；盘旋、转场）保留现有控制头；判据 = P1.a 的拟合残差（chamfer）超阈值时标 `closure_fallback`，读数里单列。
