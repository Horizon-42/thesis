...
| P1.c 解码器 `prediction_output="closure"`：c1 全体航班标签文件 → c2 输出路径接入（config / 模型 / 数据上下文 / 损失 / forecast / 导出 / 命名）→ c3 campaign（C_oracle / C_truth_intent / C_pred 对 A、O_join_duration） | **进行中：c1** | `p1_closure_oracle.py labels`、`closure_output.py`、`tests/test_closure_output.py`；campaign `closure_p1c_20260905` | 决策向量 14 个数（§五 P1.c）；家族上界（C_oracle）预计雷达引导 ADE ≈ 0.5–0.6 km；门见 §五 P1.c |
