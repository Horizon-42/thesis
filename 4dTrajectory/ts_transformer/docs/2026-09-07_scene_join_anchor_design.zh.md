...
| P1.b 速度 / 高度剖面 oracle：`closure_profile.py`（沿进度分数的分段线性慢度节点，时间对节点线性、有界最小二乘；高度节点）、`p1_closure_oracle.py speed` | 完成 | 同上；`oracle_profile.txt`、`profile_labels.json`（K=8 标签）；测试 `tests/test_closure_profile.py`（5 项） | 雷达引导，真值路径上：朴素 1308（复现 Phase 0）→ 朴素形状 × 真值时长 622 → 慢度节点 K=2 166 / K=4 97 / K=8 61 m；高度：闭合剖面 120 → 节点 K=4 30 m；两者合起来 K=4 110 m、K=8 68 m；**K=2 已过 400 m 门** |
...
- **P1.b 速度剖面 oracle（≈1 天）。** 在真值路径上（几何误差归零）量各参数化的 ADE：朴素剖面（锚点速度线性到 70 m/s）= 1308 m；朴素剖面按真值 T 缩放；沿路程分段线性地速 K 个节点（K = 4 / 8 / 16）按真值时间最小二乘拟合；垂直剖面：到汇入点线性下降 + 下滑道，对 K 节点高度剖面。**门**：某参数化 < 400 m，K 尽量小。
  > **结果（2026-09-05，`oracle_profile.txt`）**：参数化用**慢度**（1/地速）沿进度分数的分段线性节点——时间对节点线性，拟合是有界线性最小二乘（`closure_profile.py`）。雷达引导：朴素 1308（复现 Phase 0）→ 朴素形状 × 真值时长 622 → K=2 166 / K=4 97 / K=8 61 m；高度：闭合剖面（线性到汇入点的下滑道高度再下滑道）120 → 高度节点 K=4 30 m；两者合起来 K=4 110 m、K=8 68 m。**K=2 已过门**；杠杆在时长（朴素→缩放减 700 m）。
- **P1.c 解码器 `prediction_output="closure"`（≈1 周；下一步）。** 网络回归低维决策向量——按 P1.a/b 定为：d_join（航向道入口）、F3 经由位姿 (e, n, ψ)、慢度节点 K=4（时长由其积分给出）、高度节点 K=4，约 14 个数；标签 = `closure_labels.json`（F3，`canonical=False` 与超残差的航班按后备处理）与 `profile_labels.json`（按 flight_key，与名册对齐，不进 `tracks/`）；损失 = 各参数按尺度的 L1 + 现有时长项；推理时闭式重建路径 + 剖面 → 稠密状态（速度由路径切向与地速给出）→ 现有记录契约。**先不做可微几何**（先证明标签回归够用），**不做 rollout**（几何族按 25° 坡度弧线，动力学可行性作为读数报告，不作约束）。臂：C_oracle（真值标签直接重建 = 家族上界）、C_truth_intent（真值 (d_join, T) 经 intent 通道输入，网络出其余参数）、C_pred（全部自己出）；对照 simple-v3 与 O_join_duration；读数用 P0 的两族指标。**门**（雷达引导，KRDU）：C_truth_intent ADE < 1.5 km、chamfer < 500 m；C_pred 不差于 simple-v3 且直线进近不退；否则在 P1.a 的家族上加决策量，或回到控制头作后备。
