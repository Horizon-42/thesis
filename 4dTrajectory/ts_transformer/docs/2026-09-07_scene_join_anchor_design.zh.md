...
**当前状态（2026-09-05）**：Phase 0、P0 完成并提交（P0 代码 `6867512`、文档 `b34c25a`）。**用户已决定走 C**（先修输出侧，再 B 改决策变量做场景）；**P1 进行中：P1.a、P1.b 完成（代码 `d3be105`），下一步 P1.c 解码器**——见 §五 P1 的分步、结果与门。没有在跑的 campaign。磁盘只剩 4.0 GB——**下一个 campaign 前先清理**（`clean_pipeline_data.py --dry-run` 或删旧实验树）。
...
| P1.c 解码器 `prediction_output="closure"`：c1 全体航班标签文件 → c2 输出路径接入（config / 模型 / 数据上下文 / 损失 / forecast / 导出 / 命名）→ c3 campaign（C_oracle / C_truth_intent / C_pred 对 A、O_join_duration） | **进行中：c1 完成、c2 建成并过 review（修了重复接线、oracle 臂溯源、零覆盖、head 有界、损失尺度、schema v2、命名等 11 项）、c3 campaign 已启动（2026-09-06 00:0x，runner PID 481125；训练臂 C_pred → C_truth_intent，之后跑 oracle 臂文件）** | c1：`p1_closure_oracle.py labels` → `4dTrajectory/outputs/KRDU/closure_labels/closure_labels_20260905.json`（9720 架 = 名册 14435 减 openap-direct 过滤 4704 与短航迹 11；22 MB）；c2：`closure_output.py`（`ClosurePrediction` / `ClosureOutputModel` / `closure_loss_components` / `fit_labels` / `reconstruct` / `replay_batch`）+ `config.py`（`PREDICTION_CLOSURE`、`closure_*` 六字段与校验）、`train.py`（契约 `closure-v1-slowness4-height4`、损失注册、回放分支）、`models.py`、`dataset.py`（上下文行）、`batching.py`、`forecast.py`（`closure_forecast` / `forecast_closure_from_labels`）、`run_naming.py`（`closed-form` / `closure-v1`）、`anchor_eligibility.py`、`__main__.py`（`--closure-labels`、`predict --closure-from-labels`）；`tests/test_closure_output.py`（6 项，含训练→检查点→预测→导出→评估整链）；c3：`docs/experiments/closure_p1c_arms.json`（C_pred、C_truth_intent）+ `closure_p1c_oracle_arms.json`（C_oracle，在 C_pred 有检查点后跑）| c1：标签有效 96.8 %（canonical 97.9 %，残差 ≤ 1 km 98.1 %）；F3 弧长对齐误差中位：全部 7 m、雷达引导（3609 架）209 m；剖面 K=4 在真值路径上 ADE 均值 40 m（雷达引导 90）；c3 门见 §五 P1.c |
