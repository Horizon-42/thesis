...
| A/B anytime prediction + calibrated ETA | **design only** (`docs/2026-09-07_anytime_prediction_and_calibrated_eta_design.zh.md`) | the two post-L2 lines that need no new intent input: A0 re-anchor curve (fixed-anchor checkpoints + one `random_train_anchor` arm on the L1.b teacherless recipe), B1–B3 quantile duration head + split-conformal + `--cta-from-quantiles` (the first CTA arm that does not read the future); deliverable B4 = 80 % interval width vs remaining path, `s_freeze`; C = conditional diffusion over the 96 operating numbers trained on the L5.a fitted table, guided THROUGH the differentiable rollout (observed prefix / CTA / corridor), a REPLACEMENT for the CVAE if L2.e′ leaves z under ~1 nat, gated first by a retrieval ceiling on the same table (C0) |
