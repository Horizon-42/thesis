...
| L5.a fitted teacher | code done + reviewed (`dev-l5`); **the documented command does not start yet** | the fit itself is a GPU job and **has not run**: `docs/experiments/l5_fitted_teacher_arms.json` points at `4dTrajectory/outputs/KRDU/experiments/l5_fitted_teacher_20260907/basis_fit.json`, which **does not exist until** `run_ts_control_basis_oracle.py --checkpoint .../l1_lowdim_20260907/L1_native32/checkpoint.pt --out <that directory> --splits train,val` has been run (8255 flights, ~2–3 h). Until then both arms die at the dataset build, by design — the config carries the path, the dataset opens it. **First fix its provenance call** (verified 2026-09-07): that runner fingerprints the arrival manifests WITHOUT the pre-split lateral-pass roster, so on every v5 checkpoint it raises "checkpoint training data does not match the current arrival manifests" — 14 435 candidates against the checkpoint's 14 378 eligible. Reproduction + fix: repo `docs/code-health-followups.md` §19; the working shape is `run_ts_anytime_curve.current_provenance` |
| A0 / B0 anytime + calibrated ETA | code done (`dev-a0`): B0 measured, A0 replay queued | `docs/2026-09-07_anytime_prediction_and_calibrated_eta_design.zh.md` §〇 is the live status table (commands in §〇.1, B0's numbers in §〇.2). `run_ts_anytime_curve.py` = A0-fixed over a remaining-path anchor grid, strata fixed at L−1; `run_ts_eta_error_readout.py` = B0, pure CPU readout. KRDU val: vectored \|Δt\| p80 **65.8–72.5 s** against straight-in **12.0–20.3 s** on native32 / L2.d / closure — per-stratum ETA calibration is necessary, and B's 120 s veto has about one doubling of margin |
