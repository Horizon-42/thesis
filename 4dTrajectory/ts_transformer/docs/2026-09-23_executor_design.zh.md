...
```text
e    = wrap180(θ − χ)                          离目标还差多少度，取较短方向，带符号
...
**为什么这两个量要标定，而不是直接从数据里量**：滚进、回正都在 5–10 s 内完成，比观测的 15 s 速度拟合窗口短，
数据里看到的是被抹平后的样子。直接量出的"滚转速度 1.7°/s、回正时间常数 8.6 s"，主要是那个窗口的效果。所以在上面
三条约束围出的范围里取候选值：`τ_ψ ∈ {2, 3, 4} s`，`p ∈ {2, 3, 5} °/s` 中满足约束 3 的组合。然后按 §9 标定。
...
3. **标定**：每组参数在一个小网格上找使差别最小的值，候选值事先写进执行器规格：
   - `τ_ψ ∈ {2, 3, 4} s`，`p ∈ {2, 3, 5} °/s`，只取满足 §4.1 约束 3 的组合；
   - `γ̇_max ∈ {1, 1.5, 2, 3} ×` 下限；
   - `d ∈ {0, 2, 4, 6} s`。
...
| 回正时间常数 τ_ψ | 标定 | §4.1、§9 | 候选 2 / 3 / 4 s：下限 2Δt（不来回摆），上限 10°/r_turn（拆分转弯不停） |
| 坡度变化速度 p | 标定 | §4.1、§9 | 候选 2 / 3 / 5 °/s，只留回正时越过目标 ≤ 4.5° 的 |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
| 截获开始 | `|y| ≤ R(1 − cos Δχ)`，R = V/r_turn | 推导 | §4.4，几何 |
