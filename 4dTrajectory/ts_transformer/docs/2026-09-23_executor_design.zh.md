...
2. **办法 A**（`autopilot/derive.py`，合成转弯）：τ_ψ 取约束 2 允许的最大值（`10° / r_turn` 向下取到 0.5 s）；p 取使
   执行器自己在 60–140 m/s 各速度下转词表最大的一条词（150°，越过目标最多的转弯）都不越过目标超过航向容差的最小值
   （0.5°/s 一档，模拟核对）；γ̇_max 取 §5.1 下限的 2 倍。
...
| 反解 | 无参数 | 动力学方程 | §3 |
| 稳定转弯率 r_turn | 初量 2.15°/s | 数据 | 转角 ≥ 90° 的转弯，每个转弯中段转弯率的中位数，再对转弯取中位数 |
...
| 坡度变化速度 p | 初量 2.0 °/s | 原理（§9 办法 A） | 执行器自己在 60–140 m/s 转 150°（词表最大的一条词），越过目标都 ≤ 4.5° 的最小值（0.5°/s 一档，模拟）；敏感度在 train 上查 |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
...
| 发令到动作的延迟 d_ψ / d_h / d_v | 初量 2 / 0 / 0 s | §9 办法 B | 执行器按真值句子、延迟为 0 飞 train 抽样，经观测算子与标注器重读，量开始动作的词被标注器提前了几秒；负值取 0；某组一个词都没配上就拒绝 |
...
| E8 | 回放 runner 与门的读数（§11），对照图 | val 读数文档 | **代码完成并审查**：runner `executor_replay`（记录交给 evaluation，按 flight_key 与观测配对，两份报告的 schema 与 methodology 必须一致）；train 小样本走通；val 是阶段 4，等用户确认 |
