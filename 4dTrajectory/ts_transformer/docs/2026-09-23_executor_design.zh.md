...
| E8 | 回放 runner 与门的读数（§11），对照图 | val 读数文档 | **代码完成**：runner `executor_replay`（记录交给 evaluation，按 flight_key 与观测配对，两份报告的 schema 与 methodology 必须一致）；词按观测飞机的位置说（§11）。train 2,000 架（自己的动力学 1,581）：落地 98.3 %、词 95.4 %、evaluation 配对 96.5 %；直线进近各格全过，雷达引导 KSJC、KSMF 两格落地 91 % / 89 %（原因见 §11 的发现） |
