...
| 跟随航向词的坡度上限 | 32° | 词表 | `turn_bank_max_deg`；标注器按它收转弯（instruction-v3，2026-09-24 train 预跑后定，§4.1） |
| 坡度上限 φ_cap | 初量 24–25°；train 量出 25° | 数据 | 115–140 m/s 档，每个转弯坡度的中位数，再对转弯取中位数，向上取整，不超过词表 32°；instruction-v3 起只用于执行器自己的转弯 |
...
| 坡度变化速度 p | 测试参数与 train 参数上 5.0 °/s；v2 规格 2.0 °/s | 原理（§9 办法 A） | instruction-v3：都满足的最小值——执行器自己最大一次转弯（124.5°）越过目标 ≤ 4.5°；按词飞的 90° 转弯每条词都在包络内：r_turn 的理想转弯，和词表最大转弯率的转弯（观测飞机按同一个 p 压坡度），都在词表坡度上限内（60–140 m/s，每 5 m/s）；v2 时只看前一条，转弯取 150° |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
