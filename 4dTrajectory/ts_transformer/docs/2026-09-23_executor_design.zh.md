...
| 本设计 | 第二稿：控制律从动力学方程推出，每个参数写明来源；**待用户确认 §14** |
...
| **数据** | 只取数据看得清的量；看不清的用"同一个观测算子"标定（§9） | 稳定转弯率、减速率、发令到动作的延迟 |
...
- 积分器按"段"积分，每段的控制量不变（零阶保持），段内用 RK4。执行器实现为一个 **command hook**
  （`outputs/dynamics/hooks.CommandHook`）：每段开始时，积分器把这一刻的状态交给它，它返回这一段的
  `(δ_T, φ, n)`。`outputs/guidance/PlanGuidance` 就是这样接进去的，执行器与它并列，互不 import。
- **控制周期 Δt = 1 s**：每段 1 s，每个 2 s 的词步里两段。
- 动力学配置取控制路径现行配方 `simple-v3` 的那一对：`point-mass` + `scaled-transport-chart-velocity`，推力按
  `thrust-fraction`。执行器的输出因此就是 ts 控制路径能产生的那种轨迹。
...
- 由此得到：初态（位置、速度矢量、质量）、`aero_params`、`T_max`、坐标系参数（以本机着陆跑道入口为原点的东北天
  坐标）。质量是数据平面给的该机型的代表着陆质量（ADS-B 里没有真实质量）。
- 候选跑道的入口和航向，开飞前一次换算到这架航班的动力学坐标里。
...
```text
e   = wrap180(θ − χ)                              航向误差，较短方向
χ̇*  = sat(e / τ_ψ, ±r_turn)                        想要的转弯率
φ*  = 由 §3 反解；再限 |φ| ≤ φ_cap，|φ − φ_上一段| ≤ p · Δt
```

- **稳定转弯率 `r_turn`（数据）**：大转弯时飞机以大致恒定的转弯率转，而不是恒定的坡度。初量（train 12,000 架中
  转角 ≥ 90° 的 2,300 次转弯，取转弯中段）：

  | 地速 | 转弯率 p50 | 坡度 p50 |
  |---|---|---|
  | 85–100 m/s | 2.14°/s | 19.7° |
  | 100–115 m/s | 2.17°/s | 22.8° |
  | 115–140 m/s | 2.09°/s | 24.4° |

  转弯率与速度几乎无关（相关系数 −0.12），坡度随速度增大（相关系数 0.43）。所以律用转弯率，`r_turn` 取转弯率的
  中位数（初量约 2.15°/s）。大转弯的中段长于 15 s 的观测窗口，这个量看得清。
- **坡度上限 `φ_cap`（数据）**：速度很高时，恒定转弯率要的坡度会超过飞机愿意压的坡度，数据里 115–140 m/s 的坡度
  中位数 24.4°，按转弯率该是 28°。所以 `φ_cap` 取最高速度档的稳定坡度中位数（初量约 25°），且不超过词表的 32°。
- **改出转弯的时间常数 `τ_ψ`、坡度变化速度 `p`（§9 标定）**：两者都比 15 s 快，数据看不清，按 §9 用观测算子
  标定。先验约束：`τ_ψ ≥ 2Δt`，否则 1 s 的零阶保持会让航向在目标附近来回摆；`p` 使滚转进出在词表的转弯包络里
  （单调、不越过目标超过 4.5°）。
- **拆分的大转弯、盘旋**：每条词都是绝对航向，转角 ≤ 150° + 容差，较短方向就是标注器读出的方向；下一条在飞机
  还在转时到达，转弯不停。
...
以转弯率 `r_turn` 转过 Δχ，是半径 `R = V / r_turn` 的圆弧，横向移动 `R (1 − cos Δχ)`。所以：

```text
开始截获：  |y| ≤ R (1 − cos |Δχ|)，且朝航线飞（y 与 sin Δχ 异号）
```

这样转完正好在线上。若已在截获走廊的宽度之内（词表 §2.2），也立即截获。这里没有可调参数：R 由 `r_turn` 与速度决定。
...
```text
Δχ* = −sat(k_y · y, 30°)        30° 是词表的切入角
θ_line = 跑道航向 + Δχ*          作为 §4.1 的目标航向
```
...
临界阻尼（不越过中线，所以不会从走廊一侧穿到另一侧）要求 `1 = 4 τ_ψ V k_y`，即

```text
k_y = 1 / (4 V τ_ψ)
```

横向误差以时间常数 `2 τ_ψ` 衰减。`k_y` 由 `τ_ψ` 与速度决定，不单独调。

### 4.5 跑道指针与复飞

- 航线是跑道指针所指那条候选跑道的延长中线。许可之前换指针就换航线；许可之后换指针，标注里已被拒绝，执行器
  同样拒绝。
- 复飞（进近列）：取消"已许可"和"已截获"，横向把目标航向设为所指跑道的航向，纵向按爬升类爬到生效的高度目标，
  速度保持当前值。数据里没有复飞，这一条只有单元测试。
...
- **`τ_γ = 2 Δt`（推导）**：内环只负责让 γ 跟上外环给的 γ_ref。它越快越好，限度是 1 s 的零阶保持：`τ_γ = Δt`
  ...
  `γ̇_max` 决定，不由 `τ_γ` 决定。
- **`γ̇_max`（推导下限 + §9 标定）**：飞机从平飞转入按角 γ_k 下降时，航迹角只能以有限速度变化。在变过来之前，
  ...
  不因变角太慢而出管子；具体取值按 §9 标定，且不低于这个下限。
...
| 目标 T + 下降档 k | 按角度下降 | −γ_k（该档的名义角度，规格里各档的中心：0.91°、2.12°、3.06°、4.41°） |
| 目标 T + 爬升 | 按角度爬升 | +1.22°（规格里爬升类的名义角度） |
| 下降至落地 + 下降档 k | 按角度下降到落地 | −γ_k，不改平 |

下降途中来了新的角度词，就换 γ_k。方向与目标矛盾的组合在标注里已被拒绝，执行器遇到时同样拒绝。
...
载荷因子包线、推力上限、推力下限、失速下限。每架报出每条限制占多少步，以及触发时要的与给的相差多少。
...
| 结局 | 判定 |
|---|---|
| **落地** | 已许可、已截获，并沿航道越过所指跑道入口，离中线 ≤ 100 m（与标注器的落地判定相同）；越过时离入口的高度在 [0, h_max] 内。h_max 是观测航班越过入口时高度的 p99（train，实现时量） |
| 越过入口时太高 | 其他都满足，但高度 > h_max |
| 没截获就越过入口 | 越过入口时还没有"已截获"（或没有许可） |
| 入口前触地 | 还在入口前，高度已低于入口高程 |
| 超时 | 飞了观测剩余时长的 1.5 倍还没落地。1.5 倍给执行器留出速度落后的余地，又不让绕圈的航班无限飞下去 |
| 动力学失效 | 见 §8.1 |

**"按原话飞出来"** 要同时满足三条：落地；所有词都合格；没有动力学失效。这是回放门（§11）的主要读数。

---

## 9 数据看不清的参数：用同一个观测算子标定
...
- 每一列的"发令到动作的延迟" `d_ψ`、`d_h`、`d_v`。标注器的发令行是从平滑后的信号上读的：观测本身 15 s 拟合，
  航迹再做 6 s 平均。所以发令行可能比飞机真正开始动作早几秒。这个延迟补的是读法的偏差，也包含飞行员的反应时间。

**方法**：

1. **观测算子 O**：把执行器飞出的位置，按与观测相同的时刻采样，交给数据平面**同一个**速度拟合函数（15 s 居中最小
   二乘），得到"观测到的"航迹、地速、爬升率。标注器的平滑再照样做一遍。
2. **比较对象**：在每个词发令前 10 s 到发令后 60 s 的窗口里，比较经过 O 的执行器与观测：
   - 航向词比转弯率；
   - 高度词、下降角词比爬升率；
   - 速度词比地速。
3. **标定**：每组参数在一个小网格上找使差别最小的值，候选值事先写进执行器规格：
   - `τ_ψ ∈ {2, 3, 5, 8} s`；
   - `p ∈ {2, 3, 5} °/s`；
   - `γ̇_max ∈ {1, 1.5, 2, 3} ×` 下限；
   - `d ∈ {0, 2, 4, 6} s`。

   各组分开标：横向只看转弯率，纵向只看爬升率，速度只看地速。

   只用 train 的一个固定分层抽样（每个机场 400 架，只取动力学不是替代机型的航班）。**val 只用来判门，不用来调。**
4. **报出**：每组参数的误差曲面。某个参数的误差曲面是平的，说明数据仍然看不清它，就取符合先验约束、最接近真实
   飞机的那个候选值，并在规格里写明。
...
| 反解 | 无参数 | 动力学方程 | §3 |
| 稳定转弯率 r_turn | 初量 2.15°/s | 数据 | 转角 ≥ 90° 的转弯中段转弯率的中位数 |
| 坡度上限 φ_cap | 初量 25° | 数据 | 115–140 m/s 档稳定坡度的中位数（24.4°），不超过词表 32° |
| 改出时间常数 τ_ψ | 标定 | §9 | 候选 2 / 3 / 5 / 8 s，且 ≥ 2Δt |
| 坡度变化速度 p | 标定 | §9 | 候选 2 / 3 / 5 °/s |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
| 截获开始 | `|y| ≤ R(1 − cos Δχ)`，R = V/r_turn | 推导 | §4.3，几何 |
| 横向增益 k_y | `1/(4 V τ_ψ)` | 推导 | §4.4，临界阻尼 |
| 最大切入角 | 30° | 词表 | 切入角 |
| 航迹角内环 τ_γ | 2 s | 推导 | §5.1，2Δt |
| 航迹角变化速度 γ̇_max | 标定，≥ `V γ_lo² / (2ε)` | 推导 + §9 | §5.1 |
| 高度截获起点 | `V γ_k² / (2 γ̇_max)` | 推导 | §5.3 |
| 高度保持 τ_h | 8 s | 推导 | §5.4，4τ_γ，临界阻尼 |
| 下降档的名义角度 | 0.91° / 2.12° / 3.06° / 4.41°；爬升 1.22° | 词表 | 规格里各类的中心 |
...
| 发令到动作的延迟 d_ψ / d_h / d_v | 标定 | §9 | 候选 0 / 2 / 4 / 6 s |
| 落地高度上限 h_max | 待量 | 数据 | 观测越过入口时高度的 p99 |
| 超时 | 观测剩余时长 × 1.5 | 选定 | §8.3 |

标定完成后，这些值连同候选网格、标定用的抽样一起写进一份**执行器规格**，带 sha，与词表规格同样的规矩：改任何
一项就是新版本。
...
- **输入**：产物里每一架已标注的 val 航班，初态为第 0 行，词逐行生效。句子最后一行之后，词保持不变，直到落地
  或失败。
- **人群**：按机型的动力学分三组（初量：val 的 10,625 架，按 icao24 识别；数据平面的机型对照表 2026-09-23 更新过，
  ...
  | 识别出机型，但动力学用 A320 代替 | 2,469（23.2 %），多为 BCS3、P28A、CRJ7、E55P、BCS1 | 只报不判：误差说的是 A320 的气动，不是执行器 |
...
| 步 | 内容 | 验证 |
|---|---|---|
| E1 | 坐标与约定（罗盘 ↔ 数学、坡度符号）、每周期生效的词、航班输入的重建与核对 | 往返误差；重建的信号与产物逐行一致 |
| E2 | §3 的反解 | 反解后正向积分一段，变化率与想要的一致（零阶保持误差量级内） |
| E3 | 横向律（§4） | 合成状态：左右转、拆分、直角基线两侧、平行靠近；截获不越过中线 |
| E4 | 纵向律（§5） | 各档下降、中途换角、截获 T 不越过、高度保持、下降至落地 |
| E5 | 速度律与进近速度表（§6），量 a_dec / a_acc / a_unspec | 进近速度表覆盖了哪些机型 |
| E6 | hook、三层判定（§8）、落地与失败 | 合成航班整段飞：下风边—基线—五边落地 |
| E7 | 观测算子 O 与标定（§9），写执行器规格 | 误差曲面；只用 train |
| E8 | 回放 runner 与门的读数（§11），对照图 | val 读数文档 |
...
1. **动力学配置**：点质量 + 执行器自己限制坡度变化速度（§4.1 的 p），还是换成一阶滞后模型（滚转和推力有惯性，
   参数进动力学）。建议前者：滞后模型的时间常数同样比 15 s 快，数据一样看不清，要标定的参数不会更少。
2. **A320 替身航班只报不判**（val 的 23.2 %）。根本办法是在 `aircraft/` 补这些机型的动力学，那会改变 ts 所有数据集，
   需要你单独决定。
3. **"未指定"的速度用公开的进近速度**，不用从 train 拟合的每机型表。
4. **转弯按恒定转弯率飞，坡度有上限**（§4.1 的数据）。不按恒定坡度。
5. **控制周期 1 s**。
6. **看不清的参数按 §9 用观测算子标定**，只用 train，候选网格事先写定。
7. **门的两条线**：落地率 ≥ 95 %，词的合格率 ≥ 95 %，每个机场分开看。
