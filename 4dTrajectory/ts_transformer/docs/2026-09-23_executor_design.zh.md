...
| **只用词表（第二阶段，2026-09-24）** | 从数据量的最后几项也去掉，执行器除词表外只读被指跑道公布的入口跨越高度（TCH）。①速度变化的快慢取词表的一个速度级差 ÷ 速度词最短保持时间 = 5 m/s ÷ 20 s = 0.25 m/s²，减速、加速、"未指定"都用它（用户定；train 上词的合格率 97.2 %，用数据量的三个值时 97.5 %）。②落地：越过点取被指跑道公布的 TCH（FAA CIFP，harvest 的跑道数据；用户定"用跑道公布的 TCH，同时改飞法"），候选跑道没有 TCH 时标注器第一步就拒绝。③落地飞法（§5.2）：在生效的那条词的管子里下降，直到从管子出发、按最陡一档的下沿角度（3.73°）也够不到入口允许的高度（TCH ± 高度容差的一半）为止；截获之后下降不陡于"朝越过高度的高度保持律"与"到越过点的直线"中较陡的一个，入口前不会低于越过高度。train 500 架：高度词合格 86.9 % → 95.1 %，全部落地，越过高度没有一架超出 evaluation 的 TCH ± 22 m。规格格式 `ts-executor-spec-v5`、回放格式 `ts-executor-replay-v4`。正式产物：待跑（`outputs/POOLED/executor/v5_20260924/`） |
