...
| **第三版词表（instruction-v3）** | **用户确认（2026-09-24），实现于分支 `dev-vocab-v3`**：航向词逐行标注、提前 4 s 说（词表设计 §10.1）。执行器随之改：跟随航向词时转弯率只受词表最大转弯率与坡度上限限制，r_turn 只用于执行器自己的转弯（§4.1）；**跟随航向词的律改为"听到词后正好一个提前量时到达，但不快过坡度还来得及收回的转弯率"**（§4.1；2026-09-24 两轮代码审查后定：原定的一阶律 τ_ψ = L 在每次转弯的最后一两条词上必然出包络，只按剩余时间又会冲过转弯的最后一条词）；航向词、许可和跑道指针每 2 s 一行只在行首那个周期听一次，与判定按行读对齐（§4.1）；τ_ψ 只管执行器自己的转弯，取 L（§9）；航向词没有延迟（§9 办法 B 只量纵向与速度）；p 取两条都满足的最小值：执行器自己最大一次转弯（124.5°）越过目标不超过容差，按词飞的 90° 转弯每条词都在包络内（§9）；拆分大转弯不再存在（§4.2）；航向词的包络与判定改为逐行（§8.2）；规格格式 `ts-executor-spec-v3`、回放格式 `ts-executor-replay-v2`；源码指纹按模块名计入包外模块，与检出位置无关。下一代规格要重新量 |
| **只用词表（第一阶段，2026-09-24）** | 用户的原则：执行器不用词表以外的信息，靠从数据挖的参数才能做成就说明不能泛化。第一阶段（用户定）：去掉 r_turn、φ_cap（所有转弯按词表的转弯率与坡度上限 32°）、去掉延迟与办法 B（词说出即执行，每行行首听一次）、p = 32° ÷ 4 s = 8°/s、截获按词表转弯率的几何平均 1.53°/s 规划。正式产物 `outputs/POOLED/executor/v4_20260924/`（规格 `4c4038511ca2`，`8ca8d98f`）：train 每机场 400 架（自己的动力学 1,914 架）落地 99.9 %、evaluation 配对 98.8 %、词 97.7 %（航向 98.4 %、许可 98.9 %、高度 88.6 %、速度 100 %），只有 KSJC 直线进近的词没过门（高度词，§10.2 的问题）。还从数据量的：a_dec、a_acc、a_unspec 与落地瞄准（第二阶段） |
| **只用词表（第二阶段，2026-09-24）** | 从数据量的最后几项也去掉，执行器除词表外只读被指跑道公布的入口跨越高度（TCH）。①速度变化的快慢取词表的一个速度级差 ÷ 速度词最短保持时间 = 5 m/s ÷ 20 s = 0.25 m/s²，减速、加速、"未指定"都用它（用户定；train 上词的合格率 97.2 %，用数据量的三个值时 97.5 %）。②落地：越过点取被指跑道公布的 TCH（FAA CIFP，harvest 的跑道数据；用户定"用跑道公布的 TCH，同时改飞法"），候选跑道没有 TCH 时标注器第一步就拒绝。③落地飞法（§5.2）：在生效的那条词的管子里下降，直到从管子出发、按最陡一档的下沿角度（3.73°）也够不到入口允许的高度（TCH ± 高度容差的一半）为止；截获之后下降不陡于"朝越过高度的高度保持律"与"到越过点的直线"中较陡的一个，入口前不会低于越过高度。train 500 架：高度词合格 86.9 % → 95.1 %，全部落地，越过高度没有一架超出 evaluation 的 TCH ± 22 m。规格格式 `ts-executor-spec-v5`、回放格式 `ts-executor-replay-v4`。正式产物 `outputs/POOLED/executor/v5_20260924/`（规格 `0d6a68a92c6f`，`1b0cd4f4` 干净）：train 每机场 400 架，自己的动力学 1,914 架：落地 99.9 %、evaluation 配对 98.7 %、词 98.0 %（航向 98.0 %、许可 98.9 %、高度 **95.7 %**（第一阶段 88.6 %）、速度 100 %）、按原话飞出 82.5 %（72.2 %）；**每格都过门**（词最低 KMSY 被引导 96.5 %，KSJC 直线进近 93.2 → 97.8 %；evaluation 最低 KSTL 被引导 97.5 %）。航向词出包络 478 → 587 条，多在被引导航班上：减速按 0.25 m/s² 比数据的 0.28 慢，落地比观测早（中位数 −9 s，第一阶段 −3 s），按航迹说词的时钟把航向词说得更密。敏感度：τ_ψ 2–6 s、γ̇ 系数 1–3、p 6–10°/s 词 97.9–98.0 %，p 4°/s 时 96.8 % |
...
> 收得住   = sqrt(2 g p |e| / V)                         以这个转弯率转，坡度按 p 收回时正好转完 e（小坡度近似）
> |χ̇*|   = min(|e| / 剩余时间, 收得住, r_max)，方向随 e
> ```
>
> - 时刻按执行器自己的时间算；航向词（连同许可、跑道指针这两列没有延迟的词）每 2 s 一行只在行首那个周期听一次，
>   判定也从这一行起算（§8.2）。按航迹或按路程说词时，句子时间会在两行之间跨过一行；审查发现那样听到的词比判定
>   读它的行早一个周期，合成的盘旋按航迹说词时 73 条里 12 条出包络，改成只在行首听之后快转弯、下风边—基线—五边
>   在三种说词方式下都 0 条出包络。
> - "收得住"与沿线跟踪用的是同一条限制：执行器不知道哪一条词是转弯的最后一条，所以每条词都按"可能要停在这里"
>   来转。没有这一条时，过了期限按 2Δt 追，坡度收不回来：从平飞转一条 10° 的词冲过 4.6°，转弯率 3°/s 的基线转弯
>   最后一条词冲过 5.5°（审查量的）。
> - **坡度上限分两种**（2026-09-24 train 预跑后定）：跟随航向词时用词表的坡度上限 32°（标注器按它收转弯，词说的转弯
>   执行器都要能飞），执行器自己的转弯（切入、复飞、截获、沿线跟踪）用数据量出的 φ_cap（快档转弯坡度的中位数，约 25°）。
>   预跑时两者都按 25°：train 每机场 400 架（自己的动力学 1,914 架）航向词只有 90.9 % 在包络内，出包络的几乎全是转弯
>   中段落后（观测转弯要的坡度超过 25° 的词只有 74 % 在包络内）；同一批 1,000 架跟随航向词改用 32° 后 98.2 %。
> - r_max 是词表的最大转弯率（4.7°/s），坡度上限在反解里先起作用；数据量出的稳定转弯率 r_turn 只用于执行器自己的
>   转弯（自行切入、复飞、截获、沿线跟踪）。
>
...
> 60–140 m/s，读法与标注器相同）：一阶律每次转弯 19 条词里有 3–7 条出包络，p 取多大都一样；只按剩余时间 p ≥ 2.5°/s
> 时 0 条出包络、最差离词 2.8–3.9°（容差 4.5°），但会冲过转弯的最后一条词（上一条）；加上"收得住"之后要
> p ≥ 3.5°/s；再把按坡度上限的转弯也算进来、速度每 5 m/s 查一次，要 4.5°/s（测试参数上，§9）。
>
...
| 下降至落地 + 下降档 k | 按角度下降到落地 | 朝越过点（入口上方，见下）；在生效的词的管子里，直到从管子出发够不到入口允许的高度为止。截获航线之前：不比"到越过点的直线"更陡；截获之后：不比"朝越过高度的高度保持律"与"到越过点的直线"中较陡的一个更陡 |
| 复飞 + 下降至落地 | 爬升 | +γ_climb，直到来了新的高度词（§4.6） |

下降途中来了新的角度词，就换 γ_k。方向与目标矛盾的组合在标注里已被拒绝，执行器遇到时同样拒绝；"下降至落地"配的
不是下降档（平飞或爬升）也拒绝。

**落地的飞法**（`autopilot/vertical.py`，只用词表第二阶段，2026-09-24）：

- **越过点**：被指跑道**公布的入口跨越高度 TCH**（FAA CIFP 的垂直路径，harvest 的跑道数据
  `trajectory_data_process.harvest.airports.load_airport`，读取在 `autopilot/runway_data.py`；evaluation 的垂直门也以它为
  基准）。入口允许的高度是 TCH ± 高度容差的一半（25 m 的一半，12.5 m），且在落地条件之内。越过点取 TCH 挪进"生效的
  那条词的管子延伸到入口处"（`labeller.vertical.tube_bounds` 的读法：从这条高度词或角度词说出的位置起，按该档两条
  角度线，± 高度容差，沿飞过的路程；取内半部分）与允许高度的交集里；没有交集时取允许高度里离管子近的一端。
- **在管子里下降**：每个周期朝越过点飞（截获航线之后是沿中线到越过点的角度，之前是这一档的名义角度，但不比到越过点的
  直线更陡），同时限在生效那条词的管子里（该档两条边线的角度，加上高度保持律把飞机拉回边线以内的修正）。只要从管子
  出发、在平飞与词表最陡一档的下沿角度（3.73°）之间飞还够得到入口允许的高度，就一直留在管子里；够不到了才离开管子，
  直接朝越过点飞（模式 `aim_left_tube`，只在截获航线之后、不在复飞中计：落地优先）。为什么不在"这一档延伸到入口够不到"时就离开：判定在每条角度词处重新起管子，
  坡度在进近中途变陡或变缓的航班，任何一档单独都够不到入口，但后面的词会把它带过去——上一版在这时就离开管子，这些
  航班的落地词出了包络（第二阶段代码审查指出）。
- **不低于越过高度**：截获之后，下降不陡于"朝越过高度的高度保持律"与"到越过点的直线"中较陡的一个，所以入口前不会
  低于越过高度；截获之前不陡于到越过点的直线（直线是最短的路，下风边上不会下到地上）。

train 500 架（每机场 100，自己的动力学，按 `time` 说词）比较（2026-09-24）：

| 飞法 | 落地 | 越过高度超出 TCH ± 22 m | 高度词合格 | 全部词合格 | 照句子飞完 |
|---|---|---|---|---|---|
| 上一版：这一档延伸到入口够不到就离开管子 | 500 | 0 | 86.9 % | 97.2 % | 70.2 % |
| **现在：够不到允许高度才离开（按最陡一档的下沿 3.73° 算）** | **500** | **0** | **95.1 %** | **97.9 %** | **82.2 %** |
| 按最陡一档的上沿 10° 算"够得到" | 500 | 19（都偏高） | 96.4 % | 98.0 % | 83.8 % |
| 截获后不设"不低于越过高度"的限制 | 491（9 架触地） | 0 | 96.0 % | 98.0 % | 82.0 % |
| 只用高度保持律作限制（不取与直线中较陡的） | 500 | 19（偏高：离入口约 1 km 内下不去） | 95.0 % | 97.9 % | 82.0 % |
| 越过点直接取 TCH（不挪进管子） | 500 | 0 | 94.5 % | 97.9 % | 81.4 % |

审查给的两架合成直线进近（下降角 2.1° → 3.3° → 4.3° 变陡，3.9° → 2.8° → 2.0° 变缓）：上一版落地词 104/144、119/157
行在包络内，现在 144/144、157/157。

演变（更早，train，2026-09-24）：全程名义角度，100 架落地 58；截获前名义、截获后瞄准数据量出的 `h_aim`（20.8 m）
并限在第 k 档的角度内，83；放开到"平飞到最陡一档"并加截获前的直线限制后，200 架落地 98 %；再把越过高度放进管子和
数据量出的窗口（8.4–36.5 m）后，train 2,000 架落地 98.3 %；第二阶段把 `h_aim` 和窗口换成公布的 TCH（这两项的量法
归档在 `archive/executor_vocabulary_only_2026_09/`）。
...
V̇*    = sat((V_ref − V) / τ_V, [−a, +a])
V_ref ≥ 1.10 · V_stall(n)                         失速下限，用 speed_floor 的同一个定义
```

推力由 §3 的反解给出。复飞时保持当前空速（§4.6）。

"未指定"是飞行员自己落地用的速度：减到它的减速度取 `a`，和"按离入口的直线距离在入口前减到它所需的减速度"中
较大的一个，最多到词表的加速度上限。只用 `a` 时，"未指定"说得晚、五边长、速度高的航班越过入口时比机型的速度窗口快
20 m/s，evaluation 的速度门不过。

- **`a`（词表，只用词表第二阶段，2026-09-24）**：速度变化的快慢，减速、加速、"未指定"都用同一个值：词表的一个速度
  级差 ÷ 速度词最短保持时间 = 5 m/s ÷ 20 s = 0.25 m/s²（用户定；`autopilot/speed.py` `speed_change_mps2`）。原来按
  数据量三个值（train：减速 0.28、加速 0.19、"未指定"之后 0.30 m/s²），train 上词的合格率 97.5 %，换成 0.25 后 97.2 %，
  速度词全部在包络内。量它们的代码归档在 `archive/executor_vocabulary_only_2026_09/`。
- **`τ_V = δv / a`（推导）**：离目标还剩一个带的半宽 δv = 5 m/s 时，从匀减速转成指数逼近，进带时速度单调、不越过
  目标（词表的速度过渡包络）。a = 0.25 m/s² 时 `τ_V = 20 s`。
...
2. **办法 A**（`autopilot/derive.py`，合成转弯）：
   - instruction-v3：τ_ψ 取 L（选定，不是推出：执行器自己的转弯按一条词给的时间收尾；航向词有自己的律，§4.1）。
     p 取 0.5°/s 一档上、在 60–140 m/s 每 5 m/s 一个速度下都满足的最小值：
     (1) 执行器自己最大的一次转弯（90° + 容差 + 切入角 = 124.5°）越过目标不超过航向容差；
     (2) 按词飞两个 90° 转弯——转弯率 r_turn 的和词表最大转弯率 r_max 的，都不快过坡度上限在这个速度允许的——每条词
     判定的每一行都在包络内。词的读法近似标注器：观测拟合当作 15 s 居中滑动平均，再取 2 s 网格、做标注器的航迹平滑，
     然后 `per_step_words`（近似：真的拟合是对位置做最小二乘；第二轮审查核过，换成最小二乘的权重在 5° 格上一条词都不变）。
     飞出的航迹按判定的读法读（只做标注器的航迹平滑，不做观测拟合），每条词在行首那个周期听到。
     - 速度每 5 m/s 查一次：按词转弯的超出量在相邻速度之间能差 0.8°（词落在 2 s 行里的位置不同），原来只查
       60/80/100/120/140 时得 3.5°/s，5 m/s 一档得 4.0°/s（第二轮审查量的）。
     - 跟随航向词的坡度上限改为词表的 32° 之后（§4.1），最快的一个转弯改为让观测飞机也按执行器自己的 p 压坡度、
       收坡度（`observed_turn_deg`）：理想转弯一下子压到 32°，执行器也在 32° 上，任何 p 都追不上（115–135 m/s 在
       8°/s 时仍出包络）；改后问的是"执行器能不能跟上一架与自己滚转一样快的飞机飞出的任何转弯"。这样 p = 5.0°/s
       （测试参数与 train 量出的参数上都是，搜索约 1 min）。
     - （以下是跟随航向词仍按 φ_cap 时的记录）测试的参数上（r_turn 2.15°/s、坡度上限 25°）p = 4.5°/s，由 (2) 里坡度上限的转弯在 100 m/s 定（4.0°/s 时出
       0.14°）；坡度上限 24° 时 4.0°/s。整个搜索约 55 s。
     - 没有纳入的：直线上突然说一条跨两格的词。标注器读的是平滑后的航迹，直线上不会一下跨两格；而且没有合理的 p 能跟上
       （10°/s 才不出）。航迹时钟一步跨两行时会让两条航向词同时到，回放把这样同时到的词单独计数
       （`replay.summary` 的 `heading_words_told_with_a_skipped_word` 和对应的失败原因），不算成执行器的问题；
...
| 反解 | 无参数 | 动力学方程 | §3 |
| ~~稳定转弯率 r_turn~~ | 已去掉（2026-09-24，只用词表第一阶段） | — | 原为数据量出的 2.2°/s（转角 ≥ 90° 的转弯中段转弯率中位数）；现在所有转弯都在词表的转弯率范围（0.5–4.7°/s）和坡度上限 32° 内；量它的代码归档在 `archive/executor_vocabulary_only_2026_09/` |
| 跟随航向词的最大转弯率 r_max | 4.7°/s | 词表 | `turn_rate_max_deg_s`（instruction-v3）；坡度上限在反解里先起作用 |
| 坡度上限 | 32° | 词表 | `turn_bank_max_deg`；标注器按它收转弯；2026-09-24 起执行器所有转弯都用它（先是跟航向词时，只用词表第一阶段后全部） |
| ~~坡度上限 φ_cap~~ | 已去掉（2026-09-24） | — | 原为数据量出的 25°（115–140 m/s 转弯坡度中位数）；现在执行器所有转弯都用词表的坡度上限 32°（在反解里先起作用） |
| 跟随航向词的律 | `min(|e| / max(听到时刻 + L − 现在, 2Δt), sqrt(2 g p |e| / V), r_max)` | 词的含义（§4.1） | instruction-v3：听到词后正好 L 秒到达，但不快过坡度收得回的转弯率；航向词每行只在行首听一次；原定一阶律 τ_ψ = L 在转弯末尾出包络，2026-09-24 两轮审查后定 |
| 回正时间常数 τ_ψ | 提前量 L（4 s），只管执行器自己的转弯；v2 时为 `10° / r_turn`（4.5 s） | 词表（§9 办法 A） | 自己的转弯按一条词给的时间收尾；train 敏感度 2–6 s 不敏感 |
| 坡度变化速度 p | 8°/s；v3 规格 5.0°/s，v2 规格 2.0°/s | 词表（§9 办法 A） | 词表坡度上限 ÷ 提前量 = 32° ÷ 4 s（用户 2026-09-24 定）：一个提前量之内压到词表允许的最大坡度。train 敏感度：4 / 6 / 8 / 10°/s 时词 96.7 / 97.5 / 97.7 / 97.7 %。原来按合成转弯检查定 p 的办法（结果不稳）已归档 |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
| 截获开始 | 圆弧 + 压坡度 + 回正尾巴三段之和，圆弧按词表转弯率上下限的几何平均 √(0.5 × 4.7) ≈ 1.53°/s 规划 | 词表 + 推导 | §4.4；用户 2026-09-24 选几何平均：按下限 0.5°/s 规划时转弯卡在包络边界上（500 架里 17 条截获出包络），几何平均时 4 条 |
| 截获转弯的转弯率 | 切到线上的圆弧，不低于词表下限，受坡度上限 32°、4.7°/s、`|Δχ|/τ_ψ` 限制 | 推导 | §4.4；"收得住"限制在这里永远起不了作用（p τ_ψ = 32°），已去掉 |
| 走廊内的修正角 | 走廊航向容差的一半（1°） | 词表 | §4.5 |
| 落地的越过点 | 被指跑道公布的 TCH（挪进生效词的管子与允许高度 TCH ± 12.5 m 的交集） | 跑道公布数据 | §5.2；只用词表第二阶段（2026-09-24）取代数据量出的 h_aim 20.8 m 与窗口 8.4–36.5 m |
| 落地时留在管子里的条件 | 从管子出发按最陡一档的下沿 3.73° 还够得到允许高度 | 词表 | §5.2；按上沿 10° 算时离开太晚，19/500 架越过高度偏高超过 22 m |
...
| 速度变化的快慢 a | 0.25 m/s²（减速、加速、"未指定"同一个） | 词表 | 速度级差 ÷ 速度词最短保持时间 = 5 m/s ÷ 20 s（用户 2026-09-24 定）；取代数据量出的 a_dec / a_acc / a_unspec（train 0.28 / 0.19 / 0.30 m/s²） |
| 速度逼近 τ_V | `δv / a` | 推导 | §6，δv = 5 m/s |
| "未指定"的目标速度 | 公开进近速度，按质量与密度换算 | 公开数据 | `aircraft/reference_speeds` |
| 失速裕度 | 1.10 | 现有 | `speed_floor` |
| ~~发令到动作的延迟~~ | 已去掉（2026-09-24） | — | 词说出即执行（词表的含义）；原来由办法 B 量（instruction-v3 下都是 0），办法 B 已归档 |
...
- **词在哪里说**（`ExecutorParams.word_clock`，`autopilot/sentence.py`）：一条词只给目标，不给快慢（速度词不说减速多快，
  ...
  按航迹 / 按路程说词时，时钟可能一步跨两行，两条航向词同时到：前一条没飞，与它同时到的那一条单独计数
  （`replay.told_with_skipped`；门表每格有 `heading_words_told_with_a_skipped_word` 和去掉它们之后的词合格率
  `words_inside_without_them`），门本身仍按全部词算。
...
- **现行的 val 回放门**（2026-09-24，只用词表的执行器，规格 `0d6a68a92c6f`，按 `track` 说词；句子产物按运行日划分的
  `instruction_language/v4_20260924`（词表规格 `145d6911e75b`），新验证集运行日上每一架已标注的航班；`outputs/POOLED/executor/
  v6_20260924/replay-val/`，代码 `dev-prior-v3` `67e0e5c9` 干净）：自己的动力学 9,246 架进门，替代机型 498 架只报。

  | 自己的动力学 | 落地 | 词在包络里 | evaluation 配对 |
  |---|---|---|---|
  | 全部 9,246 | 99.9 % | 98.0 % | 98.6 % |
  | 直线进近 6,090 | 100 % | 98.6 % | 98.9 % |
  | 被引导 3,156 | 99.8 % | 97.8 % | 97.8 % |

  **按机场 × 进近方式的 15 格全部过门（≥ 95 %）**：落地最低 KSTL 被引导 99.6 %；词最低 KMSY 被引导 95.9 %、KSTL 被引导 97.1 %；
  evaluation 最低 KMSY 被引导 95.9 %、KSJC 被引导 96.9 %。同一规格在新训练集运行日上每机场 400 架（`replay-train/`，自己的动力学
  1,917 架）：落地 99.8 %、词 97.9 %、evaluation 98.2 %，每格都过。替代机型组（只报）：落地 100 %、evaluation 配对 73.0 %。
  执行器规格与 `v5_20260924` 的逐字节相同：执行器只读词表的固定取值，重新测量词表规格（下降角档的边界差 0.01°，最大加速度
  1.7 → 1.4 m/s²）没有改到它；落地飞法里"最陡一档的下沿"按规格读，现在是 3.74°。

  **train 敏感度**（第一版执行器的规格 `2674ab8c71a9`，每机场 400 架；只用词表的执行器的敏感度见 §0）：可取的变动里，没有一项在落地、按原话飞出、词的合格率上同时比规格的值好
