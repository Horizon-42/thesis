...
2. **办法 A**（`autopilot/derive.py`，合成转弯）：
   - instruction-v3：τ_ψ 取 L（选定，不是推出：执行器自己的转弯按一条词给的时间收尾；航向词有自己的律，§4.1）。
     ...
     飞出的航迹按判定的读法读（只做标注器的航迹平滑，不做观测拟合）。在测试的参数上（r_turn 2.15°/s、坡度上限 25°），
     p = 3.5°/s，由 (2) 定：3.0°/s 时 (1) 满足（最多 0.47°），但 120–140 m/s 按词转弯出包络 0.4–0.6°。
     没有纳入的两种合成情形：按坡度上限的转弯率转（理想转弯，观测飞机一下子就压到上限，执行器也在上限上，任何 p 都
     追不上，要 p ≥ 5°/s 才勉强不出）；直线上突然说一条跨两格的词（标注器读的是平滑后的航迹，直线上不会一下跨两格）。
     这两种留给 train 回放和敏感度（p 2–5°/s）去量；
...
| 坡度变化速度 p | 测试参数上 3.5 °/s；v2 规格 2.0 °/s | 原理（§9 办法 A） | instruction-v3：两条都满足的最小值——执行器自己最大一次转弯（124.5°）越过目标 ≤ 4.5°，按词飞的 90° 转弯每条词都在包络内（60–140 m/s）；v2 时只看前一条，转弯取 150° |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
