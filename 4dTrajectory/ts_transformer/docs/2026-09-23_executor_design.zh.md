...
| **第三版词表（instruction-v3）** | **用户确认（2026-09-24），实现于分支 `dev-vocab-v3`**：航向词逐行标注、提前 4 s 说（词表设计 §10.1）。执行器随之改：跟随航向词时转弯率只受词表最大转弯率与坡度上限限制，r_turn 只用于执行器自己的转弯（§4.1）；τ_ψ 由提前量定（§4.1 约束 2、§9）；航向词没有延迟（§9 办法 B 只量纵向与速度）；p 取执行器自己最大一次转弯（124.5°）不越过容差的最小值；拆分大转弯不再存在（§4.2）；航向词的包络与判定改为逐行（§8.2）；规格格式 `ts-executor-spec-v3`、回放格式 `ts-executor-replay-v2`；源码指纹按模块名计入包外模块，与检出位置无关。下一代规格要重新量 |
...
> 转多快由词出现的节奏定，所以跟随航向词时律是 `χ̇* = sat(e / τ_ψ, ±r_max)`，r_max 是词表的最大转弯率（4.7°/s），
> 坡度上限在反解里先起作用；数据量出的稳定转弯率 r_turn 只用于执行器自己的转弯（自行切入、复飞、沿线跟踪的航向）。
> 下面约束 2 改为：一阶律追一个匀速变化的目标，稳定落后 `r·τ_ψ`；词提前 L 说出，飞出的航迹要落在"L 之前那条词
> ± 容差"里，而词本身离观测航迹最多半格，所以要 `r_max |τ_ψ − L| ≤ 容差 − 半格`（`params.check`），办法 A 取其中最大的
> τ_ψ（设计值下即 τ_ψ = L = 4 s）。本节以下描述 instruction-v2 的推导（拆分转弯、续发提前量 10°），留作来历。
...
| 回正时间常数 τ_ψ | instruction-v3：`L + (容差 − 半格) / r_max` 向下取到 0.5 s（设计值下 = L = 4 s）；v2 时为 `10° / r_turn`（4.5 s） | 原理（§9 办法 A） | 约束 2 允许的最大值（§4.1）；敏感度在 train 上查（约束下只剩 4 s 一档） |
| 坡度变化速度 p | 初量 2.0 °/s | 原理（§9 办法 A） | instruction-v3：执行器自己用航向律飞的最大一次转弯（90° + 容差 + 切入角 = 124.5°）在 60–140 m/s 越过目标都 ≤ 4.5° 的最小值；v2 时为 150°。另记：合成测试里 2.0°/s 时，2.25°/s 转弯的头一条词在滚进时出包络 0.2°，2.5°/s 则全在包络内——train 回放要看"转弯开头的词"的合格率 |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
