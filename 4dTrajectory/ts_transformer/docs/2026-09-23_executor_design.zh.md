...
| 实现 | 代码完成到 E8（2026-09-24，分支 `dev-executor`，包 `ts_transformer/autopilot/`）；E3–E6 已审查并按审查改完。待办：正式量执行器规格、train 上的敏感度、E7–E8 代码审查。进度见 §13 |
...
| 下降至落地 + 下降档 k | 按角度下降到落地 | 截获航线之前 −γ_k；截获之后瞄准入口上方 `h_aim` 的点（沿中线的角度），限在第 k 档自己的角度范围内，不改平 |
| 复飞 + 下降至落地 | 爬升 | +γ_climb，直到来了新的高度词（§4.6） |

下降途中来了新的角度词，就换 γ_k。方向与目标矛盾的组合在标注里已被拒绝，执行器遇到时同样拒绝；"下降至落地"配的
不是下降档（平飞或爬升）也拒绝。

**落地的瞄准**：`h_aim`（数据）是 train 航班在入口处的高度：句子在入口前 1.5 km 内结束的航班，最后 10 行沿航迹外推到
入口，取中位数（初量 20.8 m）。截获之前还不知道到入口要飞多远（下风边可能先飞过入口再转回来），所以按名义角度。
train 100 架（每机场 20，2026-09-24）比较了四种做法：全程名义角度落地 58 架；截获前也瞄准 80；截获前取该档最缓的
角度 73；截获前名义、截获后瞄准 83（采用）。一档宽约 1°，词又按时间生效，执行器到某个词时不在观测飞机的位置，
全程名义角度就会差出几十米。
...
| 许可加入 | 执行器的截获转弯（从开始截获到交给沿线跟踪）单调、转弯率与坡度在 §2.3 范围内；进了走廊之后一直在走廊里（到入口前） | `labeller.lateral._turn_check`、`envelope.corridor` |
...
| 回正时间常数 τ_ψ | `10° / r_turn` 向下取到 0.5 s（初量 4.5 s） | 原理（§9 办法 A） | 约束 2 允许的最大值（§4.1）；敏感度在 train 上查 |
| 坡度变化速度 p | 初量 2.5 °/s | 原理（§9 办法 A） | 执行器自己在 60–140 m/s 转 90°，越过目标都 ≤ 4.5° 的最小值（0.5°/s 一档，模拟）；敏感度在 train 上查 |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
...
| 发令到动作的延迟 d_ψ / d_h / d_v | 初量 2 / 0 / 0 s | §9 办法 B | 执行器按真值句子、延迟为 0 飞 train 抽样，经观测算子与标注器重读，量标注器提前了几秒；负值取 0 |
...
| E7 | 按 §9 定参数（A + B），在 train 上做敏感度检查，写执行器规格 | 敏感度表；只用 train | **代码完成**：`autopilot/{measure,derive,observe,spec,replay}.py`，runner `executor_spec`、`executor_sensitivity`；正式量待 E7 代码审查之后 |
| E8 | 回放 runner 与门的读数（§11），对照图 | val 读数文档 | **代码完成**：runner `executor_replay`（记录交给 evaluation，按 flight_key 与观测配对）；train 小样本走通；val 是阶段 4，等用户确认 |
