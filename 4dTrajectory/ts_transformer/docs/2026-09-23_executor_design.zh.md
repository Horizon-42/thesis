...
8. **§9 用哪种办法定看不清的参数**：建议 A + B，再做敏感度检查；只有敏感度大时才用数据（C 或 D）。
9. **平行跑道的横向上限怎么共用**：这个上限现在是 harvest 里的私有函数（`_runway_bracket_cross_limit`）。
   建议把它原样移到 `final_approach`，公开，harvest 改为 import。行为不变，但这是对数据流水线包的改动，需要你同意。
   不同意的话，标注器和执行器按"只取常数、条件一致"的办法，在自己这边照写这条规则，并注明是镜像。
