...
词的定义见[指令词表](2026-09-23_instruction_vocabulary_design.zh.md)（规格 `103a6eae6b90`，读法 `instruction-v2`），
...
| 实现 | 进行中（2026-09-24 起，分支 `dev-executor`，包 `ts_transformer/autopilot/`），步骤与进度见 §13 |
...
**"按原话飞出来"** 要同时满足三条：落在所指跑道上；所有词都合格；没有动力学失效。evaluation 的三道门另报，并与
...
| 办法 | 怎么做 | 好处 | 问题 |
|---|---|---|---|
| **A 由约束和原理直接定** | τ_ψ、p 在 §4.1 三条约束围出的范围里，按"不出包络的最温和机动"取：τ_ψ 取上限内最大的 4 s，p 取刚好使回正越过目标 ≤ 4.5° 的值；γ̇_max 取 §5.1 下限的 2 倍 | 不依赖数据，每个数都能从词表和执行器推回去 | "最温和"是一个选择，要说明理由 |
...
1. **数据参数**（train 全部已标注航班，`autopilot/measure.py`）：
   - 稳定转弯率 r_turn：转角 ≥ 90° 的转弯去掉首尾各四分之一后，中段转弯率的中位数；
   - 坡度上限 φ_cap：同样的转弯中段，地速 115–140 m/s 那一档坡度的中位数，向上取整，不超过词表的 32°；
   - 减速 a_dec、加速 a_acc：有目标速度的速度词，从发令到进入 V ± 5 m/s 带、历时 ≥ 20 s 的过渡段的平均加速度，
     按正负分开取中位数；
   - "未指定"后的减速 a_unspec："未指定"词之后第一段 ≥ 20 s 的减速段（地速分段拟合）的斜率中位数；
   - 落地的瞄准高度：句子在入口前 1.5 km 内结束的航班，最后 10 行沿航迹外推到入口的高度的中位数（§5 的落地律）；
   - 每个航班都先用标注器重读，必须与存下的句子逐格相同，参数量的就是执行器要飞的那些句子。
2. **办法 A**（`autopilot/derive.py`，合成转弯）：τ_ψ 取约束 2 允许的最大值（`10° / r_turn` 向下取到 0.5 s）；p 取使
   执行器自己在 60–140 m/s 各速度下转 90° 都不越过目标超过航向容差的最小值（0.5°/s 一档，模拟核对）；γ̇_max 取
   §5.1 下限的 2 倍。
3. **办法 B**（`autopilot/observe.py`）：观测算子 O 就是数据平面自己的一串函数——1 Hz 的位置点经
   ...
   - 飞出的航迹经 O、再用标注器重读；
   - 每个执行器收到的词，找重读句子里同一列、同一取值、时间最近的词，二者之差（执行器收到的时刻减去重读的发令
     时刻）就是读法提前的量；
...
4. **执行器规格**（`experiments/executor_spec.py`，`autopilot/spec.py`）：上面各值连同量法、样本写进
   ...
   同样的规矩：写一次，不覆盖；只在干净的工作树上量，记下 commit 和执行器源码的 sha（`EXECUTOR_MODULES`：除规格文件
   和判定之外的执行器模块）；回放时源码 sha 不同、词表 sha 不同、或标注器源码与产物不同，都拒绝。固定的设计选择
   （Δt = 1 s、τ_γ = 2 Δt、γ̇_max 为下限的 2 倍、超时系数 1.5）是 runner 的模块常量，也写进测量文件。
5. **敏感度**（`experiments/executor_sensitivity.py`，train 每机场 400 架、动力学不是替代机型的）：τ_ψ 从 2 s 到办法 A 的值
   （再大就违反约束 2）、p 2–5°/s、γ̇_max 为下限的 1–3 倍、d ± 4 s 各自变动，报落地率、按原话飞出的比例、词的合格率、
   与观测的时间对齐误差。
...
| 回正时间常数 τ_ψ | 4 s | 原理（§9 办法 A） | 约束范围 2–4.7 s（§4.1）内最温和的；敏感度在 train 上查 |
| 坡度变化速度 p | 约 2 °/s（按 τ_ψ = 4 s 由约束 3 算，模拟核对） | 原理（§9 办法 A） | 回正越过目标 ≤ 4.5° 的最小值；敏感度在 train 上查 |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
...
| 发令到动作的延迟 d_ψ / d_h / d_v | 推算 | §9 办法 B | 合成机动经过观测算子与标注器，量标注器提前了几秒 |
...
| E7 | 按 §9 定参数（A + B），在 train 上做敏感度检查，写执行器规格 | 敏感度表；只用 train | |
...
2. A320 替身航班先不补动力学（用户正在处理 `aircraft/`），回放门里只报不判。
