...
2. **办法 A**（`autopilot/derive.py`，合成转弯）：
   - instruction-v3：τ_ψ 取 L（选定，不是推出：执行器自己的转弯按一条词给的时间收尾；航向词有自己的律，§4.1）。
     ...
     - 测试的参数上（r_turn 2.15°/s、坡度上限 25°）p = 4.5°/s，由 (2) 里坡度上限的转弯在 100 m/s 定（4.0°/s 时出
...
| 坡度变化速度 p | 测试参数上 4.5 °/s；v2 规格 2.0 °/s | 原理（§9 办法 A） | instruction-v3：都满足的最小值——执行器自己最大一次转弯（124.5°）越过目标 ≤ 4.5°，按词飞的 90° 转弯（r_turn 的和 r_max 的，都在坡度上限内）每条词都在包络内（60–140 m/s，每 5 m/s）；v2 时只看前一条，转弯取 150° |
| 许可后的修正角 | 4.5° | 词表 | 航向容差 |
