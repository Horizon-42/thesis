...
> **已取代（2026-09-07）**：跟踪器**代码已删除**（包审计 T1-9：`control/constraints/closure_tracking.py`、`forecast.track_closure_forecasts` / `tracking_config`、`predict --closure-track`、`docs/experiments/closure_p1d_arms.json`、`tests/test_closure_tracking.py`），BLOCKER 不修（`2026-09-07_latent_intent_design.zh.md` §四）。本文的"跟踪付出 +9 m"含 8 架跳段航班，剔除后为 **+10.5 m**；数字保留作历史，第一节的模块说明与第五节的复现命令都已不可执行。
