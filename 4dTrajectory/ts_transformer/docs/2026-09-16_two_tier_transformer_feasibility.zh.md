...
> **已取代（2026-09-17）**：本文档的设计与开发方案（§4、§6、§7、§10.3、§10.5、§10.7 的门）改写了用户的需求——低层做成了整段预测而非短时预测，高层段 token 层被一个不相干的测试（T0(c)）否掉、从未建。
> 现行计划见 `2026-09-17_two_tier_plan_v2.zh.md`。这里只保留测量记录：§3（误差随提前量）、§10.4（T0(b)）、§10.6（T0(c)）、§10.9（T1a，错的设计下的结果）、§十一（契约重构，仍有效）。
...
- 读未来的地方都去掉：时限用第一次询问的到达时间算（`T₀ + max(30 s, 0.1·T₀)`，§10.7 修订后与基线制导同一规则；真值来源下 T₀ = 真值时长，头来源下 = 头自己的 T）；头在任何它训练过的航班上被拒（逐航班对照头的 train 名册）。**跑道仍是已知的**（包内通例：阈值锚定坐标系）。
...
- **T2.1（条件性，预注册风险 8 的读数）**：G3 在 top-1 上读；§7 风险 8 要求"top-1 与 minADE_K 两个数，只有 minADE_K 改善而 top-1 退化时交付形态改为分布"。fan 读数按 plan 路径 §12.8 的形式建：K 个成员各自第一问用头的第 k 个分量、之后用 top-1，另加同 K、同张角、无学习结构的位移对照 fan，minADE 只与对照 fan 比。成本约为 J7 的 8 倍。**只在 G3 的 top-1 不过时建与跑**（此时它决定交付形态）；G3 过则不需要。
...
### 10.9 T1a 结果：G1 不过（2026-09-17，KRDU val 1404，runs worktree @ 87f46cc，lockstep 9 min，`two_tier_gates` 未拒任何项）

**G1 逐条（receding，按 §10.7 的读法）**：

| 条件 | p50_s1337 | p50_s2024 | 门 |
|---|---|---|---|
| 雷达引导 ADE 均值 | 2224 | 2162 | < 1000 ✗ |
| 直线 ADE 均值 | 367 | 334 | < 200 ✗ |
| 完全可飞（全部） | 0.718 | 0.791 | ≥ 0.95 ✗ |
| 建立 直线 | 0.917 | 0.909 | ≥ 0.878 ✓ |
| 建立 雷达引导 | 0.264 | 0.410 | ≥ 0.774 ✗ |

**各变体**（ADE 均值 / 完全可飞 / 建立；雷达引导 497 · 直线 904）：

| 臂 · 变体 | 雷达引导 | 直线 |
|---|---|---|
| p50_s1337 receding | 2224 / 0.316 / 0.264 | 367 / 0.941 / 0.917 |
| p50_s1337 receding-no-plan | 2373 / 0.241 / 0.296 | 356 / 0.954 / 0.926 |
| p50_s1337 one-shot | 1761 / 0.024 / 0.318 | 290 / 0.993 / 0.892 |
| p50_s2024 receding | 2162 / 0.477 / 0.410 | 334 / 0.966 / 0.909 |
| p50_s2024 receding-no-plan | 2372 / 0.557 / 0.509 | 335 / 0.971 / 0.912 |
| p50_s2024 one-shot | 1732 / 0.058 / 0.326 | 296 / 0.996 / 0.896 |
| p0_s1337 receding | 2215 / 0.264 / 0.183 | 340 / 0.979 / 0.920 |
| p0_s1337 receding-no-plan（额外，不发布） | **5365** / 0.334 / 0.187 | 309 / 0.988 / 0.933 |
| 对照：规则制导 lockstep（基线） | 1847 / 0.998 / 0.879 | 283 / 0.999 / 0.998 |

配对 ΔADE（均值，雷达引导）：receding − no-plan −150 / −209 m（计划只值这么多）；receding − one-shot **+463 / +430 m**（再问反而更差）；receding − 基线 +377 / +315 m。

**读法**：
1. **再问在漂移**（§10.4 的 T1c 触发条件成立）：雷达引导位移 p50 从 60 s 起 receding 就大于 one-shot（s1337：60 s 544 对 468，180 s 2441 对 2106，300 s 2996 对 2215），直线同向（180 s 867 对 530）。模型只在真实历史上训练过，lockstep 里的历史是自己飞出来的——T0(b) 看到的锚点分布外，随机锚点训练与计划 token 都没消掉它。
2. **不可飞全是失速**（硬违例只有 stall；one-shot 雷达引导 485/497 架）。不是结束规则造成的：checkpoint 自己的 `pred_val`（L−1 一次成形、恰好飞完 CTA、无收尾询问）完全可飞 0.650 / 0.661，全部是失速，而 N7 同契约 0.977。给定 CTA 的模型为了对准到达时间把速度压到失速——就是 L3.c 测过的机制（speed floor hook 为此而建）；T1a 臂没开 hook。
3. **计划 token 用得很少**：同一权重遮掉 token 雷达引导只差 150–210 m；从没见过缺席 token 的 p0 臂遮掉后崩到 5365 m（风险 2 的"过度依赖"在 p0 上成立，p50 的 dropout 起了作用）。
4. **即使完全消除漂移也过不了 G1**：one-shot 本身（不再问、无漂移）雷达引导 1761 / 1732 m、直线 290 / 296 m，离 1000 / 200 m 的门还远。one-shot 与只给真值时长的 L3_cta（雷达引导 1596 m，§10.3 引用）相比并没有更好——计划 token 加上这套随机锚点配方，没有把"知道计划"变成航迹精度。所以 T1c（滚动状态目标）按预注册"升为必做"，但它只针对漂移，**单独做它到不了 G1**。

**决定状态**：G1 不过、漂移成立 → 预注册的下一步是 T1c；但读法 4 说明 T1 的主要缺口不是漂移而是"学习型控制层没把计划变成精度"，这一点预注册没有覆盖，方向交用户定（见对用户的汇报）。J7（T2）照跑，结果补在下一节。
...
| T1a / T2 重发 | **J6 完：G1 不过（两种子，§10.9）**：再问漂移（receding 比 one-shot 雷达引导差 430–460 m）、失速导致不可飞（给定 CTA 的已知机制，未开 hook）、计划 token 只值 150–210 m；one-shot 本身也离门很远 → T1c 单独不够，方向待用户定。J7（T2，读 G3）运行中 | `…/two_tier_t1a_20260916/{lockstep_30s, gates_g1}` |
