...
| T1a / T2 重发 | **J6 训练完**（22:32 → 02:19，3 臂 180 epoch 无早停，每臂 ~74 min，manifest 全 completed @ 77e1372；val 1404/1404 解出）。选择指标（L−1 固定锚点共同网格 ADE）835.6 / 829.8 / 832.2 m（p50_s1337 / p50_s2024 / p0_s1337）。**一次成形预测（L−1 给真值计划 token + 真值 CTA，不再问，不是 G1 读数）**：雷达引导 ADE 均值 1764 / 1736 / 1735 m（p50 中位 1518 / 1489 / 1476），直线 298 / 305 / 303 m（239 / 239 / 246）——对照：规则制导 lockstep 1847 / 283，native32 一次成形 2870 / 445。**下一步**：runs worktree → 87f46cc，J6a lockstep（全部变体一个产物）→ J6b `two_tier_gates` 读 G1 → J7（`--plan-head`）读 G3 | `outputs/KRDU/experiments/two_tier_t1a_20260916/{T1a_*, T1a_*_pred_val}` |
