...
**KRDU 上的失效形状不同，要分开说**（见 `figures/imitation_mode_alignment.png`）：那里
两条第一模态的**形状**相关达 **+0.67**——转向最后进近那次转弯是学到了的。但
(a) 模型在 55–72 % 处多出一次**反向转弯**，真实航迹没有；(b) 逐航班技能 0.124 仍低于
下限 0.170。也就是说 KRDU 的问题不在"总体平均的转弯形状"，而在**逐航班的载荷**——
哪一班在什么时候转——外加一段凭空多出来的回转。KSJC 的近正交（0.062）是更极端的版本，
两者指向同一个缺口：控制量没有被监督。
...
### 12.5 把下限装进度量之后，实验 1 的结论要改一半

`score_control_arms.py` 现在为每个 arm 在**它自己的航班集合**上算出这两个参照物，
不再拿 1.0 当目标。同一批 KSJC arm 重新打印（其余列与已发布数字完全一致）：

| 指标 | velocity 0.5× | **2× (=simple-v2)** | 8× | 32× | 128× |
|---|---:|---:|---:|---:|---:|
| 逐航班 bank 技能 | 0.067 | **0.197** | 0.374 | **0.465** | 0.459 |
| 随机航班下限 | 0.312 | 0.312 | 0.312 | 0.312 | 0.312 |
| 孪生航班上限 | 0.543 | 0.543 | 0.543 | 0.543 | 0.543 |
| 直线 bank RMS | 1.59° | 0.79° | 0.40° | 0.46° | 0.34° |
| ADE 中位数 | 356.9 | **317.7** | 325.6 | 337.4 | 322.0 |
| FDE 均值 | 925.5 | **818.6** | 964.1 | 1111.8 | 1231.2 |

**冻进 `simple-v2` 的那一档（2×），恰恰是唯一一个 bank 技能低于随机下限的剂量。**
当时按 ADE/FDE 选剂量，而 bank 技能被拿去和 1.0 比，0.197 与 0.465 都"离目标很远"，
于是这个差别没有被看见。真实情况是：8×–128× 的 bank 技能落在下限与上限之间
（0.37–0.47，孪生上限 0.543），它们确实学到了结构，代价是 FDE 变差 18–50 %。

所以 §10「组合不叠加、velocity 2× 最优」这个结论只对**位置精度**成立；对**控制结构**
而言 2× 是最差的一档。这正是需要一个直接监督控制量的项的原因——把 bank 结构从
"用位置精度换"变成"直接教"。

### 12.6 KRDU 基线：同样的病，更重

换到 KRDU 后先训一条纯 `simple-v2` 基线（1404 条 validation 航班），指标如下：

| 指标 | KRDU simple-v2 | KSJC simple-v2 |
|---|---:|---:|
| 逐航班 bank 技能 | **0.124** | 0.197 |
| — 随机航班下限 | **0.170** | 0.312 |
| — 孪生航班上限 | **0.699** | 0.543 |
| 共用剖面占比 | 49.0 % | 17.3 % |
| 直线 bank RMS | 3.92° | 0.79° |
| ADE 中位数 | 655.7 m | 317.7 m |
| FDE 均值 | 1611.5 m | 818.6 m |

**同样低于下限**（0.124 < 0.170），而且共用剖面占比 49 % 说明近一半的 bank 能量还是
那条所有航班共用的剖面。KRDU 本身转弯更多（bank 总体标准差 5.59° vs KSJC 2.78°），
位置精度也差一倍——所以这里既是更难的任务，也是**上限更高**（孪生 0.699）、
更值得改的场景。

剂量标定（本文档统一用"相对位置项的倍数"）：在这条收敛基线上
（`val parts state` = 0.0417，未加权 imitation MSE = 0.0308），**w = 1.36 恰好是 1× 位置项**，
因此三条 arm 实为 **0.74× / 2.95× / 11.8×**。

### 12.7 剂量实验结果：直接监督控制量有效

KRDU validation 1404 条航班，`simple-v2` 基线 + 五个剂量（剂量 = imitation 项相对位置项的
倍数，w = 1.36 为 1×）：

| 剂量 | 共用剖面 | 直线 bank RMS | 变号 | bank 技能 | ADE 中位 | FDE 中位 |
|---:|---:|---:|---:|---:|---:|---:|
| **真实** | **3.2 %** | **0.55°** | **0** | — | | |
| 0（基线） | 49.0 % | 3.92° | 5.0 | 0.124 | 576.4 | 987.6 |
| 0.74× | 32.4 % | 1.13° | 1.0 | 0.449 | 562.4 | **915.4** |
| 1.47× | 40.2 % | 1.21° | 0.0 | 0.429 | 684.3 | 1040.7 |
| 2.94× | 31.0 % | 0.70° | 0.0 | 0.510 | 613.9 | 990.5 |
| 11.8× | 21.4 % | 0.54° | 0.0 | 0.591 | 523.5 | 957.3 |
| 11.8×（种子 2024） | 20.8 % | 0.58° | 0.0 | 0.572 | 525.2 | 973.2 |
| **47×** | **3.3 %** | **0.36°** | **0.0** | **0.735** | **501.0** | 987.0 |
| 参照：随机航班下限 | | | | 0.170 | | |
| 参照：同跑道孪生 | | | | 0.699 | | |

**47× 上共用剖面占比 3.3 %，与飞过的航迹 3.2 % 基本相同**——本文档 §3.2 提出的核心病症
（70 % 的 bank 能量是一条所有航班共用的剖面）在这一档上消失了。直线参考的 bank RMS
0.36° 也已低于真实的 0.55°，符号反转归零。

配对符号检验（基线 → 47×）：bank 技能 **96.6 %** 航班更好（p ≈ 0），**ADE 57.0 % 更好**
（p = 1.9e-7），FDE 54.3 % 更差（p = 1.2e-3，中位数 987.6 → 987.0，实际几乎不动）。

**这与 velocity 项的取舍性质完全不同**：velocity 剂量要拿到 bank 结构必须付出 FDE
变差 18–50 % 的代价（§12.5），而 47× 在把控制结构改到真值水平的同时 **ADE 反而更好**。
（"越强越好"只在 11.8× 以上成立；低剂量区见 §12.8。）

### 12.8 低剂量区是噪声平台，收益全在高剂量端

补上 1.47× 之后（填 0.74× 与 2.94× 之间的空档），低剂量区并不是一条平滑上升的曲线：

| 剂量 | 共用剖面 | bank 技能 | ADE 中位 | FDE 中位 |
|---:|---:|---:|---:|---:|
| 0.74× | 32.4 % | 0.449 | 562.4 | 915.4 |
| **1.47×** | **40.2 %** | **0.429** | **684.3** | **1040.7** |
| 2.94× | 31.0 % | 0.510 | 692.0 | 990.5 |

**1.47× 比它下面那一档还差**：配对检验里 0.74× → 1.47× 三项指标全负（bank 技能仅
27.1 % 航班更好，ADE 33.1 %）。它相对基线仍然大幅改善 bank 技能（0.124 → 0.429，
87.0 % 航班更好），但在剂量序列里是个凹点。

结合 2.94× 的精度凹陷，结论是：**0.74×–2.94× 这一段是一个抖动的平台，档与档之间的
差异并不构成可用的排序**；真正的收益集中在 11.8× 以上，而且从 11.8× 到 47× 仍在
大幅改善（技能 +0.144，共用剖面 21.4 % → 3.3 %）。这也解释了为什么最初按 1/4/16
的几何梯度取样是对的——如果只在低剂量区细分，会得出"这个项没什么用"的错误结论。

### 12.9 孪生参照不是上界，而且符号检验的 p 值不能当效应量读

两处必须写下来，否则后来的人会照着错误的方式读这张表。

**其一，47× 的 0.735 超过了孪生的 0.699。** 孪生匹配只用进入位置、航向和时长，而模型读的
是完整 120 s 回看轨迹——信息量本就不同。孪生是"未来的 bank 有多少是可预测的"这个问题的
**标尺**，不是任何模型的上界。原先在 `score_control_arms.py` 里写成"the best any predictor
could do"，已改。

**其二，配对符号检验在 1404 条样本上对纯种子噪声也给出 p = 3e-16。** 同一配置换种子：

| 比较 | Δbank 技能 | ΔADE 中位 | 更好比例 | p |
|---|---:|---:|---:|---:|
| **种子噪声**（11.8×，1337→2024） | −0.019 | +1.7 m | 39.1 % | 3e-16 |
| 剂量 2.94× → 11.8× | +0.062 | −88.7 m | 73.3 % | 1e-70 |
| 剂量 11.8× → 47× | +0.144 | −22.5 m | 85.5 % | 9e-172 |

所以**这张表里的 p 值只说明"方向可复现"，不说明"效应大"**，判断必须回到幅度上：
种子噪声让 bank 技能动 0.019、ADE 动 1.7 m，而剂量效应分别是它的 3–8 倍和 13–52 倍。
结论：剂量排序是真实的；2.94× 那个精度凹陷（ADE 613.9 m，而两个种子的 11.8× 都在
524 m 附近）也远超种子噪声，是曲线本身的结构，不是偶然。

### 12.10 `simple-v3`：冻结下来的生产配方

```
  simple-v3  =  simple-v2
                + control_imitation_loss_weight = 64.0     （≈47× 位置项）
```

**只差这一个字段**，其余全部继承 simple-v2（一阶滞后飞行模型、velocity 权重 0.003、
三个时间常数 1.5/2.0/0.8 s、d_model 512）。

在 1404 条 KRDU validation 航班上，相对 `simple-v2` 基线：

| 指标 | 真实 | simple-v2 | **simple-v3** |
|---|---:|---:|---:|
| 共用剖面占比 | 3.2 % | 49.0 % | **3.3 %** |
| 直线 bank RMS | 0.55° | 3.92° | **0.36°** |
| 直线段符号反转 | 0 | 5.0 | **0** |
| 逐航班 bank 技能 | — | 0.124 | **0.735** |
| — 随机航班下限 | | 0.170 | 0.170 |
| ADE 中位数 | | 655.7 m | **501.0 m** |
| FDE 中位数 | | 987.6 m | 987.0 m |

配对：bank 技能 **98.0 %** 航班更好（p ≈ 0），ADE **57.0 %** 更好（p = 1.9e-7），
FDE 持平。**这是本轮调查里第一个不需要用精度换结构的改动。**

**为什么是 47× 而不是 188×**：188× 的数字略好（技能 0.770，ADE 中位 495.2），但
(a) 控制拟合已饱和——未加权 imitation 项 0.00891 → 0.00827，仅 −7 %，而上一档是 −31 %；
(b) 已经**过冲**——直线 bank RMS 0.24°、共用剖面 3.0 %，双双越过真实的 0.55° 和 3.2 %，
即比真实航迹还平滑，那不是更准而是偏向另一侧。47× 是结构指标**落在实测真值上**的那一档。

**为什么不是低剂量档**：见 §12.8，0.74×–2.94× 是抖动平台，1.47× 甚至比 0.74× 更差。

### 12.11 仍未做的事（不要把结论说得比证据大）

- ~~只在 KRDU 一个机场验证过~~ **跨机场复现已做（KSJC，§12.12）：机制成立，剂量不通用。**
  仍只在 `val` split 上；outer-test 按开发期约定未读取。
- **47× 只跑了一个种子。** 11.8× 的双种子重复显示种子噪声很小（技能 ±0.019，
  ADE ±1.7 m，远小于剂量效应），据此推断 47× 的排名稳健，但那是推断不是实测。
- 时长头的 ~125 s 下限（§5）仍未修，与本节无关但同样未解决。

### 12.12 跨机场复现（KSJC）：机制成立，剂量不通用

KSJC 上跑同一对 arm（`simple-v2` 基线 vs `simple-v2` + 权重 64，逐字段等于 `simple-v3`），
1083 条 val 航班配对：

| 指标 | KSJC 飞行真值 | baseline | **simple-v3** |
|---|---:|---:|---:|
| 逐航班 bank 技能 | — | 0.197 | **0.678** |
| — 随机下限 / 孪生 | | 0.313 / 0.543 | 同左 |
| 共用剖面占比 | 3.2 % | 17.3 % | **6.4 %** |
| 直线 bank RMS | 0.53° | 0.79° | **0.18°** |
| 直线段符号反转 | 0 | 3.0 | **0** |
| ADE 中位 | | 284.2 | 289.5 |
| FDE 中位 | | 535.0 | 651.4 |

配对：bank 技能 **94.7 %** 航班更好（p = 1e-230）；ADE **无差别**（47.2 %，p = 0.068）；
**FDE 更差**（32.0 % 更好，即 68 % 更差，p = 9e-33）。

**机制复现了，而且更强**：技能不但跨过随机下限，还超过了孪生参照 0.543——与 KRDU 同向，
量级同样巨大。§12.1 提出的缺口（bank 从未被监督）在两个机场都成立。

**但"不付代价"只在 KRDU 成立。** 两个机场并排：

| | KRDU | KSJC |
|---|---|---|
| 技能 baseline → v3（下限） | 0.124 → 0.735（0.170） | 0.197 → 0.678（0.313） |
| 直线 bank RMS（真值） | 3.92 → 0.36°（0.41） | 0.79 → **0.18°**（0.53） |
| ADE | **改善** 57.0 %，p=1.9e-7 | 无差别，p=0.068 |
| FDE | 持平 | **变差**，p=9e-33 |

KSJC 的 v3 直线 bank 只有真值的 **三分之一**（0.18 对 0.53），过冲远比 KRDU 的 47×
（低 12 %）严重。这与 §12.4 已经测到的事实一致：**box 归一化后 bank 只占 imitation 项的
18 %（KRDU 41 %）**，同样的权重在 KSJC 上对 bank 的相对压制更强。

**所以 47× 是 KRDU 特定的剂量，不是跨机场的通用值。** `simple-v3` 冻结的是机制和一个
在 KRDU 上标定的数字；换机场需要重新标定，而 §12.4 那张通道占比表就是标定的依据。
这一点比原先预期的更重要，已写进 `config.py` 的选型注释与 `CLAUDE.md`。

### 12.13 更正：真值是逐机场的，之前用了 KSJC 的常量读 KRDU

`score_control_arms.py` 原先把 `TRUTH = {3.2 %, 0.55°}` 硬编码，而这两个数测自 **KSJC**
的飞行航迹。在 KRDU 上跑同一个脚本时这一列照常打印，于是 §12.6–§12.11 里所有"对真实
3.2 % / 0.55°"的表述都是拿 KSJC 的参照读 KRDU 的结果。

实测 KRDU 自己的飞行航迹：**共用剖面 1.8 %，直线 bank RMS 0.41°**（KSJC：3.2 % / 0.53 %，
与原常量吻合）。受影响的结论：

- "47× 共用剖面 3.3 % 对真实 3.2 %，病症消失" → 对真实 **1.8 %**，仍是 1.8 倍，
  从 49 % 大幅下降但没有归位；
- "47× 直线 bank 0.36° 低于真值 0.55°" → 真值 **0.41°**，只低 12 %；
- "188× 过冲" → 方向不变（0.24° 对 0.41°，低 41 %），幅度小于原先所述；
- **选 47× 的理由要改写**：188× 在共用剖面上其实略优（3.0 % vs 3.3 %），47× 胜在
  直线 bank RMS 离真值最近、拟合已饱和、FDE 均值更好。原先"结构指标正好落在真值上"
  的说法是错的。
- 技能、ADE、FDE 与全部配对检验**不受影响**（不依赖该常量）。

脚本已改为**逐 arm 实测真值**并作为「└ flown tracks」行打印，硬编码常量删除——跨机场
混排一张表时不会再犯同一个错误。
