...
| L3.f 参考 rollout 估计 surplus（只预测） | **跑完（2026-09-09）——空结果，且暴露阈值穿越规则在三边正横误判**：雷达引导截断点 96–99 % 距阈值 > 1 km（横偏中位 8.7 km）；L3.e/L3.f 的雷达引导数字无效，"10 km 横偏"撤回；规则修复在建（`dev-cut-rule`），修后两者重跑 | `l3f_path_stretch_ref_20260909/readout.json` | 待规则修复后重读 |
