...
（更长预算 + 26× 时长权重），并按 B 线门 3 的方式把 z 的样本扇形当分布读（真值落入扇形的份额、扇形最近解的 chamfer）——它与 C 线
"轨迹分布"的目标是同一件事。
