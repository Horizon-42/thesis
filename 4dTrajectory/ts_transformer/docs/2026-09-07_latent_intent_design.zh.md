...
| L2 CVAE 骨架（隐意图 z） | **量纲测试臂 L2z_units 跑完（2026-09-08，`l2_units_test_20260908`，180/180 未早停）——量纲论断成立，z 第一次活了**：打乱 z 的 ΔADE **+928 m**（雷达引导 +2444；此前七臂 0–60 m），minADE_6 1021 < N(0,I) 对照 1043（首次），先验总 σ 0.848（L2d 0.58），8 维全活跃；top-1 ADE 1249（native32 1322、L2d 1214），**FDE p50 688 vs 864**（雷达引导 1519 vs 1982，胜率 66–70 %）；位移中位 0.334σ（门 1σ 未过，p90 1.04）——两条候选判据分裂，打乱判据压倒性通过；最佳轮 180 = 预算受限。**L2.g 跑完（2026-09-08 晚）：四道门两种子全过**——ADE 1230 / 1194（native32 1322，种子差 37 m）、FDE p50 656 / 627（864）、打乱 z 代价 +1017 / +935 m、先验 σ 0.87 / 0.90；**但 U1 揭示 1 km 量纲摧毁坡度保真（native32+量纲 bank 0.400，L2.g 0.456/0.475，L2.g 的门没测 bank）**；**L2.h（教师 2000）跑完：bank 0.494 未过门（RMS 0.29° 反优于 native32，共同剖面 11.5 %），z 退化（打乱 +283、FDE 817）→ 隐变量线定位为带坡度代价的终点/分布仪器，主线保持 10 km 量纲**，见 §六 L2 末 | `l2_units_test_20260908/latent_readout_L2z_units.{json,txt}`、`probe/`、`readout.json` | 见 §六 L2 末 |
...
| L3.e 路径拉长 hook（只预测） | **重跑完（2026-09-09 晚，切点规则修正后，`l3e_path_stretch_20260909r`）——五门未过四**：+0 完全可飞 46.2 %、+60 s 20.0 %（失效版的 78 / 52 % 作废，旧目录已删）；+60 s 时 29.9 % 的航班（雷达引导约 80 %）从未进门、rollout 继续飞几十 km；`tromboneStretchM` 中位 0；CTA 精确服从、X = 0 | `l3e_path_stretch_20260909r/<arm>_pred_val/flyability_report.json`、`*_states.json` | 门 1–4 未过、门 5 过；见 §六 L3.e-r / L3.f-r 结果 |
| L3.f 参考 rollout 估计 surplus（只预测） | **重跑完（2026-09-09 晚，`l3f_path_stretch_ref_20260909r`）——不再是空结果，但两道新门未过**：对 L3.e-r 失速 / 推力越界样本 −9–13 %、到五边份额 +7–10 个百分点、雷达引导 \|xt\| p95 −23–29 %、偏移 0 的 `tromboneDelayS` p50 386 → 136 s（门 ≤ 30 s）；完全可飞逐位同 L3.e-r；A-4 核查已跑（`l3f_a4_check_20260909`）：优势成立，聚合指标变动 < 1 %，锚点已建立的 842 条逐位不变 | `l3f_path_stretch_ref_20260909r/<arm>_pred_val/…`、`l3f_a4_check_20260909/` | 门 6、7 未过；A-4 保留意见撤销；hook 线关闭 |
...
**扇形读数（2026-09-08 晚，决定 4(a)，`run_ts_latent_fan_readout.py`，B 线门 3 同一协议；`latent_fan_<arm>.{json,txt}`）**：
truth 到 top-1 的 chamfer p50 / 到六个先验样本中最近者 / 最近先验样本优于 top-1 的航班份额 / 到六个 N(0,I) 样本中最近者 / 其优于 top-1 的份额 / 先验扇形端点最大展开 p50：

| 臂 / 分层 | cham top-1 | 先验最近 | 份额 | 随机最近 | 份额 | 展开 |
|---|---:|---:|---:|---:|---:|---:|
| L2g 1337 全体 | 162 | **124** | **0.915** | 201 | 0.348 | 794 |
| L2g 1337 雷达引导 | 822 | **669** | 0.903 | 1101 | 0.330 | 1228 |
| L2g 2024 全体 | 158 | **124** | **0.928** | 166 | 0.343 | 821 |
| L2g 2024 雷达引导 | 804 | **650** | 0.913 | 1324 | 0.197 | 1110 |
| L2z_units 全体 | 178 | 131 | 0.926 | 210 | 0.380 | 765 |

读法：**先验扇形在三臂每个分层都决定性地优于随机扇形**——最近先验样本在 92 % 的航班上优于 top-1，随机 z 的扇形只有 35 %（六条任意路径
"白拿"的水平）；雷达引导层 669 vs 1101 m。样本确实携带锚点前方那架飞机的信息。两点保留：对照是 N(0,I) 而非同分布打乱（部分差距可能是
随机 z 落在解码器训练区之外；`shuffled/` 的 +935…+1017 m 是互补对照，一致）；扇形是读数不是覆盖率保证，展开 p50 约 800 m、雷达引导
1.1–1.3 km——与 B 线 ±70 s 的宽度是同一件事的米制表达。top-1 ADE / minADE_6 与 `latent_readout_*.json` 逐行一致。

**U1 量纲单测（2026-09-08 夜，决定 4(c)，`u1_units_only_20260908/U1_native32_units`，native32 + 1 km 量纲、无隐变量，180/180，最佳轮 174）——FDE 收益约三成五来自量纲，
但量纲**摧毁坡度保真**：**

| | native32 | **U1（只改量纲）** | L2g 1337 | L2g 2024 |
|---|---:|---:|---:|---:|
| ADE 全体 / 直线 / 雷达引导 | 1322 / 445 / 2870 | 1270 / **497** / 2631 | 1230 / 430 / 2639 | 1194 / 406 / 2579 |
| FDE p50 全体 / 直线 / 雷达引导 | 864 / 671 / 1982 | **786** / 561 / 1671 | 656 / 494 / 1449 | 627 / 497 / 1225 |
| chamfer 全体 | 224 | 227 | 162 | 158 |
| **bank skill**（门 0.70） | **0.726** | **0.400** | **0.456** | **0.475** |
| 直线参考 bank RMS（观测 0.41°） | 0.34° | **1.60°** | 0.65° | 0.71° |
| 共同剖面份额（观测 1.8 %） | 2.6 % | **31.7 %** | 25.0 % | 22.9 % |
| 时长 MAE | 25.9 | 24.9 | 24.7 | 24.3 |

读法：(1) FDE 收益的归因：全部 208–237 m 里量纲单独给 78 m（≈ 35 %），隐变量再给 130–159 m（≈ 65 %），两部分都超过 ~60 m 的 FDE 种子线。
(2) 量纲单独对 ADE 是尾部效应（均值好 52 m 但配对胜率只有 40.6 %、直线进近 497 vs 445 变差）。(3) **最重要且预注册没料到的**：把位置损失尺度改到
1 km 让坡度保真坍缩——bank skill 0.726 → 0.400、RMS 0.34° → 1.60°（比 L1 无教师的 dense 臂还差）、共同剖面 31.7 %；L2.g 两臂同样受损（0.456 / 0.475，
共同剖面 23–25 %），只是**L2.g 的四道门都没测 bank**，这个代价直到 U1 配上 `score_control_arms` 才显形。机制是 L1 的老教训：10 km 量纲下模仿项是
位置项的 ~16 倍（0.55 vs 0.035），1 km 量纲下位置项 2.3–2.5、模仿项 1.1–1.2，比例掉到 0.5——教师不再给坡度命名，共同剖面回来了。隐变量部分修复了
这个损伤（0.400 → 0.456/0.475，RMS 1.60 → 0.65/0.71），三个量纲臂的时长 MAE 都更好（24.3–24.9 vs 25.9）。
**修正 L2.g 的结论：隐变量线目前过不了其它每条线都在用的 bank 门（0.70），在修好之前不是主线组件，是"终点与分布"的仪器，带坡度代价。**

**L2.h 预注册（2026-09-08 夜，`l2h_units_teacher_dose_arms.json`，campaign `l2h_units_teacher_dose_20260908`）**：L2.g 配方 + `control_imitation_loss_weight`
2000（= 64 × ~31，把模仿/位置比例放回 10 km 平台的 ~14–16×；剂量曲线的噪声平台是 11.8×–47×），300 轮关早停，种子 1337，predict 同 L2.g，先跑一臂。
**门**：bank skill ≥ 0.70 **且** L2.g 的四道门全部保持。**否决**：bank < 0.60（剂量修不好）或打乱 ΔADE < 200 m（加重的教师又把 z 杀死）。过门则跑其无隐变量
孪生臂（native32 + 量纲 + 2000）重新归因；不过则隐变量线定位为带坡度代价的 FDE/分布仪器，主线配方保持 10 km 量纲。排在 L3.d、A2b 之后。

**L2.h 结果（2026-09-09 凌晨，`l2h_units_teacher_dose_20260908/L2h_units_imit2000`，300/300，最佳轮 195）——门未过、否决触发；剂量朝对的方向动，但不够。**

| | native32 | U1（量纲，无 z） | L2g 1337 / 2024 | **L2h（量纲 + 教师 2000）** |
|---|---:|---:|---:|---:|
| bank skill（门 0.70，否决 < 0.60） | 0.726 | 0.400 | 0.456 / 0.475 | **0.494** |
| 直线参考 bank RMS（观测 0.41°） | 0.34° | 1.60° | 0.65° / 0.71° | **0.29°** |
| 共同剖面份额（观测 1.8 %） | 2.6 % | 31.7 % | 25.0 / 22.9 % | **11.5 %** |
| 打乱 z ΔADE 全体 / 雷达引导 | — | — | +1017 / +2702 · +935 / +2406 | +283 / +513 |
| minADE_6 vs N(0,I) 对照 | — | — | 994 < 1030 · 959 < 1035 | 1159 > 1132（败） |
| FDE p50 全体 | 864 | 786 | 656 / 627 | 817 |
| top-1 ADE 全体 | 1322 | 1270 | 1230 / 1194 | 1327 |
| 损失分量（最佳轮） | — | — | state 2.3 / imit 1.1 | imit 21.9 / state 4.0 |

读法：(1) **加重教师确实修坡度形状**——RMS 0.29° 已优于 native32，共同剖面从 23–32 % 减半到 11.5 %；但 bank skill 只到 0.494：坡度平滑、
形状对，却不是每架飞机自己飞的坡度（逐航班相关失分），仍是"干净的罐头剖面"。(2) **z 活着但退化**：打乱代价 1017 → 283 m，先验样本又输给
标准正态（门 2 翻回不过），FDE 656 → 817。剂量用坡度保真换掉了隐变量质量与终点，两头都没过门。(3) 按臂文件的决定规则落在失败分支：
**隐变量线定位为带坡度代价的终点/分布仪器（L2.g 配方），主线配方保持 10 km 量纲；教师剂量轴不再加臂。** 交付上这又是"两模型"：路形与坡度
用 10 km 配方（native32 / L2d），终点分布与样本扇形用 L2.g。无隐变量孪生臂不跑。

**决定（用户 2026-09-08 晚，U1 之前）**：隐变量线以 L2.g 配方**重新成为主线组件**——z 是"给定历史下的未来分布"，与 B 线的分位数扇形、C 线的轨迹分布是同一交付物。
...
> **用户决定（2026-09-08 晚，六项）**：(1) 雷达引导 ETA 宽度的否决按**读法 (b)** 处理：B 线对直线进近成立，雷达引导的到达时刻本来就有
> ±70 s 的不确定性（控制器尚未决定何时转向），区间只是如实报出；雷达引导的时间改由调度程序指定（B3 已验证）。这是对预注册否决规则的
> 事后修正，如实标注。(2) 多锚点交付：**先两模型（L−1 用固定锚点臂、之后用随机锚点臂，A2a），后混合锚点训练（A2b）**，优先级提高。
> (3) 推迟吸收：**先做动力学里的速度下限（失速裕度）**。(4) 隐变量后续：**先做 (a) 扇形读数（只读）与 (c) native32 上单测量纲（U1，
> `u1_units_only_arms.json`），再做 (b) CTA 叠 L2.g**；整体优先级提高。(5) L1.d 暂不做。(6) KSJC 复现放最后。
> 执行顺序：U1（GPU 1 h，今晚）→ 扇形读数与 A2a 组合曲线（CPU）→ A2b 采样轴与速度下限的代码（并行，评审后预注册）→ 4(b) → KSJC。
...
2. **延时的吸收方式与 hook 无关**：两个 +60 臂的逐带地速差几乎相同（10–5 km 减速约 2 m/s），路径长度比同为 1.30；20–10 km 段横向
   RMS 约 171 / 57 / 57 m（hook / 无 hook / 参照；2026-09-09 更正：原记 1–3 m 是弧度当角度的 57 倍低估）——仍**不是蜿蜒**，1.30 的长度比不是 171 m 的横向 RMS 能解释的。
...
推到汇入前，而是压进了模型本来就偏慢的外段。**下一个设计项（用户决定 2026-09-08 晚：先做 (a)；已建成 `control/constraints/speed_floor.py`，命令 hook `speed-floor` 与组合 hook
`barrier+speed-floor`，裕度 1.10 取自包内两处既有常数但按当地密度与指令载荷计算、实际最紧；预注册 L3.d = `l3d_speed_floor_arms.json`，
L3_cta checkpoint 在偏移 0/+30/+60/+90 s 套 `barrier+speed-floor`，四个只预测臂；门：(1) 进近段失速样本较 L3.c hook 臂减少 ≥ 90 %，
(2) +60 s 完全可飞份额 ≥ 观测地板 98.4 % − 10 点，若因下限而提前到则报未吸收延时 X 不设门，(3) 横向 \|xt\| p95 ≤ 1.5× 偏移 0）**：
(a) rollout 的速度下限（V ≥ V_stall(n)·裕度，包线进动力学而不是只进指标），使外段偏慢不可能发生；(b) 汇入前的路径拉长自由度（trombone），使延时有合法去处；(c) 训练期 LPV 项换到
CTA 底座（L1.c 的 CTA 版）。

**L3.d 结果（2026-09-08 夜，`l3d_speed_floor_20260908`，`barrier+speed-floor`，四个只预测臂）——三道门全未过：下限换来另一种违反。**
偏移 0 完全可飞 8.3 → **45.4 %**、+30 s 0.3 → 21.2 %、+60 s 0 → **2.4 %**、+90 s 0.1 %；失速样本反增 94–139 %（+60 s：66,980 → 134,977）；新出现 `thrust_over_max`（+0：97,929，+60：113,254 个样本；此前为 0）；下限在 50–70 % 的步上生效、22–30 % 饱和，
平均推力改动随偏移 0.33 → 0.48；CTA 仍精确服从（未吸收延时 X = 0）。读法：下限要求的加速需要机体没有的推力——要么推力饱和、速度没抬起来、
失速仍在，要么抬起来了但推力越界；同时两极分化（三分之一的航班变完全可飞，其余违反更重）。**总时长由 CTA 固定、路径固定 → 平均速度固定，
下限单独没有地方放延时。**

几何与截断读数（配对 1402）：**下限臂的终点几何是灾难，但那是阈值之后的尾巴**——偏移 0 的 ADE 840 → 3443、终点 \|xt\| p95 1611 → **43,420 m**
（+60 s：61,375 m），chamfer 反而 70 → 77：rollout 以下限速度提前到达阈值、继续飞并转弯（时长头把总时间钉在 CTA 上，下限不许它减速到点）。
按阈值截断后，进近段本身是**改善**的：失速样本在剩余 ≥ 20 km 带从 L3.c 的 ~53,500 降到 ~36,200（−32 %，正是诊断所指的那段），
阈值后的失速从 ~14,800 升到 ~103,700；截断后完全可飞 +0：45 → **62.8 %**，+60 s：2.4 → **48.9 %**（L3.c 1.35 %）——门 2 的 88.4 % 仍未过，但差距是
40 点而不是 86 点。结论修正：**下限在进近段是有效的，代价是推力越界与"提前到达后的尾巴"；延时（偏移 0 时则是模型自身依赖的不可行减速）
必须换成五边前的路径长度——这正是 L3.e 的 trombone；同时所有带 hook 臂的可飞性/几何读数必须按阈值截断（predict 加
`--truncate-at-threshold`，L3.e 起使用，L3.d 按截断值重读）。**


#### L3.e — 汇入前路径拉长的自由度（2026-09-08 夜预注册；用户决定 (b)，排队列末尾；hook 在建）

**机制**：`trombone` 命令 hook（推理期，可组合为 `barrier+speed-floor+trombone`）：每个命令步算出"以下限速度飞剩余路径吸收不了的延时"，
把它换成**五边之前**（走廊门关闭的那段）的横向路径延长——有界坡度的航向偏置、按所需额外距离保持、再转回，载荷协调、滞后补偿、
软/硬饱和同其它 hook；on-final 门内不动。臂 `l3e_path_stretch_arms.json`（L3_cta，偏移 0/+30/+60/+90，三 hook 叠加，只预测）。
**门**：(1) `thrust_over_max` 样本 = 0（拉长吸收了下限吸收不了的部分）；(2) 进近段失速样本较 L3.c 减少 ≥ 90 %；(3) +60 s 完全可飞 ≥ 88.4 %；
(4) 五边横向包容同 L3.c，且拉长发生在 on-final 门外（门内横向 RMS 不变）；(5) CTA 精确服从，否则报未吸收延时 X。**否决**：+60 s 处门 1 或门 2
不过。**决定规则**：全过 → 推迟侧 CTA 参考以三 hook 叠加交付；1–2 过 3 不过 → 剩余不可飞在别处，读分项；1 不过 → 下限与拉长互相干扰，重设计。

**hook 已建成（2026-09-08 夜，分支 `dev-trombone`；未合并、未跑臂）。** `control/constraints/trombone.py`。
每个命令步读三样东西——调度自身的剩余时间（`RolloutStateView.remaining_s`，新增；CTA 条件下即 `T_cta − t`）、
runway 轴里到跑道口的直线距离 `D`、以及速度下限模块在同一步会守住的那个 `V_floor`（`speed_floor.floor_speed`，
抽成一处定义，两个模块不可能各算各的）——延时与它值多少路径长度是同一个恒等式：
`V_e = max(V_floor, V_h)`，`Δt_未吸收 = T_r − D/V_e`，`ΔL = V_e·T_r − D`。把 `ΔL` 花成绕直线的折线，偏角
`cos θ = D/(V_e·T_r)`——"用剩余时间飞完恰好落在跑道口"的那个航向。**速度取"正在飞的速度、以下限兜底"，
这个 max 不是细节，它是折线能收尾的原因**：下限只抬推力，所以 `V_h > V_floor` 才是常态；只按 `V_floor` 定
（第一版设计）在那里恰好失效——飞机比估计跑得快，ΔL 不再下降（实测指令推力 0.12 时盈余从 2981 m 起涨后走平），
过半点永不触发，外飞段一直持续到跨过跑道口平面，落在中线外 2.9 km。改用 `V_e` 后 `dΔL/dt = −V_e + V_h·cos θ ≤ 0`
对任何飞行速度都成立。于是整条律是状态的函数：花掉多少 θ 就自己缩多少，回到直线由恒等式保证而不是靠计时器；
**没有延时要吸收的航班，软硬两种饱和下命令都逐位不变**。
折线只朝一侧（在进入时锁存为飞机当前所在的一侧，因此只会加宽、不会横穿延长中线），到"剩余 ΔL 降到进入时的一半"
再镜像回来——不需要累加器，因为 ΔL 本身就是按买到的额外长度在消耗。坡度用屏障那套滞后补偿反解并协调载荷。

预注册留白的三个设计点，及其定论：

1. **交接规则**：只在"预测航迹与跑道航向夹角 > `ALIGNMENT_MAX_DEG`(30°)"处**开启**折线。该条件严格包含在
   "on-final 门关闭"里（门本身要求对准），所以 hook 永远不可能在五边上动手，也主动放弃了"门外但已对准"
   那一类状态（在包容锥之外、机头已指着五边）——那正是屏障一进锥就会把折线转回去的地方。对准只管**开启**、
   不管**延续**：折线的外飞段本身会把机头扫过跑道航向，若在那里噤声，飞机会停在偏 40° 且没有回程段的状态上
   （在 fixture 上实测：4 步就停，然后平行掠过跑道口外 6.6 km）。门一旦开过，该航班整条 rollout 内 hook 永久失效；
   `d ≤ 0` 之后也不动。**代价已测且不藏：约 58 % 的机队根本拉长不了。** KRDU 抽样 2998 架、L−1 锚点、观测航迹：
   58.9 % 在锚点即对准 30° 内，58.4 % 自锚点起每一行都对准，57.1 % 锚点就在门内。这些是直线进近，在宽松的包容锥下
   从几十公里外就算"在五边上"。它们的未吸收延时以 `commandHookDiagnostics.tromboneDelayS`（配 `tromboneEngagedSteps = 0`）
   报出——这是门 (5) 旁边该引用的数字，而不是放松规则的理由。
2. **转弯坡度上限 15°，由速度下限定，不是由舒适度定。** 词表唯一的保证是"值里 `+` 的顺序就是施加顺序"，
   所以 `barrier+speed-floor+trombone` 下，下限定价的是网络给的载荷因子、**不是** trombone 随后加上的这个转弯。
   转弯两头吃裕度：协调后的载荷把失速速度抬 √(1/cos μ)，它带来的诱导阻力（~tan²μ）又是下限的推力没算过的。
   在 rollout fixture 上实测（48 段、下限速度飞的四边、约 6 s 保持、V_floor 64 m/s 对 58 m/s 失速）：
   10° 上限时失速余量 −0.6 m/s，15° −0.9，20° −1.5，25° −3.3，30° 出现第一个失速样本。买路径的是**偏角**上限（45°）
   而不是坡度上限——降到 10° 落点都几乎不变——所以压低坡度不花钱，换来约 2 倍余量；两个上限各有自己的计数
   （`hook_trombone_saturated_steps` / `hook_trombone_bank_capped_steps`），"上限到底多久生效"在臂上可读。把组合顺序倒过来让下限
   精确定价才是更好的物理，但那会让一个值的拼写与它实际做的事不一致，那更糟。
3. **`hook_saturation` 软化的是界，不是模式。** 交接规则、折线的方向、过半点这三样在软硬两种形式下都是硬判据；
   `soft` 软化的是坡度饱和、门的混合权重，以及一条在"零盈余处严格为 0 且斜率为 0"的盈余斜坡——所以"没延时就不动"
   在臂实际使用的 soft 形式下同样成立。

**同时新增 `predict --truncate-at-threshold`**（`forecast.cut_at_threshold_crossing`）：把每条记录截在
**第一次**跨过跑道口平面（`d ≤ 0`）那一段里离目标最近的一行。理由是 L3.d 的几何读数：带下限的 rollout 会**提前**
到达跑道口然后继续飞（终点 |xt| p95 43–63 km、偏移 0 处 pooled ADE 840 → 3443 m），整条记录上取的可飞/几何/CTA
读数全在给落地之后的飞行打分；只看进近段，同样的下限臂在 +60 s 处完全可飞 1.35 % → 48.9 %。截断后 `final_time_s`
随之前移——这正是目的：提前到达不再隐形，而变成它本来就是的 `final_time_error_s`。永远到不了跑道口的预测**不截**，
并照实写 `truncatedAtThreshold: false`。**L3.e 四臂都已加上该开关，因此上面五道门都是在截断后的记录上评的；
与之比较的 L3.c / L3.d 数字必须先按同样方式重读，比较才成立。**

**L3.e 结果（2026-09-09 凌晨，`l3e_path_stretch_20260908`，`barrier+speed-floor+trombone`，四个只预测臂，记录按阈值截断）——切点规则失效版：下表与其读法保留为历史记录，数字不可引用，目录已删；修正后的结果见下文 L3.e-r / L3.f-r。**

| 臂 | 完全可飞 | 失速样本 | 推力越界 | 载荷过低 | 可飞采样率 | ADE | chamfer | \|xt\| p95 | 横向违反率 | 到阈值份额 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| L3.d +0（对照，截断前） | 45.4 % | 119,705 | 97,929 | 628 | 0.78 | 3443 | 77 | 43,420 | 43.8 % | — |
...
门：(1) 失速 −90 %：+0 −92 % 过，+60 −84 %（对 L3.d）擦线未过；(2) +60 完全可飞 ≥ 88.4 %：**52.4 % 未过**；(3) 五边横向 p95 ≤ 1.5× 偏移 0：过（但偏移 0
本身 10 km）；(4) 推力越界：−85 %；(5) CTA 精确服从、X = 0（每个偏移每个分位数）。trombone 诊断：**61.5 % 的航班零动作**（与"L−1 时已在五边"
的 58 % 一致）；动作的航班里 `tromboneDelayS` p50 = **391 / 421 / 451 / 480 s**——每 30 s 偏移恰增 30 s，延时被完整换成路径长度。

读法：(1) **机制成立**：延时改由路径长度吸收后，下限撞推力上限的问题消失（推力越界 −85 %、失速 −84 %、载荷过低 −99 %），偏移 0 完全可飞
从 45 % 到 78 %，+60 s 到 52 %，chamfer 是 L3 家族最好的（56–75 m）。(2) **剩余损伤是横向的、来自过度拉长**：`tromboneDelayS` 在偏移 0
就有 391 s——真值 CTA 下本不该有"吸收不了的延时"。原因是估计量用**到阈值的直线距离**算路径所需时间，而雷达引导航班的真实/预测路径（三边、
四边）远长于直线，于是把整段引导都当成"多余时间"去拉长：终点横偏 p95 10 km，偏移 0 有 46 % 的航班在 CTA 时刻没飞到阈值，ADE 4.4 km。
(3) 修法明确：拉长量应对**模型自己预测的剩余路径长度**（无 hook 的参考 rollout，名义律 hook 已有并行参考 rollout 的机制）而不是直线距离——
**L3.f（用户决定 2026-09-09，预注册，`l3f_path_stretch_ref_arms.json`，campaign `l3f_path_stretch_ref_20260909`）**：只改估计量——surplus = 剩余时间 −
无 hook 参考 rollout 的剩余路径在 V_e 下所需时间（复用名义律 hook 的并行参考 rollout），config 轴 `trombone_surplus_reference ∈ {beeline, reference-rollout}`，
...
**L3.f 结果与 L3.e/L3.f 的失效发现（2026-09-09 下午，`l3f_path_stretch_ref_20260909`）**：L3.f 是空结果——四个偏移的完全可飞份额与 L3.e
逐位相同（77.99 / 63.98 / 52.35 / 45.94 %），两道新门未动（偏移 0 的 `tromboneDelayS` 390 vs 391 s，\|xt\| p95 10,174 vs 10,162 m）；新诊断量
`tromboneRefPathM` 中位 ~12 km（25 km 的进近）、偏移 0 时 31.6 % 无参考穿越。**根因不在估计量，在阈值穿越的判定规则**（"沿航道 d ≤ 0 的第一段里的
最近点"）：雷达引导航班飞三边时与航道平行反向、横向偏数公里，在跑道正横处经过 d = 0，规则在那里判定"穿越"。实测（截断点在阈值坐标系里的位置）：
L3.e +60 s 的雷达引导截断点 **96.5 % 距阈值 > 1 km**（横偏中位 8.7 km、高出阈值 1.75 km），直线进近干净（p95 50 m）；L3.f +0 为 99.4 %。
后果：(1) **L3.e/L3.f 里"10 km 横偏"是正横截断点的偏置，不是 trombone 拉过头**——此前"剩余损伤是横向的"的读法撤回；(2) 雷达引导航班的可飞率
与几何算在到三边中段为止的窗口上，L3.e/L3.f 的雷达引导数字无效，直线进近与已建立分层不受影响；(3) 参考 rollout 的 L_ref 也量到正横点，
L3.f 的估计量从未得到正确输入。修法：穿越 = 第一个同时**在 on-final 门内**（成员锥 + 航向对齐）且 d ≤ 0 的行，`--truncate-at-threshold` 与
trombone 的参考共用一处实现（在建，`dev-cut-rule`）；修好后 L3.e 与 L3.f 各重跑四个只预测臂（2 × 20 min）再读门。此前 L3.e 关于
推力越界/失速/载荷的相对改善（都是对 L3.d 的比较，两者用同一窗口）方向可信，绝对数字待重读。


**L3.e-r / L3.f-r 结果（2026-09-09 晚，切点规则修正后各重跑四个只预测臂；`l3e_path_stretch_20260909r`、`l3f_path_stretch_ref_20260909r`，切点 = 第一个在 on-final 门内且 d ≤ 0 的行）。修正后的数字比失效版差得多，这正是修正的意义：旧切点把每条雷达引导进近的后半段丢掉了。**

切点验证（`<arm>_pred_val/*_states.json` 末状态对 `*_eval.json` 的 `target_state`，L3.e-r +60 s）：984 条截断记录里终点 \|xt\| > 1 km 的份额 **0.0 %**（失效版 96.5 %）；直线进近 887 条 \|xt\| p95 **47 m**，雷达引导 94 条 p95 494 m。`L3e_stack_p90s_pred_val/evaluation_report.json` 独立给出横向均值 30.9 m、p95 53.5 m。

到五边份额（`source.truncatedAtThreshold`；"未截断" = rollout 在门内跨越之前结束——没有延时时可能是调度先用完——或从未进门，此表不分；+30 s 以上的 22–31 % 更接近"从未进门"，雷达引导分层内约 80 %）：

| 偏移 | L3.e-r 截断 / n | 未截断 | L3.f-r 截断 / n | 未截断 |
|---|---:|---:|---:|---:|
| +0 | 427 / 1404 | 69.6 % | 521 / 1404 | 62.9 % |
| +30 | 989 / 1402 | 29.5 % | 1089 / 1402 | 22.3 % |
| +60 | 984 / 1404 | 29.9 % | 1087 / 1404 | 22.6 % |
| +90 | 965 / 1404 | 31.3 % | 1084 / 1404 | 22.8 % |

可飞（`<arm>_pred_val/flyability_report.json`，全部记录）：

| 臂 | 完全可飞 | 失速样本 | 推力越界 | 载荷过低 | 采样 | 可飞采样率 |
|---|---:|---:|---:|---:|---:|---:|
| L3.e-r +0 | 46.15 % | 111,414 | 88,782 | 597 | 780,672 | 0.79 |
| L3.e-r +30 | 31.81 % | 115,815 | 91,060 | 878 | 830,865 | 0.80 |
| L3.e-r +60 | 20.01 % | 116,962 | 90,118 | 814 | 863,169 | 0.80 |
| L3.e-r +90 | 13.53 % | 113,709 | 87,759 | 1,649 | 898,567 | 0.81 |
| L3.f-r +0 | 46.15 % | 101,183 | 77,941 | 438 | 748,784 | 0.80 |
| L3.f-r +30 | 31.81 % | 100,870 | 75,713 | 531 | 788,318 | 0.82 |
| L3.f-r +60 | 20.01 % | 105,123 | 77,886 | 627 | 819,247 | 0.81 |
| L3.f-r +90 | 13.53 % | 102,900 | 74,823 | 1,035 | 843,330 | 0.82 |

完全可飞两臂逐位相同：完全可飞的全是 trombone 从不动作的直线进近，两臂在那里逐位相同，而没有一条雷达引导航班在任一臂下完全可飞。L3.f-r 的失速 / 推力越界样本少 9–13 %，载荷过低少 27–43 %，采样少 3–4 %。失效版的 +0 采样是 389,909，修正后 780,672——这就是"丢掉后半段"的大小。

几何（`compare_constraint_arms` → `l3f_path_stretch_ref_20260909r/readout_ef.txt`，配对 1404；ADE / chamfer / 终点 \|xt\| p95 / 五边横向违反率）：直线进近 L3_cta 392 / 66 / 1066 / 75.9 %，L3.e-r +0 415 / 42 / **58** / 12.7 %，L3.f-r +0 420 / 42 / 58 / 12.7 %，两臂 +60 s 相同 499 / 51 / 50 / 76.4 %（\|Δ时长\| p50 14.5 s）；雷达引导 L3_cta 1596 / 721 / 3593 / 99.4 %，L3.e-r +0 10,321 / 3384 / 47,855 / 100 %，L3.f-r +0 10,133 / 3436 / **33,918** / 99.1 %，L3.e-r +60 9,605 / 3371 / 58,521，L3.f-r +60 9,472 / 3201 / 44,916。**雷达引导行是全部 497 架**——含从未进门、rollout 继续飞出几十 km 的约 80 %——不是 94 架截断子集，与 L3.d 的雷达引导数字不可比；直线进近在两臂间除 +0 的 5 m ADE 外逐位相同（参考 surplus 对已对准的航班无事可做）。

trombone 诊断（截断记录的 `source.commandHookDiagnostics`）：

| 臂 | n | 时长 MAE | `tromboneStretchM` p50 | `tromboneDelayS` p50 | `tromboneRefPathM` p50 | `refNoCrossing` |
|---|---:|---:|---:|---:|---:|---:|
| L3.e-r +0 | 427 | 0.00 s | 0 | 385.6 s | — | — |
| L3.e-r +60 | 984 | 60.00 s | 0 | 438.7 s | — | — |
| L3.f-r +0 | 521 | 0.00 s | 0 | **135.5 s** | 13,442 m | 64.3 % |
| L3.f-r +60 | 1087 | 60.00 s | 0 | **136.1 s** | 13,595 m | 87.0 % |

门。L3.e 的五道（L3.e-r +60 s）：(1) 失速对 L3.d −90 %：**未过**（116,962 对 134,977，−13 %）；(2) +60 s 完全可飞 ≥ 88.4 %：**未过**（20.01 %）；(3) 五边横向 p95 ≤ 1.5× 偏移 0：**未过**（全体 43,771 对 37,352）；(4) 推力越界：对 L3.d −20 %，对自身 +0（88,782）持平，**未过**；(5) X = 0、时长 MAE 恰 60.00 s：**过**。L3.f 的两道（L3.f-r +0）：(6) `tromboneDelayS` p50 ≤ 30 s：**未过**（135.5 s，失效版 390 s）；(7) 终点 \|xt\| p95 ≤ 3222 m：**未过**（全体 12,492 m）。

读法：(1) **失效版的所有可飞与几何读数作废**：完全可飞 +0 78.0 → 46.2 %、+60 s 52.4 → 20.0 %；本文件此前引用这些数字的地方（阶段报告、设计文档）以本节为准。(2) **参考 rollout 估计量不再是空结果**：对 L3.e-r 失速 / 推力越界 −9–13 %、到五边 +7–10 个百分点、雷达引导 \|xt\| p95 −23–29 %、`tromboneDelayS` 386 → 136 s 且不再随偏移每 30 s 增 30 s。但评审 A-4（软饱和下屏障的 30–40° 对准肩带正是 trombone 的准入带，`docs/2026-09-09_package_review_bugs_and_architecture.md`）在两臂上都在，两臂之差有一部分可能来自屏障门而非估计量——见下面的核查。(3) **两臂几乎所有门都未过，剩余问题已不是正横伪影**：约 30 % 的航班（雷达引导约 80 %）从未进门、rollout 继续飞几十 km，`tromboneStretchM` 中位 0 说明中位航班没有得到任何拉长——hook 没有作用在最需要它的航班上。这与 §十 的判断一致：拉长自由度应由规划层的路线构造给出（`docs/2026-09-09_plan_and_guidance_design.md` §4.2），而不是由推理期 hook 事后补。

**A-4 修复与预注册核查（2026-09-09 晚）**。A-4 在 `dev-pg` 修复（`c544db0`）：composite 里有 trombone 时屏障限定在硬门内（门内逐位不变，门外静默；硬饱和下是恒等）。两次重跑都在修复前的代码上跑，重叠带对读数的影响未测。预注册一次**核查**（不是新 campaign）：用修复后的树只重跑 `L3f_ref_p60s` 一臂（只预测，约 20 min，campaign `l3f_a4_check_20260909`），与 L3.f-r +60 s 配对比较——(a) 完全可飞、失速 / 推力越界样本、到五边份额、雷达引导 \|xt\| p95、`tromboneDelayS` p50；(b) 预期：直线进近逐位不变（门内不变），雷达引导只在 30–40° 肩带上变化；(c) 判读：若 L3.f-r 对 L3.e-r 的优势在核查臂上消失，该优势归因于屏障门而非估计量；若不变，归因于估计量成立。结果写回本节。

**核查结果（2026-09-09 晚，`l3f_a4_check_20260909/L3f_ref_p60s_pred_val`，修复后的树 `d235180`，对 `l3f_path_stretch_ref_20260909r/L3f_ref_p60s_pred_val` 配对 1404）。归因于估计量成立；聚合指标变动都在 1 % 以内。**
逐记录 SHA-256（`*_states.json`）：锚点已建立的 842 条 **0 条不同**（"门内不变"精确成立）；直线进近 904 条中 62 条（6.9 %）不同——直线进近按曲折度定义，其中锚点尚未建立的航班起步在硬门外，那里屏障现在静默，所以"直线进近 ⇒ 不变"是错的表述，"锚点已建立 ⇒ 不变"才是数据支持的；雷达引导 497 条中 395 条（79.5 %）不同，即修复作用的地方。
聚合（`flyability_report.json`，修复后 / 修复前）：完全可飞 20.01 / 20.01 %；失速 105,859 / 105,123（+0.7 %）；推力越界 78,684 / 77,886（+1.0 %）；载荷过低 657 / 627；截断 1,075 / 1,087（雷达引导 187 / 197）；雷达引导切点 \|xt\| p95 521 / 520 m；`tromboneDelayS` p50 134.9 / 134.9 s；雷达引导 `tromboneStretchM` p50 142 / 142 m（全体 p50 0）；时长 MAE 60.00 / 60.00 s。
判读：L3.f-r 对 L3.e-r 的优势在修复后的树上成立——到五边 +6.5 个百分点（修复前 +7.4）、失速 −9.5 % / 推力越界 −12.7 %（修复前 −10.1 / −13.6）、`tromboneDelayS` 438.7 → 134.9 s；上一节的 A-4 保留意见撤销。两点记下：把屏障限定在硬门内**略有代价**而不是收益（到五边少 12 架、失速/推力样本多约 1 %，方向一致但很小）——正确性换来的，不是要回退的理由；中位航班仍几乎没有拉长，这才是实质缺口，A-4 与它无关。
