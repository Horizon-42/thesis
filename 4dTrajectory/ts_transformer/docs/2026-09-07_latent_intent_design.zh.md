...
| L1.c 训练期 LPV 走廊项（L1.b 胜者上） | **预注册（2026-09-08，用户决定，优先级提前）**：09-05 的否决是带教师 64 测的，无教师底座上未测；两训练臂（λ 2e-4 平价 / 1e-3 同剂量）+ 两只预测臂（软屏障 hook 在底座 / 在平价臂）；排在 L1.b hr16 之后、L3 反事实之前 | `l1c_procedure_arms.json` | 违反率 −5 点 ∧ 雷达引导 ADE ≤ +30 m ∧ bank ≥ 0.70；否决 雷达引导 ADE > +100 m；见 §六 L1.c |
...
| L3.c 推迟吸收进走廊（只预测） | **预注册（2026-09-08，用户决定）**：L3_cta checkpoint 在偏移 0/+30/+60/+90 s 套软屏障 hook，问延时被减速吸收、推到汇入前、还是吸收不了（提前到）；排在 A0.b 之后 | `l3c_delay_corridor_arms.json` | 走廊违反率 ≤ 偏移 0 hook +5 点 ∧ +30 s 时长误差 30±10 s ∧ 可飞率 ≥ 8.3 %；见 §六 L3.c |
