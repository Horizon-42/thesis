...
| L3.e 路径拉长 hook（只预测） | **hook 已建成、臂可跑（2026-09-08 夜，`dev-trombone`）**：`control/constraints/trombone.py` + 组合 `barrier+trombone` / `barrier+speed-floor+trombone`；另加 `predict --truncate-at-threshold`（读数改在进近段上取，四臂都已加）。等价性对 `617539a`：1094 个非计时叶子全同、354 个存盘配置 0 改名。**已知代价：约 58 % 的航班在锚点就已对准五边（KRDU 抽样 2998 架：58.9 % 锚点对准 / 58.4 % 全程对准 / 57.1 % 已在门内），这些航班拉长不了**，其未吸收延时以 `tromboneDelayS` 报出 | `l3e_path_stretch_arms.json` | 推力越界 = 0 ∧ 失速 −90 % ∧ +60 s 可飞 ≥ 88 % ∧ 五边包容 ∧ CTA 服从；见 §六 L3.e |
