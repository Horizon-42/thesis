...
| L3.e 路径拉长 hook（只预测） | **预注册（2026-09-08 夜，用户决定，队列末尾；hook 在建）**：`barrier+speed-floor+trombone` 三 hook 叠加，延时换成五边前的横向延长 | `l3e_path_stretch_arms.json` | 推力越界 = 0 ∧ 失速 −90 % ∧ +60 s 可飞 ≥ 88 % ∧ 五边包容 ∧ CTA 服从；见 §六 L3.e |
