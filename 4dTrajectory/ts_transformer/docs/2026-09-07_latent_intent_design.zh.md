...
| L3.f 参考 rollout 估计 surplus（只预测） | **重跑完（2026-09-09 晚，`l3f_path_stretch_ref_20260909r`）——不再是空结果，但两道新门未过**：对 L3.e-r 失速 / 推力越界样本 −9–13 %、到五边份额 +7–10 个百分点、雷达引导 \|xt\| p95 −23–29 %、偏移 0 的 `tromboneDelayS` p50 386 → 136 s（门 ≤ 30 s）；完全可飞逐位同 L3.e-r；A-4 重叠带未测，核查臂已预注册 | `l3f_path_stretch_ref_20260909r/<arm>_pred_val/…` | 门 6、7 未过；A-4 核查待跑 |
