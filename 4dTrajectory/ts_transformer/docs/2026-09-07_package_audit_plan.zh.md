...
| 19 ✅`dba2450` | `__main__.main`（714 行）→ `cli/{train,cross_validate,evaluate_fit,freeze_test,predict}.py`，各暴露 `HELP` + `add_cli_arguments()` + `run_cli()`；`__main__.py` 131 行 = bootstrap + `COMMANDS` 表。**15 个与字段异名的标志全部改名**（见 CHANGELOG），60 行 `cli_values` 手写映射变成 54 个字段名的 `CLI_CONFIG_FIELDS` + import 时断言 | 两套 config 表面靠手写映射。`train`/`cross-validate` 逐字相同的 75 行队列装配折成 `common.prepare_training_run` → `TrainingRun` |
| 20 ✅`2fc5560` | `dynamics/backends.py` 三个后端类折成一张 `(endpoint_fn, dense_fn, post_fn, runs_hooks)` 表；`_TransportChartResults` 删除（它是写了两遍的 `post_fn`）；`_refuse_hook` 六处调用变成 dataclass 上的一个 `_admit` | 451 → 437 行——行数不是重点，重点是新增一对要提供的东西现在写在注册表里。位精确用探针测过：三个注册对（外加带 barrier hook 的滞后对）的 endpoint + dense，13 个张量摘要与反传梯度，6 行全部逐字节相同 |
