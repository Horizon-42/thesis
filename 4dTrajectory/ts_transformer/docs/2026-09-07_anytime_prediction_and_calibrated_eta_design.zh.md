...
| A2b 混合锚点训练 | **跑完（2026-09-08 夜）**：份额 0.3 / 0.5 的 L−1 ADE 1504 / 1464（门 1447；0.5 在种子线内），冻结点 8 km 保持（24.6 / 29.5 s），否决干净；0.5 配对处处优于 0.3；两模型规则仍是交付形态 | `a2b_l1_share_20260908/readout.json`、`anytime_a2b_20260908/` | L−1 门按数字未过（0.5 差 17 m）、冻结点过；见 §2.4f 末 |
...
| B1 分位数时长头 | **跑完（2026-09-08，`b1_quantile_20260907/B1_quantile`，`73d829f`，164 轮早停）**：对预注册参照 native32 全分层更好（ADE 1282 vs 1322，直线 420 vs 445，雷达引导 2805 vs 2870，胜率 56 %；时长 MAE 23.9 vs 25.9 s）；**门 1 两条款过**；`B1_point_matched` 第二种子 1373（第一种子 1248）——26× 时长权重的 74 m **不复现、不采纳**；种子散布 125 m = 噪声线的 4 倍；分位数头的时长精度（23.9 / 直线 10.3 s）存活；对 L2d（隐变量臂，仅作上下文）劣 68 m；变动超种子噪声 → 预注册的对照臂 `B1_point_matched` 触发，已加入臂文件待跑 | `b1_quantile_arms.json`、`readout_vs_native32.json`、`eta_error_vs_native32.json` | 门 1 过；见 §3.5 |
...
| B1.b 双头时长 | **跑完（2026-09-08，单种子，180 轮）**：q50 MAE 24.2 / 直线 10.4 s（复现分位数头，MAE 条款过）；但直线 ADE 461、直线 FDE 687（否决）、chamfer 232 四臂最差——两头不叠加。**不采纳。** B 线交付 = B1_quantile 分位数头 + B2 校准 + B3 CTA 解码 | `b1b_two_head_20260908/{readout,eta_error}.json` | MAE 过、ADE 不过；见 §3.1b 结果 |
...
### 2.4f A2b：把一部分抽签留给 L−1（2026-09-08 用户决定 (b)，代码已建，未训练）
...
**A2b 结果（2026-09-08 夜，`a2b_l1_share_20260908`，回放 `anytime_a2b_20260908`，两臂 180/180 未早停，最佳轮 169 / 160）——冻结点保持，L−1 门差一点，否决干净。**

| | native32 | A0b path_uniform | **A2b 份额 0.3** | **A2b 份额 0.5** |
|---|---:|---:|---:|---:|
| L−1 全体 ADE（门 ≤ 1447） | 1322 | 1782 | 1504（差 57） | **1464（差 17，在种子线内）** |
| L−1 全体 FDE p50 / chamfer | 864 / 224 | 1477 / 229 | 1272 / 212 | **1117 / 199** |
| L−1 直线 / 雷达引导 ADE | 445 / 2870 | 628 / 3839 | 574 / 3149 | **556 / 3071** |
| s_freeze（\|Δt\| p80 < 30 s） | 未达到 | 8 km（19.8 s） | **8 km（24.6 s）** | **8 km（29.5 s，压线）** |
| 12 km 雷达引导 ADE p50（否决 > +100） | — | 561 | 584（+23） | 596（+35） |
| 门 1 单调 / 门 2 | — | 过 / 16 km | 过 / 16 km（1368） | 过 / 16 km（1360） |
| 实际 L−1 抽签份额 / 无 L−1 锚点航班 | — | — | 0.302 / 5 | 0.501 / 5 |

读法：(1) 混合锚点做到了它的目的——收回随机锚点化在 L−1 的代价的约三分之二（1782 → 1464），冻结点、单调性与 12 km 否决都保持。
(2) **0.3 与 0.5 在训练侧网格指标上打平（628.8 / 627.8），配对读数上 0.5 处处更好**（ADE、FDE、chamfer、每个分层）：网格指标对锚点集平均，
抹掉了 L−1 上的差别——它是选择指标，这点要记住。(3) 0.5 的代价是冻结点余量只剩 0.5 s；再提高份额大概率丢掉 8 km 冻结点。
(4) **L−1 门按数字两臂都未过**（0.5 差 17 m，在 125 m 种子线内，不可分辨）。**决定（建议）**：A2a 两模型规则仍是交付形态；A2b 0.5 作为
"单模型近似"记录，不再加份额臂（0.7 会丢冻结点）；若要单模型，下一步是第二种子确认 0.5 的 1464，而不是新的份额。
...
**`B1_point_matched_s2024`（2026-09-08 下午，第二种子，143/180 早停，最佳轮 123）——74 m 不复现，且种子散布是噪声线的四倍。**

| | native32 | point_matched 种子 1337 | point_matched 种子 2024 |
|---|---:|---:|---:|
| ADE 全体 / 直线 / 雷达引导 | 1322 / 445 / 2870 | 1248 / 413 / 2721 | **1373 / 451 / 3004** |
| FDE p50 全体 / 直线 | 864 / 671 | 877 / 582 | 937 / 659 |
| 时长 MAE 全体 / 直线 | 25.9 / 13.6 s | 25.1 / 13.1 | 26.2 / 13.9 |
| 对 native32 的 ADE 配对胜率 | — | 61.4 %（中位 −26 m） | 46.8 %（中位 +9 m） |

- **26× 时长权重不采纳**：两个种子 1248 与 1373 跨在 native32 的 1322 两侧，没有证据说明它改善 ADE；时长 MAE 也跨在 25.9 两侧。
  B1_point_matched 单种子得出的"ADE 收益来自时长权重"**撤回**。
- **存活的一半**：分位数头的时长精度——B1_quantile 23.9 / 直线 10.3 s 优于两个点估计种子（25.1、26.2 / 13.1、13.9）和 native32，
  直线进近的 3 s 差在两种子散布（0.85 s）之外；全体的 1.2–2.3 s 差在散布（1.1 s）边缘。
- **方法论结论（比这个臂本身重要）**：预注册用的 30 m 种子噪声线来自 state 路径的两种子（5–22 m），对 control 路径的 180 轮臂偏小
  四倍——两种子在全体 ADE 上差 **125 m**（其中一部分是收敛差：1337 跑满 180 轮、2024 在 143 轮早停）。今天所有单种子、差距在
  ~125 m 以内的 ADE 结论（B1 的 −40、hr8+TV 的 +10、A0.b path_uniform 的 −20..−90、L5_fitted64 的 −42）都降为"未证明"；
  时长 MAE 的种子散布约 1 s。写入契约 §六 与 CLAUDE.md：**control 臂的 pooled ADE 差 < ~125 m 不作为证据，门要么两种子要么按此线读**。
- 对 B1.b 的门 1 的影响（事后修正，如实标注）：ADE 条款改为"不劣于两个 point_matched 种子中较差者（1373）且不劣于 native32 超过种子线"，
  MAE 条款不变；B1.b 已在训，读数按此。
- 更公平的复现应像 A0.b 的 p180 那样关早停；两种子不足以判定哪个种子"典型"。

**B1.b 结果（2026-09-08 下午，`b1b_two_head_20260908/B1b_two_head`，180/180 未早停，最佳轮 163，单种子）——MAE 条款过、ADE 条款不过；两头不叠加。**

| | native32 | B1_quantile | B1_point_matched（种子 1337 / 2024） | **B1b_two_head** |
|---|---:|---:|---:|---:|
| ADE 全体 / 直线 / 雷达引导 | 1322 / 445 / 2870 | 1282 / 420 / 2805 | 1248 / 413 / 2721 · 1373 / 451 / 3004 | 1307 / **461** / 2799 |
| FDE p50 全体 / 直线 | 864 / 671 | 860 / 595 | 877 / 582 · 937 / 659 | 899 / **687** |
| chamfer 全体 | 224 | 205 | 177 · 211 | **232** |
| 时长 MAE（实现时长）全体 / 直线 | 25.9 / 13.6 | 23.9 / 10.3 | 25.1 / 13.1 · 26.2 / 13.9 | 25.6 / 13.4 |
| **q50 MAE** 全体 / 直线 | — | **23.9 / 10.3** | — | **24.2 / 10.4** |
| 部署覆盖率 α=0.2 / 0.5（单切分） | — | 0.745 / 0.450 | — | 0.748 / 0.452 |
| 雷达引导 80 % 宽度 | — | 142 s | — | 143 s |

损失分量（最佳轮）：final_time 0.164、duration_quantile 0.075——两个头都在承重。读法：(1) **MAE 条款（预注册）过**：q50 的 24.2 / 10.4 s
与 B1_quantile 的 23.9 / 10.3 差 0.3 / 0.1 s，双头完整复现了分位数头的时长精度。(2) **ADE 条款（事后重定）不过**：全体 1307 在种子线内，
但直线进近 461 是四臂最差、直线 FDE 687 触发常设否决、chamfer 232 是 B 家族里唯一劣于 native32 的——它继承了分位数头的时序、
丢掉了点估计头的路径，两头没有把各自的优势叠起来。(3) 门 2 α=0.2 单切分 0.748 与 B1_quantile 的 0.745 同一处擦线（五切分探针已证明
是切分噪声）；雷达引导宽度 143 s，否决同前。**决定：B1.b 不采纳、不再加种子。B 线交付形态 = B1_quantile 的分位数头（ETA 分布，B2 校准）
+ B3 的 CTA 条件解码；路径模型另选（26× 时长权重已撤回）。**
...
（ADE 1282 / 420 / 2805 vs 1322 / 445 / 2870，FDE p50 860 / 595 / 1935 vs 864 / 671 / 1982，配对胜率 55.6–55.8 %）；MAE 条款**过**——
时长 MAE 全体 **23.9 vs 25.9 s**（直线 10.3 vs 13.6，雷达引导 45.4 vs 45.0，剩余 < 13 km 8.8 vs 11.8），且 native32 的系统性偏晚（带号 Δt 中位 +3.5 s）
在 B1 消失（+0.1 s）；\|Δt\| p50 全体 11.5 s、直线 7.3 s、雷达引导 38.8 s。**门 1 两条款都过。**第一次读数误以 L2d 为参照得出"全分层劣 68 m"，已更正；对 L2d 的劣势只作上下文。
...
**`B1_point_matched`（2026-09-08，180/180 未早停，最佳轮 175，`final_time_loss_weight` 26.0，点估计头）——分辨结果：两种收益来自不同机制、互不重叠。**

| 分层 | native32 ADE / FDE p50 / chamfer | B1_quantile | **B1_point_matched** | 时长 MAE：native32 / **B1_quantile** / point_matched |
|---|---|---|---|---|
| 全体 | 1322 / 864 / 224 | 1282 / 860 / 205 | **1248** / 877 / **177** | 25.9 / **23.9** / 25.1 s |
| 直线进近 | 445 / 671 / — | 420 / 595 | **413** / 582 | 13.6 / **10.3** / 13.1 s |
| 雷达引导 | 2870 / 1982 / — | 2805 / 1935 | **2721** / 2046 | 45.0 / 45.4 / 43.7 s |

- **ADE 的收益来自时长项权重，不是分位数头**：点估计头在 26 倍时长权重下全分层优于 B1_quantile（1248 vs 1282，chamfer 177 vs 205），
  比 native32 好 **74 m**——一个配方改动白拿的量，但单种子、且 180 轮仍在下降；主线配方是否采纳待第二种子。
- **时长 MAE 的收益来自分位数头**：point_matched 的 MAE 25.1 / 直线 13.1 s 几乎不比 native32 好，B1_quantile 23.9 / **10.3 s**（直线 −24 %）。
  把时长损失加重不买时长精度，预测分布取中位数才买。
- point_matched 在终点上略差（FDE p50 877 vs 860，雷达引导 2046 vs 1935）：它赢在沿路，不在终点。
- **候选下一臂（待用户决定）B1.b 双头**：点估计头（权重 26，驱动 rollout 时长）+ 分位数头（只出 ETA 分布与校准区间），两种收益
  同时拿；成本一个训练臂。以及主线配方的 `final_time_loss_weight` 提升需第二种子确认后再改 simple-v3。
