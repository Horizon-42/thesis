...
- **The third actuator is a load factor under every law EXCEPT `specific-force+path-angle`**, where it is
  ...
  the config refused the pairing (design §14.9). The hooks still read it blind, so every hook stays
  refused under that contract (`CONTROL_PARAMETERIZATION_SCOPES`).
...
| assessing a hierarchical / two-tier model (a short-horizon control layer re-asked under a plan layer, segment tokens, a multi-aircraft graph layer), or reading a model's error against LEAD TIME | `docs/2026-09-16_two_tier_transformer_feasibility.zh.md` (§2: three evaluation protocols that must never be mixed — a 60 s ADE is low for EVERY model; §4: the hierarchy bounds the compounding term but not the intent-limited plan term; §6–7: gates and the T0–T4 plan; §10: execution — T0(b) open-loop chaining loses, T0(c) longer history carries nothing (G2 fails, no T3); §11: the path-angle contract merged and the contracts refactored into one row each). Readouts: `run_ts.py lead_time_error`, `chain_sensitivity`, `tracker_lockstep` (`--plan-head` for T2) |
