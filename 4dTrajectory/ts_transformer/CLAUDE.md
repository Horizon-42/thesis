...
| assessing a hierarchical / two-tier model (a short-horizon control layer re-asked under a plan layer, segment tokens, a multi-aircraft graph layer), or reading a model's error against LEAD TIME | `docs/2026-09-16_two_tier_transformer_feasibility.zh.md` (§2: three evaluation protocols that must never be mixed — a 60 s ADE is low for EVERY model; §4: the hierarchy bounds the compounding term but not the intent-limited plan term; §6–7: gates and the T0–T4 plan). The readout is `run_ts.py lead_time_error` |
