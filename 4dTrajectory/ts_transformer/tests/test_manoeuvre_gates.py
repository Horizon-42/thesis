...
def _payload(*, vectored_ade: float, straight_ade: float, flyable: float, established: float,
             at_vectored=(500.0, 900.0), at_straight=(200.0, 400.0), name="arm") -> dict:
    def cell(ade, at):
    ...
    return {"executor_name": name, "strata": {
...
def test_gate_x_reads_flyable_established_against_the_guidance_and_the_two_ade_lines():
    good = _payload(vectored_ade=1200.0, straight_ade=200.0, flyable=0.97, established=0.85)
    ...
    weak = _payload(vectored_ade=1200.0, straight_ade=200.0, flyable=0.97, established=0.80)
...
def test_gate_p_open_loop_and_the_discrete_vs_continuous_control():
    open_loop = _payload(vectored_ade=0, straight_ade=0, flyable=1, established=1, at_vectored=(800.0, 1500.0), at_straight=(400.0, 600.0))
    assert g.gate_p_open_loop({1337: open_loop, 2024: open_loop})["passes"]
    late = _payload(vectored_ade=0, straight_ade=0, flyable=1, established=1, at_vectored=(800.0, 1530.0), at_straight=(400.0, 600.0))
