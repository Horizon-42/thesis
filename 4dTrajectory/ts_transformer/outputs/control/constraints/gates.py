...
class RunwayAxesView:
    ...
    vertical_speed: torch.Tensor    # [B] chart vu (+ up); the height rate over a hold
...
def runway_axes_view(state: RolloutStateView, runway_heading_rad: torch.Tensor) -> RunwayAxesView:
    ...
    hold = state.duration_s.to(chart.dtype).clamp(min=_HOLD_FLOOR_S)
    return RunwayAxesView(
        d=d, xt=xt, heading_error=heading_error,
        cos_align=cos_align, ground_speed=ground_speed, speed=speed,
        path_angle=torch.atan2(vu, ground_speed), vertical_speed=vu, height=u,
        mass=chart[:, 6],
