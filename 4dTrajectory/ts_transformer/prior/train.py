...
over the aircraft-steps the prior speaks at (`model.asked_entries`: from the first predicted step on), per such step.
Design §9 step 1: every scene is one flight."""
...
def to_batch(split: Split, indices: Sequence[int], device: torch.device) -> dict[str, torch.Tensor]:
    ...
    candidate slots: features, relative, static, in_force, since, targets, airport, present, edges."""
    ...
    for b, f in enumerate(flights):
        ...
        targets[b, 0, : f.rows], present[b, 0, : f.rows] = f.targets, True
    tensors = {"features": features, "relative": relative, "static": static, "in_force": in_force, "since": since,
               "targets": targets, "present": present, "airport": np.array([f.airport for f in flights], dtype=np.int64)}
...
def column_nll(logits: list[torch.Tensor], targets: torch.Tensor, present: torch.Tensor) -> torch.Tensor:
    """Summed negative log-likelihood per column over the aircraft-steps the prior speaks at: [6]."""
    asked = asked_entries(present)
    return torch.stack([nn.functional.cross_entropy(logit[asked], targets[..., c][asked], reduction="sum")
...
def evaluate(model: Prior, split: Split, config: TrainConfig, device: torch.device) -> dict[str, Any]:
    ...
    for indices in batches(split.flights, config.tokens_per_batch, None):
        ...
        total += column_nll(batch_logits(model, batch), batch["targets"], batch["present"]).double()
        steps += int(asked_entries(batch["present"]).sum())
...
def train(model: Prior, train_split: Split, val_split: Split, config: TrainConfig, device: torch.device,
    ...
    for epoch in range(1, config.max_epochs + 1):
        ...
        for indices in batches(train_split.flights, config.tokens_per_batch, rng):
            ...
            asked = int(asked_entries(batch["present"]).sum())
            loss = column_nll(batch_logits(model, batch), batch["targets"], batch["present"]).sum() / asked
