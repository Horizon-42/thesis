...
def state_token(row: np.ndarray, origin: np.ndarray) -> np.ndarray:
    """The six scaled features of one chart row, the position taken from ``origin`` (the
    threshold's chart row, `FlightSeries.target_chart`). A row slower than
...
def flight_sequence(series: FlightSeries, reading: Reading) -> InstructionSequence:
    ...
    states = state_tokens(series.supervision_times, series.supervision_values, reading.positions_s, series.target_chart)
    ...
    return InstructionSequence(
        ...
        typecode=str(series.scenario.aircraft.code), runway_course_rad=float(series.scenario.target.psi), ends_at_landing=True,
    )
...
def rolled_sequence(truth: InstructionSequence, reading: Reading, flown_times: np.ndarray, flown_values: np.ndarray,
    ...
    return InstructionSequence(
        ...
        typecode=truth.typecode, runway_course_rad=truth.runway_course_rad, ends_at_landing=len(said) == truth.length,
    )
