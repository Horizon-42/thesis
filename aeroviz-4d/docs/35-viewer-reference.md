...
- 三维的几个补充（2026-09-24）：航迹下面贴地画一条**地面投影**——贴地的包络只能对着它读，斜着看时空中的航迹
  ...
  左栏遮住了画面的一部分），之后相机归用户。
- **最快和最慢的转弯**（两条转弯路径，转弯区就夹在它们之间）在三维和平面图里都画，有自己的开关
  `trainingLayers.turnPaths`（面板 "turn paths"，和候选跑道一样独立于横向包络的开关）：贴地画，颜色跟转弯区的判定
  （不过为红，最快的那条沿着转弯区的边走），最慢的转弯转不完时画虚线；包括截获转弯。
