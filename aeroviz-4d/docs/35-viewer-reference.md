...
各不相同，一起亮就把没对齐的几段画成了"同一时刻"。改之前是按步高亮：点航向带，高度管也跟着变黄。
...
- 每类词亮什么（一个词对应它那一列的第几个包络，解析器核对过一一对应）：航向词→它自己的转弯区、转弯可能
  结束的平行四边形、漏斗（截获之后也是它自己的，走廊属于许可）；进近的"许可"词→截获转弯、走廊及其中线、
  截获后的航道带；高度词→它的管子；跑道词→所指的跑道和延长中线；下降角词和速度词在三维里没有位置上的包络，
  只画生效段——读数窗口里下降角词亮它在高度图上的竖线，速度词亮它的过渡和速度带。
...
- 三维：航迹用 `signals.altitudeHaeM`（椭球高）；高度管子是一面墙，沿飞机自己的地面航迹，上下沿为导出的
  ...
  一次换好（h = H + N，EGM96）。
- 三维的几个补充（2026-09-24）：航迹下面贴地画一条**地面投影**——贴地的包络只能对着它读，斜着看时空中的航迹
  和地面上的包络有视差；贴地多边形画不出轮廓线，所以每个贴地包络另画一条**贴地边线**，带标注器的判定，与平面图
  一致（红 = 判定不过，虚线 = 不判的保持段、截获转弯）；每根管子另画**上下沿两条线**——±25 m 的墙在任何正常距离
  都缩成航迹下面的一条缝；**选中一架航班时相机取景一次**（`frameTrajectoryCamera`，框住航迹和每个转弯区——大转弯最慢的边界伸出航迹
  十几公里——留 1.5 倍余量，因为句子条和
  左栏遮住了画面的一部分），之后相机归用户。
- **开关分开、默认只画看得懂的**（用户 2026-09-24："只有最慢和最快转弯速率这两条曲线是可以理解的"）：原来的
  `lateral` 拆成 `turnRegions`（转弯区，默认关）、`holdFunnels`（保持漏斗，默认关）、`corridor`（走廊，默认开）；
  `turnPaths`（默认开）画最快和最慢的转弯（贴地，颜色跟转弯判定，最慢的转不完时虚线，含截获转弯）以及转弯可能
  结束的平行四边形（含截获转弯的，`captureTurnEnd`）。航向图的保持带是航向容差，总是画。三维右下角有图例
  （`TrainingLegend`），只列打开的开关。
- **保持判定弱**：判了、但漏斗起始宽度超过 `TRAINING_WEAK_HOLD_WIDTH_M`（2 km，读数阈值）的保持段，边线画点线
  （三维 dashPattern 0x3333，平面图 "1 3"），航向图标题与句子条头部写明。大转弯后漏斗从十几公里宽的"可能结束
  的位置"出发，这样的判定说明不了航向保持得好不好；判定本身不变。
