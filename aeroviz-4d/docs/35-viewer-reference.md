...
（2026-09-21 傍晚，词表 `segment-v14` → `box-v2-wedge`；本条取代了原来那版"走廊"的写法）。

三件事，都会让人以为界面坏了：

**一、Training 页上某个集合读不了，多半是产物被取代了，不是读者坏了。** 读者按名字钉住读法
（`TRAINING_READING_RULE = "box-v2-wedge"`）和 schema（`aeroviz-training-sample-v2`），对不上整份
...
- **高度词是一个目标 + 它的后向可达楔形**（往上张 1.5°、往下张 1.0°，段末收到目标的 ±5 %）。
  它是这份词表里唯一一个**绝对的位置界**，三维里画成一面 Cesium `wall`
  （`altHaeLoM` / `altHaeHiM`，**HAE**；这一版 `Hi` 就是上界，不像上一版的走廊要反着读）。
- **航向词和速度词只给位移界**：一个词生效期间它们允许的那块地是有界的，画成一个长方体
  （`polygon` + `extrudedHeight`）。它们**很窄**（4 s 的保持、2° 的航向盒 ≈ 十来米横向），
  这个窄本身就是结论 —— 横向上一个词几乎什么都没说。
- **它是推出来的可达集，不是词本身**：词约束的是每一时刻的状态，不是路径。
- **没有"按规则飞出来的那条橙线"了**：飞一句包围盒需要跟高度的执行器（回放门，未做）。

**三、判决算在平滑信号上，不是原始行上。** 盒子是从滑动平均上读出来的（航道 6 s、速度和高度 10 s），
同一条航迹算在平滑线上 **100 %** 在盒子里、算在原始行上 **93 %**。导出器写 `readCourseDeg` /
`readSpeedMps` / `readHeightM` 三列专供判决，原始列画在它们后面（只画平滑线等于画了一条没人飞过的
信号）。**航向是在 wrapped 上平滑的**，跨 ±180 会把 +179° 和 −179° 平均成 0° —— 标注器自己的算术，
照抄不改。

**产物的标注器不在本仓库**，所以导出器那一侧的盒子是**照产物的 spec 重建的**；顶替"和原件对一遍"的是
包含率算两遍（导出器一遍、前端拿着列再一遍），对不上就拒整份文件。五机场 200 架 39 670 / 39 670 行
全在盒子里，是这个重建目前唯一的凭据。
