...
| **T6b ← 下一步** | 两条航迹进三维（白 / 橙）；导出器补经纬高，**并在出口处把 MSL 换成 HAE**（Cesium 的 `cartographicDegrees` 是椭球高，V27） | 导出器 + 新 hook `useTrainingTrackLayer` | 切换显示开关，两条线能单独开关；三维里的高度与地形、与观测 CZML 对得上（差 ~33 m 就是这次换算漏了） | T6a |
...
**进度（2026-09-21 凌晨）**：T1 / T2 / T3 / T4a / T4b / T5 / T8 已完成，导出已跑两次（第二次带上了几何航迹）。下一步 T6（两条线进三维）。T7 仍等阶段 B3′ 的先验产物。
