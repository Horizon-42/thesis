...
| `geometric.*`（T5 落地时定死的） | `manoeuvre/instruction_kinematics.py`。**改了三处字段名，都是为了它们不会被读错**：没有 `endsAtThreshold`（`endReason` 已经说了）、没有 `lon/lat/altM`（跑道坐标系那几路才是平面图要的，经纬高在 T6）、`crossTrackP95M` 改叫 **`gapP95M`**（它是**两条线之间距离**的 p95，不是几何航迹自己的横向偏移）。另加 `comparedS` / `comparedFraction`：几何航迹提前停时，平均差只覆盖了进近的一部分，**覆盖了多少必须写出来** |
| `geometry`（顶层一块） | `instruction_kinematics.assumptions()`，§5.4 那六条假设 + 常数来自哪几个文件 |
...
**进度（2026-09-21 凌晨）**：**T1–T6、T8 全部完成**，导出跑了四次（几何航迹、修正过的到达判定、经纬高）。**只剩 T7**，它等阶段 B3′ 的先验产物，不是这个模块能推进的。
