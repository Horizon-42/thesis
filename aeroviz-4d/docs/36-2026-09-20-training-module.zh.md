...
- **怎么启动**：先在句子条上点一个色块选中一个词，再按句子条头部的蓝色按钮 **▶ Fly this segment**（没选词时它灰着，写
  "select a word first"；飞行中写 "Flying …"；飞完变成 **↻ Fly again**）。左侧面板最上面的 **Autopilot (live)** 一栏里有开关
  "fly a word's segment as soon as its band is clicked"（默认开）：开着时点色块就直接飞，不必再按按钮。只有点击才请求（按钮或
  色块，`trainingPick`），游标不请求——图表上悬停就会移动游标，而飞出的线正是要悬停着看的；同一段再飞一次是同一选择的新一次
  尝试；换了航班，上一架的选择不再飞。
...
- **结果卡（面板最上面一栏）**：标题是词与步数（航向词写出多飞到哪一步）；第一行大字是判定，颜色同航迹；然后并排两个数——
  **模拟飞行时间**（下面是观测飞机飞同一段的时间）与**计算用时**（后端从开始处理到答复；下面是浏览器往返时间）；再是判定依据
  ...
  的写毫秒）、用的规格、代码 sha、词钟。后端没开、拒绝、答复对不上时说清楚原因。
- **三维**：一架飞机从说这个词的地方把这一段"飞出来"（按实际时间加速，至少 8 倍、整段不超过 20 s），标签走一个模拟时钟
  ...
  行在地面投影上画红。"Replay in 3D" 重放同一个答复，不再请求。不用共享的 Cesium 时钟。
