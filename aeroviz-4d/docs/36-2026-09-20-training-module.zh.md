...
| **T5 ← 下一步** | 几何航迹（Python）：§5.3 的运动学，写进 `sample.json` 的 `geometric`；常数全部 import，不抄 | 导出器里一个独立模块 + 单元测试 | 三个手算用例：保持航向直飞、转 90° 的半径对得上 `route_turn_radius_m`、从 1000 m 降到 0 的时间对得上 12 s 收敛 + 6° 上限 | T3 |
...
**进度（2026-09-20 晚）**：T1 / T2 / T3 / T4a / T4b 已完成，导出已跑，界面上是真数据。下一步 T5（几何航迹）。
