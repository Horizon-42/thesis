...
import type { ProcedureBranchRenderArgs } from "./procedureSceneRenderer";
...
export function isProcedureAnnotationLabelId(entityId: string): boolean {
  return entityId.startsWith(PROCEDURE_ANNOTATION_LABEL_PREFIX);
}

export function isProcedureMeasurementEntityId(entityId: string): boolean {
  return (
...
export function procedureEntityShow(
  baseVisible: boolean,
...
function protectionSurfaceForSegment(
  ...
  return branchBundle.protectionSurfaces.find(
    (surface) => surface.segmentId === segmentId && surface.kind === kind,
  ) ?? null;
}
...
function addSegmentEntities(
  ...
  if (isFinalSegment(segmentBundle.segment)) {
    const verticalReferencePoints = finalVerticalReferencePoints(
      segmentBundle,
    ...
    if (verticalReferencePoints.length >= 2) {
      ...
      addPolyline(
        ...
        verticalReferencePoints.map((point) =>
          elevatedPoint(point, FINAL_VERTICAL_REFERENCE_HEIGHT_OFFSET_M),
        ),
        procedureEntityShow(visible, verticalReferenceAnnotation, displayLevel),
        5,
  ...
  if (!protectionSurfaceForSegment(branchBundle, segmentBundle.segment.segmentId, "FINAL_LNAV_OEA")) {
    ids.push(
...
function addBranchProtectionSurfaceEntities(
  ...
  protectionSurfaces.forEach((surface) => {
    ...
    if (surface.kind === "MISSED_CONNECTOR") {
      ...
      const baseConnectorId =
        `${PROCEDURE_SEGMENT_ENTITY_PREFIX}${bundle.packageId}-${branchBundle.branchId}` +
        `-missed-connector-surface-${index}`;
      const targetFixNote = surface.lateral.notes.find((note) => note.startsWith("Target fix "));
      const targetFixLabel = targetFixNote
        ?.replace(/^Target fix\s+/, "")
        .replace(/\.$/, "");
      const primaryId = `${baseConnectorId}-primary`;
      const annotation = annotationBase({
        entityId: primaryId,
        label: targetFixLabel ? `Connector to ${targetFixLabel}` : "Missed connector",
        ...
        legs: segmentBundle?.legs,
        parameters: [
          param("Source CA leg", surface.sourceLegIds.join(", ")),
          param("Target fix", targetFixLabel),
...
function addBranchMissedCaMahfConnectorEntities(
  ...
  (branchBundle.missedCaMahfConnectors ?? []).forEach((connector, index) => {
    ...
    addPolyline(
      ...
      connector.geoPositions.map((point) =>
        elevatedPoint(point, CA_MAHF_CONNECTOR_HEIGHT_OFFSET_M),
      ),
      procedureEntityShow(visible, annotation, displayLevel),
      4,
...
export function procedureBranchVisible(
  pkg: ProcedurePackage | null,
  branchId: string,
...
function addProcedureBranchEntities(
  viewer: Cesium.Viewer,
  bundle: ProcedureRenderBundle,
  branchBundle: BranchGeometryBundle,
  branchIndex: number,
...
export function renderProcedureBranchEntities(args: ProcedureBranchRenderArgs): string[] {
  return addProcedureBranchEntities(
    args.viewer,
    args.bundle,
    args.branchBundle,
    args.branchIndex,
    args.pkg,
    args.visible,
    args.annotationVisible,
    args.widthMeasurementVisible,
    args.displayLevel,
  );
}
