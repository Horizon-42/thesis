 ...
 *
 * The executor flew the VECTORED flight (landed; the clearance's corridor was left for five rows, so that one word is
 * outside) and did not fly the STRAIGHT-IN one (no identified type). The prior ranks, at every step, the truth's word
...
const word = (row: number, column: number, value: number, status: string, checks: unknown[] = [], reason: string | null = null) =>
  ({ row, column, value, status, flownRow: row, checks, reason });
    ...
    flights: [
      {
        ...
        counts: { wordsJudged: 6, wordsInside: 5, headingWordsNotJudged: 0 },
        track: {
          ...
          trackDeg: range(rows).map((row) => (row < 10 ? 270 : row < 25 ? 180 : 90)), distanceM: range(rows, 0, 200),
        },
        words: [
          ...
          word(0, 2, WORD.heading270, "inside", [check("hold in its funnel", true, 11, 11)]),
          ...
          word(10, 1, WORD.cleared, "outside", [check("capture turn monotone", true), check("corridor held to the landing", false, 30, 35)]),
          word(10, 2, WORD.heading180, "inside", [check("turn reached its target", true), check("turn monotone", true), check("turn rate and bank", true)]),
