...
def rollout_piecewise_constant(
    ...
    *,
    ...
    """Integrate ``controls`` (one per segment of ``[0, total_duration]``) forward.

    By default the segments are equal (``Δ = total_duration / len(controls)``). Pass
    ``segment_durations`` (one per control, e.g. a multiphase optimiser's non-uniform per-phase
    segment times) to use those instead; ``total_duration`` is then their sum.

    Within a segment the simulator is stepped at
    ...
    if segment_durations is not None:
        if len(segment_durations) != segment_count:
            raise ValueError("segment_durations must align 1:1 with controls")
        boundaries, acc = [], 0.0
        ...
        boundaries = [(i + 1) * segment_duration for i in range(segment_count)]
    ...
    for segment_index, control in enumerate(controls):
        segment_end = boundaries[segment_index]
