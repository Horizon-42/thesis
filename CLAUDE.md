...
- **A flight's identity is `flight_key` = `id_runway_icao24_landingTime` (`flight_scenarios.identity`), NEVER `id` alone — and this has bitten three separate layers.** The raw harvest carries **no unique flight id at all**: `id` is a copy of the callsign, and OpenSky stores state vectors by icao24 + time, so an "arrival" is a segment this project derives — identity is (which aircraft, when). Measured on the 996 KRDU arrivals: `id` → 552 distinct, `icao24` → 717, `id_runway` → 778, `id_runway_icao24` → 874, **`icao24`+landing time → 996**. The extra fields in `flight_key` are for filename readability, not uniqueness. Casualties so far: the ts train/val/test split (leaked; `predict --split test` returned every namesake, 48 flights for an 18-flight split), and the comparison-CZML group key (`id_runway` silently dropped 22% of a full batch). The same function produces the ts record stems, the optimizer's record filenames (`_scenario_filename` wraps it), and now the CZML group key (via the record filename stem), so they cannot drift.
...
- **Comparison CZML reference tracks can be the wrong flight for a duplicated callsign** (the last unfixed corner of the `flight_key` gotcha above). `_reference_entity_from_adsb` looks the observed track up by callsign in `trajectories.czml`, which disambiguates duplicates POSITIONALLY (`ASA677`, `ASA677_2`, …), and the summary row carries only the callsign — so the right `_N` is not recoverable. Predictions and ALL metrics are correct (evaluation reads `references/{flight_key}_reference_eval.json`); only the white line is at risk. Real fix = id `generate_czml` entities by `flight_key` too, which changes the observed-layer entity ids `selectedFlightId`/FlightTable depend on — bigger blast radius, deliberately deferred.
