...
- ~~ts prediction categories unverified in-browser~~ — **VERIFIED in-browser 2026-07-20** (Linux Chrome, KRDU): all 4 `Predicted (…)` categories load, purple prediction + white reference paths render together, the `Predicted` legend checkbox removes only the purple, and the Optimization panel's metrics track the selected category (752 m / 2.0% for iTransformer full vs 3184 m / 0.0% for PatchTST window — matching the evaluation reports). Note the comparison entities are time-windowed: at a clock time outside a group's availability the scene is legitimately empty, so pause inside a window (≈08:09 UTC on the KRDU data) before concluding nothing renders.
