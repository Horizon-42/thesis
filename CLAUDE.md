...
- ~~Pre-existing numpy failure in `collocation/tests/test_optimizer.py::test_fixed_time_objective_weights_control_effort_at_one`~~ — did NOT reproduce on 2026-07-20, still green after the prediction-overlay merge (full suite: 699 pass, both groups exit 0). `run_all_tests.sh`'s header still documents it as expected; drop that paragraph if it stays green.
...
- **Observed-track evaluation is designed but NOT built** (the original 2026-07-20 request). `evaluation/` is unchanged so far. Needs: an explicit arrival-event concept (observed `states[-1]` is not the arrival — see the gotcha), an established-on-final precondition with `not_established` as a first-class outcome, an observed-batch writer emitting a `summary.json` roster (precedent: `ts_transformer/export.py`), publication as a comparison category, and a `subject` field so the frontend stops reporting a meaningless `solve_rate` of 1.0 for observed data. The frontend needs NO new plumbing — `evaluationReport.ts` / `OptimizationSummary.tsx` / `EvaluationReportWindow.tsx` already fetch and render this schema.
