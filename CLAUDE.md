...
- ~~Pre-existing numpy failure in `collocation/tests/test_optimizer.py::test_fixed_time_objective_weights_control_effort_at_one`~~ — did NOT reproduce on 2026-07-20 (full suite: 692 pass, both groups exit 0). `run_all_tests.sh`'s header still documents it as expected; drop that paragraph if it stays green.
