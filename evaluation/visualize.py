...
<p class="note">Runway-threshold geometric verdict. Lateral is half the published runway width (did the crossing lie over the pavement) — not a navigation-containment bound. Vertical uses the published-TCH path and the 22 m RNAV/RNP terminal vertical bound. Speed uses the stall-anchored window [1.23·Vs(n), 1.23·Vs1g + 20 kt] at the record's crossing mass and load factor n: computed subjects are judged on the crossing model airspeed, observed baselines on the fitted crossing GROUND speed corrected by the field's METAR headwind into an airspeed estimate (shown as est, ±5 kt declared) when a report is usable, else on the raw ground speed as a stated proxy (shown as GS); their load factor is inverted from the flight's own final 20 s of kinematics. This is not touchdown or landing certification. Observed events reuse the producer-side threshold event, carry explicitly uncalibrated uncertainty, and are never refitted by evaluation.</p>
