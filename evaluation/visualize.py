...
<p class="note">Runway-threshold geometric verdict. LPV vertical and overall remain indeterminate when the validated RTCA vertical scale is unavailable. Observed events reuse the runway-assignment fit; evaluation does not refit ADS-B.</p>
...
Plotly.newPlot("vert",[{type:"bar",x:M.map(r=>r.file||r.id),y:M.map(r=>r.vertical_m),marker:{color:colors}}],{title:"Signed vertical deviation (m); missing LPV bound = indeterminate",margin:{b:90}},{displayModeBar:false});
...
document.getElementById("rows").innerHTML="<tr><th>flight identity</th><th>subject</th><th>benchmark</th><th>event</th><th>lateral</th><th>vertical</th><th>overall</th><th>x (m)</th><th>z (m)</th><th>reason</th></tr>"+ROWS.map(r=>`<tr><td>${esc(r.flight_key||r.file||r.id)}</td><td>${r.subject}</td><td>${r.benchmark}</td><td>${r.event_status}</td><td>${r.lateral_result}</td><td>${r.vertical_result}</td><td>${r.verdict}</td><td>${r.cross_track_m===undefined?"—":r.cross_track_m.toFixed(2)}</td><td>${r.vertical_m===undefined?"—":r.vertical_m.toFixed(2)}</td><td>${esc(r.reason)}</td></tr>`).join("");</script></body></html>"""
